...
%% @doc Attempt to monitor pid as port, and port as pid
mon_port_invalid_type(doc) -> "monitor/2: Monitor pid as port and port as pid";
mon_port_invalid_type(suite) -> [];
mon_port_invalid_type(Config) when is_list(Config) ->
    Port = hd(erlang:ports()),
    ?assertError(badarg, erlang:monitor(port, self())),
    ?assertError(badarg, erlang:monitor(process, Port)),
    ok.

%% @doc 1. With local port
mon_port_local(doc) -> "monitor/2: Local port";
...
mon_port_remote_on_remote(doc) ->
    "monitor/2: Remote port pointing to remote node";
mon_port_remote_on_remote(suite) -> [];
mon_port_remote_on_remote(Config) when is_list(Config) ->
...
%% @doc 4. Remote port belongs to this node and does not exist
%% Port4 produces #Port<0.167772160> which should not exist in a test run
mon_port_bad_remote_on_local(doc) ->
    "monitor/2: Remote non-existent port pointing to local node";
mon_port_bad_remote_on_local(suite) -> [];
mon_port_bad_remote_on_local(Config) when is_list(Config) ->
...
%% @doc 5. Monitor owner (origin) dies before port is closed
mon_port_origin_dies(doc) -> "monitor/2: Monitor origin dies early";
mon_port_origin_dies(suite) -> [];
mon_port_origin_dies(Config) when is_list(Config) ->
...
%% @doc 6. Monitor a named port
mon_port_named(doc) -> "monitor/2: Local port by name";
mon_port_named(suite) -> [];
mon_port_named(Config) when is_list(Config) ->
    Name6 = test_port6,
...
%% @doc 7. Named does not exist: Should succeed but immediately send 'DOWN'
mon_port_bad_named(doc) -> "monitor/2: Monitor by non-existing name";
mon_port_bad_named(suite) -> [];
mon_port_bad_named(Config) when is_list(Config) ->
    Name7 = test_port7,
...
%% @doc 8. Monitor a pid and demonitor by ref
mon_port_pid_demonitor(doc) -> "monitor/2: Monitor by port id and demonitor";
mon_port_pid_demonitor(suite) -> [];
mon_port_pid_demonitor(Config) when is_list(Config) ->
...
%% @doc 9. Monitor by name and demonitor by ref
mon_port_name_demonitor(doc) -> "monitor/2: Monitor by name and demonitor";
mon_port_name_demonitor(suite) -> [];
mon_port_name_demonitor(Config) when is_list(Config) ->
    Name9 = test_port9,
