...
#define PRIMOP_LIST(ATOM,ADDRESS)	\
    { {0,_unchecked_atom_val(ATOM)}, (const void *) ADDRESS },
...
static struct primop *primop_table_get(Eterm name)
{
    ...
    return (struct primop *) hash_get(&primop_table, &tmpl);
}
...
static struct hipe_mfa_info **hipe_mfa_info_table_alloc_bucket(unsigned int size)
{
    ...
    struct hipe_mfa_info **bucket = (struct hipe_mfa_info **) erts_alloc(ERTS_ALC_T_HIPE, nbytes);
...
BIF_RETTYPE hipe_bifs_add_ref_2(BIF_ALIST_2)
{
    ...
    refers_to = (struct hipe_mfa_info_list *) erts_alloc(ERTS_ALC_T_HIPE, sizeof(*refers_to));
