...
Process *hipe_mode_switch(Process *p, unsigned cmd, Eterm reg[])
{
 ...
 do_return_from_native:
    ...
    switch (result) {
	   ...
	   * stack: to this_ end hipe_${ARCH}_glue.S stores the BIF's
	  ...
	  unsigned int i, is_recursive = 0;
	      ...
	      /* Some architectures (risc) need this_ re-reset of nsp as the
	  ...
	  if ((Sint)closure->fe->address[-1] < 0) {
	      ...
	      /* Update arity to reflect the new_ parameters. */
	      arity += i;
