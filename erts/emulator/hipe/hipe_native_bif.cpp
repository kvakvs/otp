...
BIF_RETTYPE hipe_set_timeout(BIF_ALIST_1)
{
    ...
    /*
     * def_arg_reg[0] is (re)set unconditionally, in case this_ is the
     * 2nd/3rd/... iteration through the receive loop: in order to pass
     * a boolean flag to native code indicating timeout or new_ message,
...
static void hipe_save_stacktrace(Process* c_p, Eterm args)
{
    ...
    /* Must mark this_ as a native-code exception. */
