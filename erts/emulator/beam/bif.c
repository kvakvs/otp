...
demonitor_local_port(Process *origin, Eterm ref, Eterm target)
{
    ...
    mon = erts_remove_monitor(&ERTS_P_MONITORS(origin), ref);
    ...
    if (port) {
        Eterm trap_ref;
        switch (erts_port_demonitor(origin, port, ref, &trap_ref)) {
        case ERTS_PORT_OP_DROPPED:
        case ERTS_PORT_OP_BADARG:
            break;
        case ERTS_PORT_OP_SCHEDULED:
 ...
 */
static Eterm demonitor(Process *c_p, Eterm ref, Eterm *multip)
{
...
BIF_RETTYPE demonitor_1(BIF_ALIST_1)
{
    ...
    case am_false:      BIF_RET(am_false);
...
{
    Eterm ref = erts_make_ref(origin);
    ...
    if (IS_TRACED_FL(origin, F_TRACE_PROCS)) {
        trace_proc(origin, ERTS_PROC_LOCK_MAIN, origin, am_monitor, target);
    }

    if (!port) {
res_no_proc:
        erts_smp_proc_lock(origin, p_locks);
        /* Send the DOWN message immediately. Ref is made on the fly */
        erts_queue_monitor_message(origin, &p_locks, ref,
                                   am_port, target, am_noproc);
        erts_smp_proc_unlock(origin, p_locks);
    }
    else {
...
{
    Eterm ret = erts_make_ref(self);

    ErtsProcLocks p_locks = ERTS_PROC_LOCK_MAIN | ERTS_PROC_LOCK_LINK;
    Process     *proc = NULL;
    Port        *port = NULL;
    Eterm       whereis = erts_whereis_name_to_id(self, target_name);

    erts_smp_proc_lock(self, ERTS_PROC_LOCK_LINK);

    if (is_pid(whereis)) {
        proc = erts_pid2proc(self, p_locks, whereis, ERTS_PROC_LOCK_LINK);
...
local_pid:
    ...
    } else if (is_internal_port(target)) {
local_port:
        ret = local_port_monitor(BIF_P, target);
    } else if (is_external_port(target)) {
