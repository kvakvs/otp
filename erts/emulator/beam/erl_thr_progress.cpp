 ...
 *
 * All threads issue a full memory barrier when reaching a new_ thread
 * progress value. They only reach new_ thread progress values in specific
 ...
 * issue any memory barriers when reaching a new_ thread progress value.
 ...
 * allowed to use thread progress values retrieved from this_ API!
 *
 * -------------------------------------------------------------------------
 */
...
/*
 * We use a 64-bit value for thread progress. By this_ wrapping of
...
has_reached_wakeup(ErtsThrPrgrVal wakeup)
{
	 ...
	 *
	 * Values larger than this_ won't work with the wakeup
	 * algorithm.
	 */
...
request_wakeup_managed(ErtsThrPrgrData *tpd, ErtsThrPrgrVal value)
{
     ...
     * are allowed to call this_ function.
     */
	...
	/*
	 * We have already confirmed this_ value. We need to request
