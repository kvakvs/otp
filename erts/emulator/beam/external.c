...
static B2TContext* b2t_export_context(Process* p, B2TContext* src)
{
    ...
    B2TContext *ctx = (B2TContext *)ERTS_MAGIC_BIN_DATA(context_b);
    Eterm* hp;
				      ...
				      Export *bif_init, Eterm arg0, Eterm arg1)
{
	...
	ctx->arg[1] = arg1;
        ...
        is_first_call = 0;
        ctx = (B2TContext *)ERTS_MAGIC_BIN_DATA(context_b);
...
{
    TTBContext *context = (TTBContext *)ERTS_MAGIC_BIN_DATA(context_bin);
    if (context->alive) {
	context->alive = 0;
	switch (context->state) {
	case TTBSize:
...
#define EXPORT_CONTEXT()						\
	...
	if (context_b == NULL) {					\
	    context_b = erts_create_magic_binary(sizeof(TTBContext),    \
            ...
            context = (TTBContext *)ERTS_MAGIC_BIN_DATA(context_b);			\
