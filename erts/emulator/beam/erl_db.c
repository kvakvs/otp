...
struct MetaNameTabEntry {
    ...
    union {
	DbTable *tb;
        struct MetaNameTabEntry *mvec;
    }pu;
};
static MetaNameTabEntry *meta_name_tab;

static unsigned meta_name_tab_mask;

static ERTS_INLINE
struct MetaNameTabEntry *meta_name_tab_bucket(Eterm name,
                                              erts_smp_rwmtx_t** lockp)
{
    unsigned bix = atom_val(name) & meta_name_tab_mask;
    struct MetaNameTabEntry *bucket = &meta_name_tab[bix];
#ifdef ERTS_SMP
			  ...
			  int meta_already_locked)
{
	...
	if (slot < db_max_tabs && IS_SLOT_ALIVE(slot))
        ...
        struct MetaNameTabEntry *bucket = meta_name_tab_bucket(id,&mtl);
	if (!meta_already_locked)
	    erts_smp_rwmtx_rlock(mtl);
	else{
...
static int insert_named_tab(Eterm name_atom, DbTable* tb, int have_lock)
{
    ...
    struct MetaNameTabEntry *new_entry;
    struct MetaNameTabEntry *bucket = meta_name_tab_bucket(name_atom,
							      &rwlock);
    if (!have_lock)
	...
	new_entry = bucket;
        ...
        struct MetaNameTabEntry *entries;
	Uint cnt;
	if (is_atom(bucket->u.name_atom)) { /* single */
	    size_t size;
	    ...
	    cnt = 2;
            size = sizeof(struct MetaNameTabEntry)*cnt;
            entries = (struct MetaNameTabEntry *)
                    erts_db_alloc_nt(ERTS_ALC_T_DB_NTAB_ENT, size);
		...
		if (bucket->pu.mvec[i].u.name_atom == name_atom) {
            ...
            old_size = sizeof(struct MetaNameTabEntry)*cnt;
            size = sizeof(struct MetaNameTabEntry)*(cnt+1);
            entries = (struct MetaNameTabEntry *)
                    erts_db_realloc_nt(ERTS_ALC_T_DB_NTAB_ENT,
					 bucket->pu.mvec,
					 old_size,
					 size);
...
static int remove_named_tab(DbTable *tb, int have_lock)
{
	...
	if (cnt == 2) { /* multi -> single */
	    size_t size;
            struct MetaNameTabEntry* entries = bucket->pu.mvec;
	    *bucket = entries[1-i];
            size = sizeof(struct MetaNameTabEntry)*cnt;
		...
		bucket->pu.mvec[i] = bucket->pu.mvec[cnt];
            ...
            old_size = sizeof(struct MetaNameTabEntry)*(cnt+1);
            size = sizeof(struct MetaNameTabEntry)*cnt;
            bucket->pu.mvec = (struct MetaNameTabEntry *)
                    erts_db_realloc_nt(ERTS_ALC_T_DB_NTAB_ENT,
						 bucket->pu.mvec,
						 old_size,
						 size);
...
void init_db(ErtsDbSpinCount db_spin_count)
{
    ...
    size = sizeof(struct MetaNameTabEntry)*(meta_name_tab_mask+1);
    meta_name_tab = (MetaNameTabEntry *)erts_db_alloc_nt(ERTS_ALC_T_DB_TABLES, size);
    ERTS_ETS_MISC_MEM_ADD(size);
