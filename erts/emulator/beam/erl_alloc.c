...
erts_alloc_fix_alloc_shrink(int ix, erts_aint32_t flgs)
{
	...
	return erts_alcu_fix_alloc_shrink(
            (Allctr_t *)erts_allctrs_info[ERTS_ALC_A_FIXED_SIZE].extra, flgs);
#else
...
UWord erts_alc_test(UWord op, UWord a1, UWord a2, UWord a3)
{
...
#endif
	case 0xf08: {
            ethr_mutex *mtx = (ethr_mutex *)erts_alloc(ERTS_ALC_T_UNDEF, sizeof(ethr_mutex));
	...
	case 0xf0b:
	    ...
	    break;
	case 0xf0c: {
            ethr_cond *cnd = (ethr_cond *)erts_alloc(ERTS_ALC_T_UNDEF, sizeof(ethr_cond));
	...
	case 0xf10: {
            ethr_tid *tid = (ethr_tid *)erts_alloc(ERTS_ALC_T_UNDEF, sizeof(ethr_tid));
	...
	case 0xf16: {
		...
		ts->allctr[i]->mbc_header_size += extra_hdr_sz;
                typedef void (*Fn)(Allctr_t *, Carrier_t *);
                ts->allctr[i]->creating_mbc = (Fn)new_creating_mbc;
                ts->allctr[i]->destroying_mbc = (Fn)new_destroying_mbc;
	    }
