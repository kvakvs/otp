...
erts_bif_trace(int bif_index, Process* p, Eterm* args, BeamInstr* I)
{
    ...
    func = (decltype(func))bif_table[bif_index].f;
...
erts_trace_time_call(Process* c_p, BeamInstr* I, BpDataTime* bdt)
{
	...
	pbt = (decltype(pbt))Alloc(sizeof(process_breakpoint_time_t));
		   ...
		   enum erts_break_op count_op, ErtsTracer tracer)
{
	...
	g = (decltype(g))Alloc(sizeof(GenericBp));
	g->orig_instr = *pc;
	...
	bmt = (decltype(bmt))Alloc(sizeof(BpMetaTracer));
	...
	bcp = (decltype(bcp))Alloc(sizeof(BpCount));
	...
	bdt = (decltype(bdt))Alloc(sizeof(BpDataTime));
	erts_refc_init(&bdt->refc, 1);
	bdt->n = erts_no_schedulers;
	bdt->hash = (decltype(bdt->hash))Alloc(sizeof(bp_time_hash_t)*(bdt->n));
	for (i = 0; i < bdt->n; i++) {
	    bp_hash_init(&(bdt->hash[i]), 32);
	}
	bp->time = bdt;
