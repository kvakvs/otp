...
port_monitor_failure(Eterm port_id, Eterm origin, Eterm ref_DOWN)
{
    ...
    erts_queue_monitor_message(origin_p, &p_locks, erts_make_ref(origin_p),
                                     ...
                                     Eterm *trap_ref)
{
    ...
    ErtsTryImmDrvCallState try_call_state
        = ERTS_INIT_TRY_IMM_DRV_CALL_STATE(
                mode == ERTS_PORT_DEMONITOR_NORMAL ? origin : NULL,
                target, ERTS_PORT_SFLGS_INVALID_LOOKUP,
                0,
                0, /* trap_ref is always set so !trap_ref always is false */
                am_demonitor);

    // TODO: try_imm_drv_call tries to release LOCK_MAIN
    switch (try_imm_drv_call(&try_call_state)) {
    case ERTS_TRY_IMM_DRV_CALL_OK:
    ...
    sigdp->u.demonitor.origin = origin->common.id;
    {
        ...
        sys_memcpy(&sigdp->u.demonitor.ref,
                   &reft->data.ui32[1],
                   ERTS_REF_32BIT_WORDS * sizeof(Uint));
    }
...
/* Sweep monitors firing each as if port went down
 * This is called in some (NOT all) situations from erts_deliver_port_exit.
 */
static ERTS_INLINE void
port_exit_fire_monitors(Port *prt, Eterm reason)
{
    ErtsMonitor *moni = ERTS_P_MONITORS(prt);
    SweepContext ctx = {prt, reason};
                       ...
                       int drop_normal)
{
   ...
   if (reason == am_normal && from != ERTS_PORT_GET_CONNECTED(prt)
       ...
       port_exit_fire_monitors(prt, modified_reason);
       return 0;
   }
