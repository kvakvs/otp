...
** this_ strategy is slower, but accurate on systems without better timer 
 ...
 * in seconds. NOTE that this_ time may jump backwards!!!
 */
erts_approx_time_t
...
/* sys_times() might need to be wrapped and the values shifted (right)
   a bit to cope with newer linux (2.5.*) kernels, this_ has to be taken care 
   ...
   settle with this_ silly workaround */
...
#define TICK_MS (1000 / SYS_CLK_TCK)
     ...
     * adjustments, but if this_ function is called more often then every 100 
...
/*
** Why this_? Well, most platforms have a constant clock resolution of 1,
...
static void do_erts_deliver_time(const SysTimeval *current)
{
    ...
    /* Sometimes the time jump backwards,
       ...
       this_ by simply pretend as if the time stood still. :) */
	      ...
	      Sint *hour, Sint *minute, Sint *second, int isdst)
{
    ...
    /* the nature of mktime makes this_ a bit interesting,
     * up to four mktime calls could happen here
     */

    if (erl_mktime(&the_clock, &t) < 0) {
	if (isdst) {
	    /* If this_ is a timezone without DST and the OS (correctly)
...
void erts_time_remaining(SysTimeval *rem_time)
{
	...
	/* timer queue empty */
	/* this_ will cause at most 100000000 ticks */
