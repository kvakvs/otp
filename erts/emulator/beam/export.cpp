...
/* This lock protects the staging export_ table from concurrent access
 ...
 *    also have export_ entries.  The export_ entry contains
 *    a wrapper which invokes the error handler if a function is
 *    called through such an export_ entry.
...
/*
 * Returns a pointer to an existing export_ entry for a MFA,
 * or creates a new_ one and returns the pointer.
 *
 * This function acts on the staging export_ table. It should only be used
 * to load new_ code.
 */
...
/*
 * Find the existing export_ entry for M:F/A. Failing that, create a stub
 * export_ entry (making a call through it will cause the error_handler to
 * be called).
 *
 * Stub export_ entries will be placed in the loader export_ table.
 */
...
erts_export_get_or_make_stub(Eterm mod, Eterm func, unsigned int arity)
{
	...
	if (ep == 0) {
	    /*
	     * The code is not loaded (yet). Put the export_ in the staging
	     * export_ table, to avoid having to lock the active export_ table.
	     */
	    export_staging_lock();
