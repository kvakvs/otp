...
BIF_RETTYPE erl_ddll_try_load_3(BIF_ALIST_3)
{
  ...
  if ((name = pick_list_or_atom(name_term)) == nullptr) {
    goto error;
  }
  ...
  path = erts_convert_filename_to_encoding(path_term, nullptr, 0,
  ...
  if ((drv = lookup_driver(name)) != nullptr) {
...
/*
   ...
   remove/decrement this_ pid (which should be an LOADED tagged one).
   If the state is RELOADING, this_ pid should be in list as LOADED tagged,
...
Eterm erl_ddll_try_unload_2(BIF_ALIST_2)
{
  ...
  if ((pe = find_proc_entry(dh, BIF_P, ERL_DE_PROC_LOADED)) == nullptr) {
  ...
  if (dh->status == ERL_DE_RELOAD ||
    ...
    dh->reload_full_path = dh->reload_driver_name = nullptr;
    dh->reload_flags = 0;
  }
...
BIF_RETTYPE erl_ddll_info_2(BIF_ALIST_2)
{
  ...
  case am_permanent:
    if (drv->handle != nullptr && drv->handle->status == ERL_DE_PERMANENT) {
      res = am_true;
    } else {
      res = am_false;
    }
...
BIF_RETTYPE erl_ddll_format_error_int_1(BIF_ALIST_1)
{
  ...
  case am_not_loaded_by_this_process:
    errstring = "DDLL driver was not loaded by this_ process";
    break;

  case am_not_pending:
    errstring = "DDLL load not pending for this_ driver name";
    break;
                               ...
                               ErtsProcLocks plocks)
{
  ...
  if ((name = pick_list_or_atom(tp[1])) == nullptr) {
    BIF_ERROR(p, BADARG);
  }
...
static void dereference_all_processes(DE_Handle *dh)
{
  ...
  for (p  = dh->procs; p != nullptr; p = p->next) {
...
{
  assert_drv_list_locked();
  return ((dh == nullptr) || (dh->status != ERL_DE_FORCE_UNLOAD &&
                              dh->status != ERL_DE_FORCE_RELOAD));
}
...
char *erts_ddll_error(int code)
{
  ...
  case ERL_DE_ERROR_NO_DDLL_FUNCTIONALITY:
    return "DDLL functionality not available on this_ platform";
...
static Eterm notify_when_loaded(Process *p, Eterm name_term, char *name, ErtsProcLocks plocks)
{
  ...
  default:
    erl::exit(1, "Internal error, unknown state %u in dynamic driver.", drv->handle->status);
  }
...
static void add_proc_loaded(DE_Handle *dh, Process *proc)
{
  DE_ProcEntry *p;
  assert_drv_list_rwlocked();
  p = (DE_ProcEntry *)erts_alloc(ERTS_ALC_T_DDLL_PROCESS, sizeof(DE_ProcEntry));
  p->proc = proc;
  p->flags = 0;
...
static void set_driver_reloading(DE_Handle *dh, Process *proc, char *path, char *name, size_t flags)
{
  ...
  dh->reload_full_path = (char *)erts_alloc(ERTS_ALC_T_DDLL_HANDLE, sys_strlen(path) + 1);
  strcpy(dh->reload_full_path, path);
  dh->reload_driver_name = (char *)erts_alloc(ERTS_ALC_T_DDLL_HANDLE, sys_strlen(name) + 1);
...
static int do_load_driver_entry(DE_Handle *dh, char *path, char *name)
{
  ...
  if ((res =  erts_sys_ddll_open(path, &(dh->handle), nullptr)) != ERL_DE_NO_ERROR) {
    return res;
  }
  ...
  dp = (ErlDrvEntry *)erts_sys_ddll_call_init(init_handle);
  ...
  dh->full_path = (char *)erts_alloc(ERTS_ALC_T_DDLL_HANDLE, sys_strlen(path) + 1);
...
{
  int res;
  auto dh = (DE_Handle *)erts_alloc(ERTS_ALC_T_DDLL_HANDLE, sizeof(DE_Handle));
  ...
  dh->status = -1;
  dh->reload_full_path = nullptr;
  dh->reload_driver_name = nullptr;
  dh->reload_flags = 0;
  dh->full_path = nullptr;
  dh->flags = 0;
...
static void unload_driver_entry(DE_Handle *dh)
{
  do_unload_driver_entry(dh, nullptr);

  if (dh->full_path != nullptr) {
    erts_free(ERTS_ALC_T_DDLL_HANDLE, (void *) dh->full_path);
  }
...
static int errdesc_to_code(Eterm errdesc, int *code /* out */)
{
  ...
  if (is_atom(errdesc)) {
    ...
    for (i = 0; errcode_tab[i].atm != nullptr; ++i) {
...
static Eterm mkatom(const char *str)
{
  return erts_atom_put((uint8_t *) str,
                       sys_strlen(str),
  ...
  for (pe = dh->procs; pe != nullptr; pe = pe->next) {
    ...
    if (i < num_pei) {
      ...
      if (num_pei >= num_pei_allocated) {
        pei = (pei == nullptr)
              ? (ProcEntryInfo *)erts_alloc(ERTS_ALC_T_DDLL_TMP_BUF,
                                            sizeof(ProcEntryInfo) * (num_pei_allocated = 10))
              : (ProcEntryInfo *)erts_realloc(ERTS_ALC_T_DDLL_TMP_BUF, pei,
                                              sizeof(ProcEntryInfo) * (num_pei_allocated += 10));
      }
...
static erts_driver_t *lookup_driver(const char *name)
{
  ...
  for (drv = driver_list; drv != nullptr && strcmp(drv->name, name); drv = drv->next)
    ;
