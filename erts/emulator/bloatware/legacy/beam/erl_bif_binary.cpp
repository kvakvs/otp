...
static void ac_restore_find_all(ACFindAllState *state, char *buff)
{
  ...
  if (state->allocated > 0) {
    state->out = (FindallData *)erts_alloc(ERTS_ALC_T_TMP,
...
{
  int i;
  auto cd = ERTS_MAGIC_BIN_DATA<CommonData *>(bp);
...
static BIF_RETTYPE do_longest_common(Process *p, Eterm list, int direction)
{
  ...
  cd = ERTS_MAGIC_BIN_DATA<CommonData *>(mb);
  l = list;
    ...
    Eterm orig_list, int direction)
{
  ...
  cd = ERTS_MAGIC_BIN_DATA<CommonData *>(bin);
  ...
  if ((target_size - size) >= reds) {
    ...
    if (*(binary_val(orig)) == HEADER_PROC_BIN) {
      ...
      if (!(cbs->temp_alloc)) { /* alignment not needed, need to copy */
        uint8_t *tmp = (uint8_t *)erts_alloc(ERTS_ALC_T_BINARY_BUFFER, size);
    ...
    t = (uint8_t *) cbs->result->orig_bytes; /* No offset or anything */
    pos = 0;
    i = 0;
