...
typedef struct {
  size_t value;     /* Value of label (nullptr if not known yet). */
  size_t patches;   /* Index (into code buffer) to first location
         * which must be patched with the value of this_ label.
         */
#ifdef ERTS_SMP
  size_t looprec_targeted;  /* Non-zero if this_ label is the target of a loop_rec
         * instruction.
         */
#endif
} Label;
...
typedef struct {
  ...
  int arity;      /* Arity. */
  size_t patches;   /* Index to locations in code to
...
#define MakeIffId(a, b, c, d) \
  (((size_t) (a) << 24) | ((size_t) (b) << 16) | ((size_t) (c) << 8) | (size_t) (d))
...
typedef struct {
  ...
  size_t heap_size;   /* (Exact) size on the heap. */
...
typedef struct LoaderState {
  ...
  struct {
    uint8_t *start;   /* Start of chunk (in binary). */
  ...
  uint8_t *code_start;    /* Start of code file. */
  ...
  uint8_t mod_md5[16];    /* MD5 for module code. */
  int may_load_nif;           /* true if NIFs may later be loaded for this_ module */
  ...
  ExportEntry *export_; /* Pointer to export_ table. */
  ...
  size_t total_literal_size;  /* Total heap size for all literals. */
...
static Eterm insert_new_code(Process *c_p, ErtsProcLocks c_p_locks,
                             ...
                             BeamInstr *code, size_t size);
static int init_iff_file(LoaderState *stp, uint8_t *code, size_t size);
static int scan_iff_file(LoaderState *stp, size_t *chunk_types,
                         size_t num_types, size_t num_mandatory);
...
static int get_tag_and_value(LoaderState *stp, size_t len_code,
                             unsigned tag, BeamInstr *result);
static int new_label(LoaderState *stp);
static void new_literal_patch(LoaderState *stp, int pos);
static void new_string_patch(LoaderState *stp, int pos);
static size_t new_literal(LoaderState *stp, Eterm **hpp, size_t heap_size);
...
int patch(Eterm Addresses, size_t fe);
static int safe_mul(UWord a, UWord b, UWord *resp);

static int must_swap_floats;

size_t erts_total_code_size;
/**********************************************************************/
...
erts_preload_module(Process *c_p,
         ...
         */
                    uint8_t *code, /* Points to the code to load */
                    size_t size) /* Size of code to load. */
{
...
extern void check_allocated_block(size_t type, void *blk);
#define CHKALLOC() check_allocators()
#define CHKBLK(TYPE,BLK) if ((BLK) != nullptr) check_allocated_block((TYPE),(BLK))
#else
...
erts_prepare_loading(Binary *magic, Process *c_p, Eterm group_leader,
                     Eterm *modp, uint8_t *code, size_t unloaded_size)
{
  ...
  /*
   * Read and validate the export_ table.  (This must be done after
   * loading the code, because it contains labels.)
   */

  CHKBLK(ERTS_ALC_T_CODE, stp->code);
  define_file(stp, "export_ table", EXP_CHUNK);
...
{
  Eterm retval;
  auto stp = ERTS_MAGIC_BIN_DATA<LoaderState *>(magic);

  /*
   * No other process may run since we will update the export_
  ...
  /*
   * Make current code for the module old and insert the new_ code
  ...
  /*
   * Ready for the final touch: fixing the export_ table entries for
...
#endif
#endif
  stp->code = nullptr;    /* Prevent code from being freed. */
  *modp = stp->module;
...
loader_state_dtor(Binary *magic)
{
  ...
  if (stp->export_ != 0) {
    erts_free(ERTS_ALC_T_PREPARED_CODE, (void *) stp->export_);
    stp->export_ = 0;
  }
...
static int
init_iff_file(LoaderState *stp, uint8_t *code, size_t size)
{
  size_t form_id = MakeIffId('F', 'O', 'R', '1');
  size_t id;
  size_t count;
  ...
  if (MakeIffId(code[0], code[1], code[2], code[3]) != form_id) {
    ...
    code = (uint8_t *)stp->bin->orig_bytes;
  ...
  /*
   * Verify that this_ is a BEAM file.
   */
 ...
 */
static int
scan_iff_file(LoaderState *stp, size_t *chunk_types, size_t num_types, size_t num_mandatory)
{
  size_t count;
  size_t id;
  int i;
...
verify_chunks(LoaderState *stp)
{
  ...
  if (stp->chunks[LAMBDA_CHUNK].start != 0) {
    uint8_t *start = stp->chunks[LAMBDA_CHUNK].start;
    size_t left = stp->chunks[LAMBDA_CHUNK].size;
...
load_import_table(LoaderState *stp)
{
  ...
  for (i = 0; i < stp->num_imports; i++) {
    ...
    stp->import[i].bf = nullptr;

    /*
           * If the export_ entry refers to a BIF, get the pointer to
     * the BIF function.
     */
    if ((e = erts_active_export_entry(mod, func, arity)) != nullptr) {
...
read_export_table(LoaderState *stp)
{
  ...
  for (i = 0; i < stp->num_exps; i++) {
    ...
    stp->export_[i].arity = arity;
    GetInt(stp, 4, n);

    if (n >= stp->num_labels) {
      LoadError3(stp, "export_ table entry %d: invalid label %d (highest defined label is %d)", i, n,
                 stp->num_labels);
    }
    ...
    if (value == 0) {
      LoadError2(stp, "export_ table entry %d: label %d not resolved", i, n);
    }

    stp->export_[i].address = address = stp->code + value;
...
read_literal_table(LoaderState *stp)
{
  ...
  uncompressed = (uint8_t *)erts_alloc(ERTS_ALC_T_TMP, uncompressed_sz);
  ...
  for (i = 0; i < stp->num_literals; i++) {
    ...
    hp = stp->literals[i].heap = (Eterm *)erts_alloc(ERTS_ALC_T_PREPARED_CODE,
...
read_code_header(LoaderState *stp)
{
  ...
  if (opcode_max > MAX_GENERIC_OPCODE) {
    LoadError2(stp,
               ...
               "  To fix this_, please recompile this_ module with an "
               ERLANG_OTP_RELEASE " compiler.\n"
               "  (Use of opcode %d; this_ emulator supports "
               "only up to %d.)",
...
load_code(LoaderState *stp)
{
  ...
  int specific;
  size_t last_label = 0;  /* Number of last label. */
  size_t function_number = 0;
  GenOp *last_op = nullptr;
  GenOp **last_op_next = nullptr;
  int arity;
  int retval = 1;
...
get_next_instr:
    ...
    /*
     * Create a new_ generic operation and put it last in the chain.
     */
    if (last_op_next == nullptr) {
      last_op_next = &(stp->genop);

      while (*last_op_next != nullptr) {
        last_op_next = &(*last_op_next)->next;
      }
    }

    NEW_GENOP(stp, last_op);
    last_op->next = nullptr;
    last_op->op = new_op;
...
do_transform:
    ...
    if (stp->genop == nullptr) {
      last_op_next = nullptr;
      goto get_next_instr;
    }
...

    {
      ...
      if (i == num_specific) {
        ...
        if (num_specific == 0 && gen_opc[tmp_op->op].transform == -1) {
          LoadError0(stp, "please re-compile this_ module with an "
                     ERLANG_OTP_RELEASE " compiler ");
        }
        ...
        case genop_unsupported_guard_bif_3: {
          ...
          size_t arity = (size_t) stp->genop->a[2].val;
    ...
    while (*sign) {
      ...
      case 'c':   /* Tagged constant */
        switch (tag) {
        case TAG_i:
          code[ci++] = (BeamInstr) make_small((size_t) tmp_op->a[arg].val);
          break;
...
inline GenOp *gen_get_i__error(GenOp *op, GenOpArg Fail)
{
  op->op = genop_jump_1;
  op->arity = 1;
  op->a[0] = Fail;
                 ...
                 GenOpArg Flags, GenOpArg Dst)
{
  ...
  if (Size.type == TAG_i) {
    if (!safe_mul(Size.val, Unit.val, &bits)) {
      return gen_get_i__error(op, Fail);
...
binary_too_big(LoaderState *stp, GenOpArg Size)
{
  return Size.type == TAG_o ||
         (Size.type == TAG_u && ((Size.val >> (8 * sizeof(size_t) - 3)) != 0));
}
...
/*
 * Test whether this_ label was targeted by a loop_rec/2 instruction.
 */
...
freeze_code(LoaderState *stp)
{
  ...
  literal_end = (size_t *)(code + stp->ci);
  /*
   ...
   */
  {
    ...
    for (i = 0; i < stp->num_literals; i++) {
      ...
      /*
       * Re-link the off_heap list for this_ term onto the
  ...
  if (attr_size) {
    uint8_t *attr = str_table + strtab_size;
  ...
  if (compile_size) {
    uint8_t *compile_info = str_table + strtab_size + attr_size;
    CHKBLK(ERTS_ALC_T_CODE, code);
  ...
  for (i = 0; i < stp->num_labels; i++) {
    size_t this_patch;
    size_t next_patch;
    size_t value = stp->labels[i].value;
...
final_touch(LoaderState *stp)
{
  ...
  for (i = 0; i < stp->num_exps; i++) {
    Export *ep;
    BeamInstr *address = stp->export_[i].address;
    ...
    ep = erts_export_put(stp->module, stp->export_[i].function,
                         stp->export_[i].arity);
...
transform_engine(LoaderState *st)
{
  ...
  size_t *restart;    /* Where to restart if current match fails. */
  ...
  size_t *pc;
  int rval;
  static size_t restart_fail[1] = {TOP_fail};
  ...
  for (;;) {
    ...
    case TOP_next_instr:
      ...
      if (instr == nullptr) {
         ...
         * this_ combination matches or not.
         */
...
#undef RVAL
    ...
    case TOP_is_bif: {
       ...
       * call instruction, this_ will work, but it will be a
      ...
      if (i >= st->num_imports || st->import[i].bf == nullptr) {
        goto restart;
      }
    ...
    case TOP_is_not_bif: {
       ...
       * erlang:apply/2,3 are strange. They exist as (dummy) BIFs
                   * so that they are included in the export_ table before
       ...
       * This is necessary because after setting a trace pattern on
                   * them, you cannot no longer see from the export_ entry that
       * they are special.
       */
      if (i < st->num_imports) {
        if (st->import[i].bf != nullptr ||
    ...
    case TOP_commit:
      ...
      /*
       * The left-hand side of this_ transformation matched.
...
#undef RVAL
      ...
      default:
        new_instr = nullptr; /* Silence compiler warning. */
        ASSERT(0);
      }
...
{
  load_printf(line, stp, "unexpected end of %s when reading %d uint8_t(s)",
...
static int
get_tag_and_value(LoaderState *stp, size_t len_code,
                  unsigned tag, BeamInstr *result)
{
  size_t count;
  ssize_t val;
   ...
   * fitting in an size_t. If it does not fit, we'll indicate overflow
   * by changing the tag to TAG_o.
   */

  if (tag != TAG_i) {
    if (count == sizeof(size_t) + 1) {
      size_t msb;

      /*
       * The encoded value has one more byte than an size_t.
       * It will still fit in an size_t if the most significant
       * byte is 0.
       */
      GetByte(stp, msb);
      GetInt(stp, sizeof(size_t), *result);

      if (msb != 0) {
        /* Overflow: Negative or too big. */
        return TAG_o;
      }
    } else if (count == sizeof(size_t)) {
      /*
  ...
  /*
   * TAG_i: First handle values up to the size of an size_t (i.e. either
   * a small or a bignum).
   */
...
static size_t
new_literal(LoaderState *stp, Eterm **hpp, size_t heap_size)
{
  Literal *lit;
...
erts_module_info_0(Process *p, Eterm module)
{
  ...
  if (erts_get_module(module, erts_active_code_ix()) == nullptr) {
    return THE_NON_VALUE;
  }
                    ...
                    Eterm mod) /* Tagged atom for module. */
{
  ...
  for (i = num_functions - 1; i >= 0 ; i--) {
    ...
    /*
     * If the function name is [], this_ entry is a stub for
...
native_addresses(Process *p, Eterm mod)
{
  ...
  for (i = num_functions - 1; i >= 0 ; i--) {
    ...
    if (func_info[1] != 0) {
      Eterm addr;
      ASSERT(is_atom(name));
      addr = erts_bld_uint(&hp, nullptr, func_info[1]);
      tuple = erts_bld_tuple(&hp, nullptr, 3, name, make_small(arity), addr);
      result = erts_bld_cons(&hp, nullptr, tuple, result);
    }
  }
...
erts_build_mfa_item(FunctionInfo *fi, Eterm *hp, Eterm args, Eterm *mfa_p)
{
  ...
  if (fi->loc != LINE_INVALID_LOCATION) {
    ...
    if (file == 0) {
      ...
      file_term = util::buf_to_intlist(&hp, ".erl", 4, NIL);
      file_term = util::buf_to_intlist(&hp, (char *)ap->name, ap->len, file_term);
    } else {
      Atom *ap = atom_tab(atom_val((fi->fname_ptr)[file - 1]));
      file_term = util::buf_to_intlist(&hp, (char *)ap->name, ap->len, NIL);
    }
...
/*
 * Returns a pointer to {module, function, arity}, or nullptr if not found.
 */
BeamInstr *
...
code_get_chunk_2(BIF_ALIST_2)
{
  ...
  if ((start = erts_get_aligned_binary_bytes(Bin, &temp_alloc)) == nullptr) {
error:
...
code_module_md5_1(BIF_ALIST_1)
{
  ...
  if ((bytes = erts_get_aligned_binary_bytes(Bin, &temp_alloc)) == nullptr) {
    free_loader_state(magic);
    BIF_ERROR(p, BADARG);
  }
...
static BeamInstr *
make_stub(BeamInstr *fp, Eterm mod, Eterm func, size_t arity, size_t native, BeamInstr OpCode)
{
...
stub_copy_info(LoaderState *stp,
               ...
               uint8_t *info, /* Where to store info. */
...
{
  ssize_t decoded_size;
  size_t size = stp->chunks[chunk].size;
...
stub_final_touch(LoaderState *stp, BeamInstr *fp)
{
  ...
  for (i = 0; i < n; i++) {
    if (stp->export_[i].function == function && stp->export_[i].arity == arity) {
...
*/
int
patch(Eterm Addresses, size_t fe)
{
#ifdef HIPE
  Eterm *listp;
  Eterm tuple;
  ...
  while (!is_nil(Addresses)) {
    ...
    hipe_patch_address((size_t *)AddressToPatch, patchtype, fe);
...
patch_funentries(Eterm Patchlist)
{
  ...
  while (!is_nil(Patchlist)) {
    ...
    fe->native_address = (size_t *)native_address;
...
erts_make_stub_module(Process *p, Eterm Mod, Eterm Beam, Eterm Info)
{
  ...
  if ((bytes = erts_get_aligned_binary_bytes(Beam, &temp_alloc)) == nullptr) {
    goto error;
  }
...
#endif
    fp = make_stub(fp, Mod, func, arity, (size_t)native_address, op);
  }
...
static int safe_mul(UWord a, UWord b, UWord *resp)
{
  size_t res = a * b; /* XXX:Pan - used in bit syntax, the multiplication has to be stored in size_t */
  *resp = res;
