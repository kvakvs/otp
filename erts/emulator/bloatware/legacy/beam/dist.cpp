...
/* define this_ to get a lot of debug output */
...
/* Turn this_ on to get printouts of all distribution messages
...
static void bw(uint8_t *buf, ErlDrvSizeT sz)
{
...
static void
dist_msg_dbg(ErtsDistExternal *edep, char *what, uint8_t *buf, int sz)
{
...
Export *dsend2_trap = nullptr;
Export *dsend3_trap = nullptr;
/*Export* dsend_nosuspend_trap = nullptr;*/
Export *dlink_trap = nullptr;
Export *dunlink_trap = nullptr;
Export *dmonitor_node_trap = nullptr;
Export *dgroup_leader_trap = nullptr;
Export *dexit_trap = nullptr;
Export *dmonitor_p_trap = nullptr;
...
get_suspended_on_de(DistEntry *dep, uint32_t unset_qflgs)
{
  ...
  if (dep->qflgs & ERTS_DE_QFLG_EXIT) {
    ...
    dep->suspended = nullptr;
    erts_proclist_fetch(&suspended, nullptr);
    return suspended;
  }
}
...
int is_node_name_atom(Eterm a)
{
  ...
  i = atom_val(a);
  ASSERT((i > 0) && (i < atom_table_size()) && (atom_tab(i) != nullptr));
...
static void doit_monitor_net_exits(ErtsMonitor *mon, void *vnecp)
{
  ...
  if (mon->type == MON_ORIGIN) {
    ...
    /* ASSERT(rmon != nullptr); nope, can happen during process exit */
    if (rmon != nullptr) {
    ...
    /* ASSERT(rmon != nullptr); can happen during process exit */
    if (rmon != nullptr) {
...
static void doit_link_net_exits_sub(ErtsLink *sublnk, void *vlnecp)
{
  ...
  if (is_internal_pid(lnk->pid)) {
    ...
    xres = erts_send_exit_signal(nullptr,
                                 sublnk->pid,
                                 rp,
                                 &rp_locks,
                                 am_noconnection,
    ...
    if (rlnk) {
      ...
      if (xres >= 0 && IS_TRACED_FL(rp, F_TRACE_PROCS)) {
        ...
        trace_proc(nullptr, rp, am_getting_unlinked, sublnk->pid);
      }
    }
...
set_node_not_alive(void *unused)
{
  ...
  erts_is_alive = 0;
  send_nodes_mon_msgs(nullptr, am_nodedown, nodename, am_visible, nodedown.reason);
  nodedown.reason = NIL;
  bp = nodedown.bp;
  nodedown.bp = nullptr;
  ...
  if (dep == erts_this_dist_entry) {  /* Net kernel has died (clean up!!) */
    ...
    if (no_dist_port == 0) {
      ...
      for (i = 0; i < no_dist_port; i++) {
        ...
        erts_port_exit(nullptr, ERTS_PORT_SIG_FLG_FORCE_SCHED,
                       prt, dist_port[i], nd_reason, nullptr);
      }
    ...
    dep->monitors = nullptr;
    dep->nlinks = nullptr;
    dep->node_links = nullptr;
...
static void clear_dist_entry(DistEntry *dep)
{
  ...
  dep->out_queue.first = nullptr;
  dep->out_queue.last = nullptr;
  dep->finalized_out_queue.first = nullptr;
  dep->finalized_out_queue.last = nullptr;
  dep->status = 0;
...
#define DIST_CTL_DEFAULT_SIZE 64
  ...
  case DOP_LINK:
    ...
    if (IS_TRACED_FL(rp, F_TRACE_PROCS)) {
      trace_proc(nullptr, rp, am_getting_linked, from);
    }
  ...
  case DOP_UNLINK: {
    ...
    if (IS_TRACED_FL(rp, F_TRACE_PROCS) && lnk != nullptr) {
      trace_proc(nullptr, rp, am_getting_unlinked, from);
    }
  ...
  case DOP_MONITOR_P: {
    ...
    if (is_atom(watched)) {
      name = watched;
      rp = erts_whereis_process(nullptr, 0,
  ...
  case DOP_DEMONITOR_P:
    ...
    /* ASSERT(mon != nullptr); can happen in case of broken dist message */
    if (mon == nullptr) {
      break;
    }
  ...
  case DOP_MONITOR_P_EXIT: {
     ...
     * removed info about monitor. In this_ case, do nothing
  ...
  case DOP_EXIT: {
    ...
    rp = erts_pid2proc(nullptr, 0, to, rp_locks);
    ...
    if (!rp) {
      ...
      /* If lnk == nullptr, we have unlinked on this_ side, i.e.
      * ignore exit.
      */
      if (lnk) {
        int xres;
  ...
  case DOP_GROUP_LEADER:
    ...
    to = tuple[3];     /* new_ member */

    if (is_not_pid(from) || is_not_pid(to)) {
      goto invalid_message;
    }

    rp = erts_pid2proc(nullptr, 0, to, ERTS_PROC_LOCK_MAIN);
...
dsig_send(ErtsDSigData *dsdp, Eterm ctl, Eterm msg, int force_busy)
{
  ...
  if (cid != dsdp->cid
    ...
    erts_smp_mtx_unlock(&dep->qlock);
    erts_schedule_dist_command(nullptr, dep);
    erts_smp_de_runlock(dep);
...
preempted:
  ...
  if (sched_flags & ERTS_PTS_FLG_EXIT) {
    ...
    erts_schedule_dist_command(prt, nullptr);
  }
...
static void doit_print_monitor_info(ErtsMonitor *mon, void *vptdp)
{
  ...
  if (!rp || (rmon = erts_lookup_monitor(ERTS_P_MONITORS(rp), mon->ref)) == nullptr) {
    ...
    if (is_not_atom(rmon->pid)) {
      ...
      erts_print(to, arg, "{%T, %T}\n",
                 rmon->name,
                 rmon->pid); /* which in this_ case is the
...
{
  auto pcontext = (PrintNodeLinkContext *)vpcontext;
...
info_dist_entry(int to, void *arg, DistEntry *dep, int visible, int connected)
{
  ...
  erts_print_node_info(to, arg, dep->sysname, nullptr, nullptr);
...
BIF_RETTYPE setnode_2(BIF_ALIST_2)
{
  ...
  if (BIF_ARG_1 == am_Noname) { /* cant use this_ name !! */
    goto error;
  }
  ...
  if (dsend2_trap->addressv[0] == nullptr ||
      dsend3_trap->addressv[0] == nullptr ||
      /*  dsend_nosuspend_trap->address == nullptr ||*/
      dlink_trap->addressv[0] == nullptr ||
      dunlink_trap->addressv[0] == nullptr ||
      dmonitor_node_trap->addressv[0] == nullptr ||
      dgroup_leader_trap->addressv[0] == nullptr ||
      dmonitor_p_trap->addressv[0] == nullptr ||
      dexit_trap->addressv[0] == nullptr) {
    goto error;
  }
...
BIF_RETTYPE setnode_3(BIF_ALIST_3)
{
  ...
  if (dep->status & ERTS_DE_SFLG_EXITING) {
    ...
    plp->next = nullptr;
    erts_suspend(BIF_P, ERTS_PROC_LOCK_MAIN, nullptr);
    erts_smp_mtx_lock(&dep->qlock);
    erts_proclist_store_last(&dep->suspended, plp);
    erts_smp_mtx_unlock(&dep->qlock);
    goto yield;
   ...
   */
  {
    ...
    erl_drv_busy_msgq_limits(ERTS_Port2ErlDrvPort(pp), &disable, nullptr);
  }
  ...
  erts_smp_de_rwunlock(dep);
  dep = nullptr; /* inc of refc transferred to port (dist_entry field) */
...
BIF_RETTYPE nodes_1(BIF_ALIST_1)
{
  ...
  while (is_list(arg_list)) {
    ...
    case am_known:
      visible = hidden = not_connected = this_ = 1;
      break;
...
insert_nodes_monitor(Process *c_p, uint32_t opts)
{
  ...
  if (xnmp) {
    ...
    if (xnmp->opts != opts) {
      ...
      if (!xnmp->no) {
        /*
         * 'no' wrapped; transfer all prevous monitors to new_
         * element (which will be the next element in the list)
         * and set this_ to one...
         */
        xnmp->no = 1;
        goto alloc_new;
      }
  ...
  while (nmp && nmp->proc == c_p) {
    if (!all && nmp->opts != opts) {
      ...
      if (c_p->nodes_monitors == free_nmp) {
        c_p->nodes_monitors = nmp && nmp->proc == c_p ? nmp : nullptr;
      }
...
/*
 * Note, this_ function is only used for debuging.
 */
  ...
  for (nmp = nodes_monitors_end; nmp; nmp = nmp->prev) {
    ...
    for (i = 0; i < nmp->no; i++) {
      ...
      if (nmp->opts & ERTS_NODES_MON_OPT_TYPES) {
        ...
        case ERTS_NODES_MON_OPT_TYPES:
          type = am_all;
          break;

        case ERTS_NODES_MON_OPT_TYPE_VISIBLE:
          type = am_visible;
          break;

        case ERTS_NODES_MON_OPT_TYPE_HIDDEN:
          type = am_hidden;
          break;
