                     ...
                     int legacy)
{
  ...
  uint32_t bits, cl, cli, ix, ix_per_cache_line, tab_cache_lines;
  char *tab_end;
  ...
  if (size > ERTS_PTAB_MAX_SIZE) {
    ...
    bits = erts_fit_in_bits_int32((int32_t) size - 1);
  }
  ...
  ptab->r.o.tab = (ethr_atomic_t *)erts_alloc_permanent_cache_aligned(atype, alloc_sz);
  ...
  if (size == ERTS_PTAB_MAX_SIZE) {
     ...
     *
     * In order to fix this_, we insert a pointer from the table
                      ...
                      void (*init_ptab_el)(void *, Eterm))
{
  ...
  if (ptab->r.o.free_id_data) {
    do {
      ix = (uint32_t) erts_smp_atomic32_inc_read_acqb(&ptab->vola.tile.aid_ix);
...
{
  auto ptdep = (ErtsPTabDeletedElement *)erts_alloc(ERTS_ALC_T_PTAB_LIST_DEL,
                         ...
                         ErtsPTabElementCommon *ptab_el)
{
  ...
  if (ptab->r.o.free_id_data) {
    uint32_t prev_data;
    /* Next data for this_ slot... */
    data = (uint32_t) erts_ptab_id2data(ptab, ptab_el->id);
    data += ptab->r.o.max;
    data &= ~(~((uint32_t) 0) << ERTS_PTAB_ID_DATA_SIZE);
    ...
    do {
      ix = (uint32_t) erts_smp_atomic32_inc_read_relb(&ptab->vola.tile.fid_ix);
      ix = ix_to_free_id_data_ix(ptab, ix);
...
erts_ptab_list(Process *c_p, ErtsPTab *ptab)
{
   ...
   *                execution of the BIF calling this_ function.
  ...
  auto ptlbdp = ERTS_MAGIC_BIN_DATA<ErtsPTabListBifData *>(mbp);
...
{
  auto ptlbdp = ERTS_MAGIC_BIN_DATA<ErtsPTabListBifData *>(bp);
  ...
  if (ptlbdp->state != INITIALIZING) {
    ...
    if (ptlbdp->bif_invocation) {
      ...
      if (ptdep->prev) {
        /*
         * Only remove this_ bif invokation when we
        ...
        if (ptdep->next) {
          ...
          /*
           * At the time of writing this_ branch cannot be
           * reached. I don't want to remove this_ code though
           * since it may be possible to reach this_ line
...
ptab_list_bif_engine(Process *c_p, Eterm *res_accp, Binary *mbp)
{
  ...
  do {
    switch (ptlbdp->state) {
    case INITIALIZING:
      ptlbdp->chunk = (ErtsPTabListBifChunkInfo *)erts_alloc(ERTS_ALC_T_PTAB_LIST_CNKI,
      ...
      ptlbdp->pid = (Eterm *)erts_alloc(ERTS_ALC_T_PTAB_LIST_PIDS,
      ...
      if (ptab->list.data.chunks == 1) {
        ...
        ptlbdp->bif_invocation
          = (ErtsPTabDeletedElement *)erts_alloc(ERTS_ALC_T_PTAB_LIST_DEL,
              sizeof(ErtsPTabDeletedElement));
    ...
    case INSPECTING_TABLE: {
      ...
      if (end_ix >= ptab->r.o.max) {
        ...
        end_ix = ptab->r.o.max;
        indices = end_ix - ix;
        /* What to do when done with this_ chunk */
    ...
    case BUILDING_RESULT: {
      ...
      if (min_ix == 0) {
        ...
        ptlbdp->pid = (Eterm *)erts_realloc(ERTS_ALC_T_PTAB_LIST_PIDS,
                                            ptlbdp->pid,
                                            sizeof(Eterm) * ptlbdp->pid_sz);
#if ERTS_PTAB_LIST_BIF_DEBUGLEVEL >= ERTS_PTAB_LIST_DBGLVL_CHK_FOUND_PIDS
        ptlbdp->debug.pid_started
...
erts_ptab_test_next_id(ErtsPTab *ptab, int set, size_t next)
{
  ...
  if (ptab->r.o.free_id_data) {
    ...
    if (set) {
      uint32_t i, max_ix, num, stop_id_ix;
      max_ix = ptab->r.o.max - 1;
      num = next;
      id_ix = (uint32_t) erts_smp_atomic32_read_nob(&ptab->vola.tile.aid_ix);

      for (i = 0; i <= max_ix; ++i) {
        uint32_t pix;
        ++num;
        num &= ~(~((uint32_t) 0) << ERTS_PTAB_ID_DATA_SIZE);
