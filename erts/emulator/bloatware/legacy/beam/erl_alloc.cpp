...
#ifdef DEBUG
static size_t install_debug_functions(void);
#if 0
#define HARD_DEBUG
#ifdef __GNUC__
#warning "* * * * * * * * * * * * * *"
...
#define DEFAULT_ALLCTR_INIT {   \
    /*allctrinit_t util*/ ERTS_DEFAULT_ALLCTR_INIT, \
    /*gfallctrinit_t gf*/ ERTS_DEFAULT_GF_ALLCTR_INIT,  \
    /*bfallctrinit_t bf*/ ERTS_DEFAULT_BF_ALLCTR_INIT,  \
    /*afallctrinit_t af*/ ERTS_DEFAULT_AF_ALLCTR_INIT,  \
    /*aoffallctrinit_t aoff*/ ERTS_DEFAULT_AOFF_ALLCTR_INIT }
...
#define ERTS_AU_INIT__ { /*int enable*/ 0, \
  /*int thr_spec*/ 0, \
  /*int carrier_mig_allow*/ 1, \
  /*enum allctr_type atype*/ GOODFIT, \
  /*struct init*/ DEFAULT_ALLCTR_INIT, \
  /*struct default_*/ {1, 1, 1, 1}}
...
#endif
    {
      auto allctr = (Allctr_t *)erts_allctrs_info[ERTS_ALC_A_FIXED_SIZE].extra;
...
erts_alloc_init(int *argc, char **argv, ErtsAllocInitOpts *eaiop)
{
  ...
  fix_type_sizes[ERTS_ALC_FIX_TYPE_IX(ERTS_ALC_T_MONITOR_SH)]
    = ERTS_MONITOR_SH_SIZE * sizeof(size_t);
  fix_type_sizes[ERTS_ALC_FIX_TYPE_IX(ERTS_ALC_T_NLINK_SH)]
    = ERTS_LINK_SH_SIZE * sizeof(size_t);
#endif
...
static void *
erts_realloc_fixed_size(ErtsAlcType_t type, void *extra, void *p, size_t size)
{
  erl::exit(erts::ABORT_EXIT,
...
get_kb_value(char *param_end, char **argv, int *ip)
{
  ...
  if (errno != 0 || rest == value || tmp < 0 || max < ((size_t) tmp)) {
    bad_value(param, param_end, value);
  }
...
erts_alloc_fix_alloc_shrink(int ix, erts_aint32_t flgs)
{
  ...
  if (ix == 0 && erts_allctrs_info[ERTS_ALC_A_FIXED_SIZE].extra)
    return erts_alcu_fix_alloc_shrink(
             (Allctr_t_ *)erts_allctrs_info[ERTS_ALC_A_FIXED_SIZE].extra, flgs);
...
__decl_noreturn void
erts_alloc_enomem(ErtsAlcType_t type, size_t size)
{
  erts_alloc_n_enomem(ERTS_ALC_T2N(type), size);
}

__decl_noreturn void
erts_alloc_n_enomem(ErtsAlcType_t n, size_t size)
{
  erts_alc_fatal_error(ERTS_ALC_E_NOMEM, ERTS_ALC_O_ALLOC, n, size);
}

__decl_noreturn void
erts_realloc_enomem(ErtsAlcType_t type, void *ptr, size_t size)
{
  erts_realloc_n_enomem(ERTS_ALC_T2N(type), ptr, size);
}

__decl_noreturn void
erts_realloc_n_enomem(ErtsAlcType_t n, void *ptr, size_t size)
{
...
# ifdef DEBUG
        ...
        if (!found_one) {
          ...
          ASSERT(is_low == allctr->mseg_opt.low_mem);
        }
...
{
  /*
   * NOTE! When updating this_ function, make sure to also update
...
#define ERTS_MEM_NEED_ALL_ALCU (!erts_instr_stat && want_tot_or_sys)
  ...
  if (want.atom || want.atom_used) {
    size_t reserved_atom_space, atom_space;
        ...
        case ERTS_ALC_A_SYSTEM:
          ...
          if (sas.trim_threshold >= 0) {
            ...
            ts[l++] = erts_bld_uint(hpp, szp,
                                    (size_t) sas.trim_threshold);
          }

          if (sas.top_pad >= 0) {
            as[l] = am_atom_put("tp", 2);
            ts[l++] = erts_bld_uint(hpp, szp, (size_t) sas.top_pad);
          }
...
reply_alloc_info(void *vair)
{
  ...
  size_t sched_id = erts_get_scheduler_id();
...
void *sys_realloc(void *ptr, size_t sz)
{
...
UWord erts_alc_test(UWord op, UWord a1, UWord a2, UWord a3)
{
  switch (op >> 8) {
  case 0x0:
    return erts_alcu_test(op,  a1, a2);

  case 0x1:
    return erts_gfalc_test(op, a1, a2);

  case 0x2:
    return erts_bfalc_test(op, a1, a2);

  case 0x3:
    return erts_afalc_test(op, a1, a2);

  case 0x4:
    return erts_mseg_test(op,  a1, a2, a3);

  case 0x5:
    return erts_aoffalc_test(op, a1, a2);
  ...
  case 0xf:
    ...
    case 0xf06:
      return (UWord)((Allctr_t *) a1)->thread_safe;
    ...
    case 0xf08: {
      auto mtx = (ethr_mutex *)erts_alloc(ERTS_ALC_T_UNDEF, sizeof(ethr_mutex));
...
static hdbg_mblk *
hdbg_alloc(void *p, size_t s, ErtsAlcType_t n)
{
  hdbg_mblk *mblk;
...
static void *check_memory_fence(void *ptr, size_t *size, ErtsAlcType_t n, int func);

void check_allocated_block(size_t type, void *blk)
{
  size_t dummy;
...
static void *
set_memory_fence(void *ptr, size_t sz, ErtsAlcType_t n)
{
  UWord *ui_ptr;
  UWord pattern;
#ifdef HARD_DEBUG
...
static void *
check_memory_fence(void *ptr, size_t *size, ErtsAlcType_t n, int func)
{
  size_t sz;
  size_t found_type;
  UWord pre_pattern;
...
static void *
debug_alloc(ErtsAlcType_t n, void *extra, size_t size)
{
  ...
  fprintf(stderr, "0x%lx = alloc(%s, %lu)\r\n",
          (size_t) res, ERTS_ALC_N2TD(n), size);
#endif
...
static void *
debug_realloc(ErtsAlcType_t n, void *extra, void *ptr, size_t size)
{
  ErtsAllocatorFunctions_t *real_af = (ErtsAllocatorFunctions_t *) extra;
  size_t dsize;
  size_t old_size;
  ...
  fprintf(stderr, "0x%lx = realloc(%s, 0x%lx, %lu)\r\n",
          (size_t) res, ERTS_ALC_N2TD(n), (size_t) ptr, size);
#endif
