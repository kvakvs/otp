...
L_copy_boxed:
      ...
      case ARITYVAL_SUBTAG: {
        ...
        *argp = make_tuple_rel(htop, dst_base);
        tp = htop;  /* tp is pointer to new_ arity value */
        *htop++ = *objp++; /* copy arity value */
        ...
        if (const_flag) {
          const_tuple = tp; /* this_ is the latest const_tuple */
        }
      }
      break;
      ...
      case SUB_BINARY_SUBTAG: {
        ...
        if (thing_subtag(*objp) == HEAP_BINARY_SUBTAG) {
          ...
          to->size = real_size;
          sys_memcpy(to->data, ((uint8_t *)from->data) + offset, real_size);
        } else {
 ...
 * pointers are offsetted to point correctly in the new_ location.
 *
