 ...
 * not by an initializer. Otherwise a new_ instance of the emulator will
...
erl_first_process_otp(char *modname, void *code, unsigned size, int argc, char **argv)
{
  ...
  start_mod = erts_atom_put((uint8_t *) modname, sys_strlen(modname), ERTS_ATOM_ENC_LATIN1, 1);
  ...
  for (i = argc - 1; i >= 0; i--) {
    ...
    args = CONS(hp, new_binary(&parent, (uint8_t *)argv[i], len), args);
    hp += 2;
  }

  env = new_binary(&parent, (uint8_t *)code, size);
...
void erts_usage(void)
{
  ...
  erts_fprintf(stderr, "               Note that this_ flag is deprecated!\n");
  erts_fprintf(stderr, "-M<X> <Y>      memory allocator switches,\n");
  erts_fprintf(stderr, "               see the erts_alloc(3) documentation for more info.\n");
  erts_fprintf(stderr,
               "-pc <set>      Control what characters are considered printable (default latin1)\n");
  erts_fprintf(stderr, "-P number      set maximum number of processes on this_ node,\n");
  erts_fprintf(stderr, "               valid range is [%d-%d]\n",
               ERTS_MIN_PROCESSES, ERTS_MAX_PROCESSES);
  erts_fprintf(stderr, "-Q number      set maximum number of ports on this_ node,\n");
  erts_fprintf(stderr, "               valid range is [%d-%d]\n",
  ...
  erts_fprintf(stderr, "-scl bool_      enable/disable compaction of scheduler load,\n");
  erts_fprintf(stderr, "               see the erl(1) documentation for more info.\n");
  erts_fprintf(stderr, "-sct cput      set cpu topology,\n");
  erts_fprintf(stderr, "               see the erl(1) documentation for more info.\n");
  erts_fprintf(stderr, "-secio bool_    enable/disable eager check I/O scheduling,\n");
  erts_fprintf(stderr, "               see the erl(1) documentation for more info.\n");
#if ERTS_HAVE_SCHED_UTIL_BALANCING_SUPPORT_OPT
  erts_fprintf(stderr, "-sub bool_      enable/disable scheduler utilization balancing,\n");
#else
    ...
    case 'v':
      ...
      if (argv[i][2] == '\0') {
        ...
        for (ch = argv[i] + 2; *ch != '\0'; ch++) {
          ...
          case 'M':
            verbose |= DEBUG_MEMORY;
            break;

          case 'a':
            verbose |= DEBUG_ALLOCATION;
            break;

          case 't':
            verbose |= DEBUG_THREADS;
            break;

          case 'p':
            verbose |= DEBUG_PROCESSES;
            break;
          ...
          default :
            erts_fprintf(stderr, "Unknown verbose option: %c\n", *ch);
          }
        }
      }
      ...
      if (verbose & DEBUG_SYSTEM) {
        erts_printf("SYSTEM ");
      }

      if (verbose & DEBUG_PRIVATE_GC) {
        erts_printf("PRIVATE_GC ");
      }

      if (verbose & DEBUG_MEMORY) {
        erts_printf("PARANOID_MEMORY ");
      }

      if (verbose & DEBUG_ALLOCATION) {
        erts_printf("ALLOCATION ");
      }

      if (verbose & DEBUG_THREADS) {
        erts_printf("THREADS ");
      }

      if (verbose & DEBUG_PROCESSES) {
        erts_printf("PROCESSES ");
      }

      if (verbose & DEBUG_MESSAGES) {
        erts_printf("MESSAGES ");
      }
