...
static Export *flush_monitor_message_trap = nullptr;
static Export *set_cpu_topology_trap = nullptr;
static Export *await_proc_exit_trap = nullptr;
static Export *await_port_send_result_trap = nullptr;
Export *erts_format_cpu_topology_trap = nullptr;
...
/* Utility to add a new_ link between processes p and another internal
...
BIF_RETTYPE link_1(BIF_ALIST_1)
{
  ...
  if (is_internal_port(BIF_ARG_1)) {
    ...
    if (send_link_signal) {
      Eterm ref;
      Eterm *refp = erts_port_synchronous_ops ? &ref : nullptr;
  ...
  if (is_external_pid(BIF_ARG_1)) {
    ...
    if (erts_lookup_link(ERTS_P_LINKS(BIF_P), BIF_ARG_1) != nullptr) {
      erts_smp_proc_unlock(BIF_P, ERTS_PROC_LOCK_LINK);
      BIF_RET(am_true);
    } else {
      ErtsLink *lnk;
...
remote_demonitor(Process *c_p, DistEntry *dep, Eterm ref, Eterm to)
{
  ...
  case ERTS_DSIG_PREP_NOT_CONNECTED:
#ifndef ERTS_SMP
    /* XXX Is this_ possible? Shouldn't this_ link
    ...
    /*
     * In the smp case this_ is possible if the node goes
  ...
  case ERTS_DSIG_PREP_CONNECTED:
    ...
    if (!dmon) {
#ifndef ERTS_SMP
      /* XXX How is this_ possible? Shouldn't this_ link
...
{
  ErtsMonitor *mon = nullptr;  /* The monitor entry to delete */
  Process  *rp;    /* Local target process */
  Eterm     to = NIL;    /* Monitor link traget */
  DistEntry *dep = nullptr;  /* Target's distribution entry */
  int deref_de = 0;
  int res;
  int unlock_link = 1;
  ...
  if (is_not_internal_ref(ref)) {
    ...
    goto done; /* Cannot be this_ monitor's ref */
  }
...
local_pid:
    ...
    if (!erts_is_alive && remote_node != am_Noname) {
      goto error; /* Remote monitor from (this_) undistributed node */
    }
...
/**********************************************************************/
/* this_ is a combination of the spawn and link BIFs */
...
BIF_RETTYPE unlink_1(BIF_ALIST_1)
{
  ...
  if (is_external_pid(BIF_ARG_1)) {
    ...
    int code;
    ErtsDSigData dsd;
    /* Blind removal, we might have trapped or anything, this_ leaves
  ...
  if (!rp) {
    ...
    if (IS_TRACED_FL(rp, F_TRACE_PROCS) && rl != nullptr) {
...
BIF_RETTYPE hibernate_3(BIF_ALIST_3)
{
   ...
   * this_ function is only called from HiPE or when the call could not
   * be translated.
   */
  Eterm reg[3];
...
/**********************************************************************/
/* this_ is like throw/1 except that we set freason to EXC_EXIT */
...
/**********************************************************************/
/* raise an exception of given class_, value and stacktrace.
 *
...
/**********************************************************************/
/* this_ sets some process info- trapping exits or the error handler */
...
BIF_RETTYPE process_flag_2(BIF_ALIST_2)
{
  ...
  if (BIF_ARG_1 == am_error_handler) {
    ...
    */
    ...
    old = (ERTS_PSFLG_BOUND & state) ? curr : nullptr;
...
BIF_RETTYPE process_flag_3(BIF_ALIST_3)
{
  ...
  if ((rp = erts_pid2proc(BIF_P, ERTS_PROC_LOCK_MAIN,
                          BIF_ARG_1, ERTS_PROC_LOCK_MAIN)) == nullptr) {
    BIF_ERROR(BIF_P, BADARG);
  }
...
BIF_RETTYPE unregister_1(BIF_ALIST_1)
{
  ...
  res = erts_unregister_name(BIF_P, ERTS_PROC_LOCK_MAIN, nullptr, BIF_ARG_1);
...
/* this_ is a rather unsafe BIF as it allows users to do nasty things. */
...
port_common:
    ...
    /* sysname_to_connected_dist_entry will return nullptr if there
    ...
    if (IS_TRACED(p)) { /* XXX Is this_ really neccessary ??? */
      trace_send(p, to, msg);
    }
 ...
 * this_ function to the error handling code if the apply instruction
 ...
 * like this_:  apply(erlang, apply, [M, F, A]). Not recommended.)
 */
...
/* returns the head of a list - this_ function is unecessary
...
BIF_RETTYPE atom_to_list_1(BIF_ALIST_1)
{
  ...
  ares =
#endif
    erts_analyze_utf8(ap->name, ap->len, &err_pos, &num_chars, nullptr);
...
BIF_RETTYPE list_to_atom_1(BIF_ALIST_1)
{
  ...
  res = erts_atom_put((uint8_t *) buf, i, ERTS_ATOM_ENC_LATIN1, 1);
  ASSERT(is_atom(res));
  erts_free(ERTS_ALC_T_TMP, (void *) buf);
  BIF_RET(res);
}
...
error:
  ...
  buf = (uint8_t *) erts_alloc(ERTS_ALC_T_TMP, bufsz);
  ...
  /*
     ...
     this_ we will look for a possible EXP_SIGN (+ | -) followed by the
  ...
  */
  while (1) {
    ...
    if (CAR(list_val(list)) == make_small('-')) {
      ...
      case INT:   /* would like this_ to be ok, example "2e2",
    ...
    if (i == bufsz - 1)
      buf = (uint8_t *) erts_realloc(ERTS_ALC_T_TMP,
                                     (void *) buf,
...
BIF_RETTYPE binary_to_float_1(BIF_ALIST_1)
{
  ...
  buf = (uint8_t *)erts_alloc(ERTS_ALC_T_TMP, size + 1);
  ...
  if (*real_bin == HEADER_PROC_BIN) {
    ...
    bytes = (uint8_t *)(&(((ErlHeapBin *) real_bin)->data)) + offs;
  }
...
/* return the pid of our own process, in most cases this_ has been replaced by
...
localtime_to_universaltime_2(BIF_ALIST_2)
{
  ...
  if (dst == am_true) {
    isdst = 1;
  } else if (dst == am_false) {
    isdst = 0;
  } else if (dst == am_undefined) {
    isdst = -1;
  } else {
    goto error;
  }
...
BIF_RETTYPE garbage_collect_0(BIF_ALIST_0)
{
  ...
  reds = erts_garbage_collect(BIF_P, 0, nullptr, 0);
...
BIF_RETTYPE phash_2(BIF_ALIST_2)
{
  ...
  if (range) {
    ...
    /*
     * XXX In this_ case, there will still be a ArithAlloc() in erts_mixed_plus().
     */
 ...
 * receive is done before this_ receive. This optimization removes an
...
void erts_init_bif(void)
{
  ...
  erts_smp_atomic_init_nob(&erts_dead_ports_ptr, (erts_aint_t) nullptr);
...
#ifdef HARDDEBUG
/*
You'll need this_ line in bif.tab to be able to use this_ debug bif
...
BIF_RETTYPE send_to_logger_2(BIF_ALIST_2)
{
  ...
  if (erts_iolist_size(BIF_ARG_2, &len) != 0) {
    ...
    ErlDrvSizeT len2;
#endif
    buf = (uint8_t *) erts_alloc(ERTS_ALC_T_TMP, len + 1);
#ifdef DEBUG
    len2 =
#else
    (void)
...
{
#ifdef USE_VM_PROBES
  BIF_RET((DT_UTAG(BIF_P) == NIL
           || !(DT_UTAG_FLAGS(BIF_P) & DT_UTAG_PERMANENT)) ? am_undefined : DT_UTAG(BIF_P));
#else
  BIF_RET(am_undefined);
#endif
}
