...
static void dump_attributes(int to, void *to_arg, uint8_t *ptr, int size);
    ...
    if (!ERTS_IS_CRASH_DUMPING) {
      ...
      if (modp != nullptr &&
        ...
        if (code != nullptr && code[MI_ATTR_PTR]) {
          ...
          dump_attributes(to, to_arg, (uint8_t *) code[MI_ATTR_PTR],
                          code[MI_ATTR_SIZE]);
        }

        if (code != nullptr && code[MI_COMPILE_PTR]) {
          erts_print(to, to_arg, "Current compilation info: ");
          dump_attributes(to, to_arg, (uint8_t *) code[MI_COMPILE_PTR],
...
static void
dump_attributes(int to, void *to_arg, uint8_t *ptr, int size)
{
  while (size-- > 0) {
