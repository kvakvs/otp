...
erts_mk_magic_binary_term(Eterm **hpp, ErlOffHeap *ohp, Binary *mbp)
{
  ...
  *hpp += PROC_BIN_SIZE;
  ...
  pb->val = mbp;
  pb->bytes = (uint8_t *) mbp->orig_bytes;
  pb->flags = 0;
...
erts_proc_store_ref(Process *c_p, uint32_t ref[ERTS_MAX_REF_NUMBERS])
{
  Eterm *hp = (Eterm *)vm::heap_alloc(c_p, REF_THING_SIZE);
...
extern int erts_net_message(Port *, DistEntry *,
                            uint8_t *, ErlDrvSizeT, uint8_t *, ErlDrvSizeT);
...
#ifdef MESS_DEBUG
void print_pass_through(int, uint8_t *, int);
#endif
...
void bin_write(int, void *, uint8_t *, size_t);
