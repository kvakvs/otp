...
static Export *c_to_b_int_trap_exportp = nullptr;
static Export *c_to_l_int_trap_exportp = nullptr;
...
{
  auto rc = ERTS_MAGIC_BIN_DATA<RestartContext *>(bp);
...
static RestartContext *get_rc_from_bin(Eterm bin)
{
  ...
  return ERTS_MAGIC_BIN_DATA<RestartContext *>(mbp);
}
...
static ssize_t latin1_binary_need(Eterm binary)
{
  ...
  if (bitoffs != 0) {
    ...
    /* The call to erts_get_aligned_binary_bytes cannot fail as
       we'we already checked bitsize and that this_ is a binary */
  }

  size = binary_size(binary);

  for (i = 0; i < size; ++i) {
    if (bytes[i] & ((uint8_t) 0x80)) {
      need += 2;
    } else {
      need += 1;
    }
...
static int utf8_len(uint8_t first)
{
  if ((first & ((uint8_t) 0x80)) == 0) {
    return 1;
  } else if ((first & ((uint8_t) 0xE0)) == 0xC0) {
    return 2;
  } else if ((first & ((uint8_t) 0xF0)) == 0xE0) {
    return 3;
  } else if ((first & ((uint8_t) 0xF8)) == 0xF0) {
    return 4;
  }
                         ...
                         uint8_t **err_pos, size_t *characters)
{
  ...
  if (leftover != nullptr && *num_leftovers) {
    ...
    c = copy_utf8_bin(target, leftover, need, nullptr, nullptr, &tmp_err_pos, characters);
  ...
  while (size) {
    if (((*source) & ((uint8_t) 0x80)) == 0) {
      *(target++) = *(source++);
      ...
      if (size < 2 || ((source[1] & ((uint8_t) 0xC0)) != 0x80) ||
      ...
      *(target++) = *(source++);
      ...
      if (size < 3 || ((source[1] & ((uint8_t) 0xC0)) != 0x80) ||
      ...
      *(target++) = *(source++);
      ...
      if (size < 4 || ((source[1] & ((uint8_t) 0xC0)) != 0x80) ||
          ((source[2] & ((uint8_t) 0xC0)) != 0x80) ||
...
L_Again:   /* Restart with sublist, old listend was pushed on stack */
      ...
      for (;;) {
        /* loop over one flat list of bytes and binaries
  ...
  *err = 0;
  ...
  if (is_binary(ioterm)) {
    ...
    /* This is done to avoid splitting binaries in two
       ...
       For cases where errors are not returned this_ is unnecessary */
    if (!latin1) {
      /* Find a valid character boundary */
      while (size > (*left) &&
             (((uint8_t) bytes[(*left)]) & ((uint8_t) 0xC0)) == ((uint8_t) 0x80)) {
        ++(*left);
      }
    }
      ...
      *pos += num;
      ...
      while (i < size) {
        if (bytes[i] < 0x80) {
          ...
          target[(*pos)++] = ((bytes[i] >> 6) | ((uint8_t) 0xC0));
          target[(*pos)++] = ((bytes[i] & 0x3F) | ((uint8_t) 0x80));
          ++i;
        }
...
L_Again:   /* Restart with sublist, old listend was pushed on stack */
      ...
      for (;;) {
        ...
        if (is_small(obj)) { /* Always small in unicode*/
          if (*num_leftovers) {
            /* Have rest from previous bin and this_ is an integer, not allowed */
            *err = 1;
            goto done;
          }
    ...
    if ((*left) && !is_list(ioterm) && !is_nil(ioterm)) {
      /* inproper list end */
      if (is_binary(ioterm)) {
        ioterm = do_build_utf8(p, ioterm, left, latin1, target, pos, characters, err, leftover,
                               num_leftovers);
...
static int check_leftovers(uint8_t *source, int size)
{
  if (((*source) & ((uint8_t) 0xE0)) == 0xC0) {
    return 0;
  } else if (((*source) & ((uint8_t) 0xF0)) == 0xE0) {
    if (size < 2 ||
        (size < 3 && ((source[1] & ((uint8_t) 0xC0)) == 0x80))) {
      return 0;
    }
  } else if (((*source) & ((uint8_t) 0xF8)) == 0xF0) {
    if (size < 2 ||
...
static BIF_RETTYPE build_utf8_return(Process *p, Eterm bin, int pos,
                                     ...
                                     uint8_t *leftover, int num_leftovers, Eterm latin1)
{
  Eterm *hp;
  Eterm ret;
...
static BIF_RETTYPE characters_to_utf8_trap(BIF_ALIST_3)
{
  ...
  int pos;
  int err;
  uint8_t leftover[4]; /* used for temp buffer too,
...
BIF_RETTYPE unicode_characters_to_binary_2(BIF_ALIST_2)
{
  ...
  bin = erts_new_mso_binary(BIF_P, (uint8_t *)nullptr, need);
                                     ...
                                     Eterm latin1, int left)
{
  ...
  if (err) {
    ...
    BIF_RET(finalize_list_to_list(p, bytes, rest_term, 0U, pos, characters, ERTS_UTF8_ERROR, left,
                                  NIL));
  } else if (rest_term == NIL && num_leftovers != 0) {
    Eterm leftover_bin = new_binary(p, leftover, num_leftovers);
...
static BIF_RETTYPE characters_to_list_trap_1(BIF_ALIST_3)
{
  ...
  bytes = rc->bytes;
  rc->bytes = nullptr; /* to avoid free due to later GC */
...
BIF_RETTYPE unicode_characters_to_list_2(BIF_ALIST_2)
{
  ...
  if (is_binary(BIF_ARG_1) && !latin1) { /* Optimized behaviour for this_ case */
  ...
  bytes = (uint8_t *)alloc_restart(need);
  cost_to_proc(BIF_P, simple_loops_to_common(cost_of_utf8_need));
  left = allowed_iterations(BIF_P) -
         simple_loops_to_common(cost_of_utf8_need);
  ...
  while (size) {
    if (((*source) & ((uint8_t) 0x80)) == 0) {
      ...
      if (num_latin1_chars) {
        ...
        if ((source[0] & ((uint8_t) 0xFC)) != ((uint8_t) 0xC0)) {
          is_latin1 = 0;
        }
      }
...
static BIF_RETTYPE characters_to_list_trap_2(BIF_ALIST_3)
{
  ...
  bytes = rc->bytes;
  rc->bytes = nullptr; /* Don't want this_ freed just yet... */
 ...
 * indicates how to return (error, incomplete or ok) in this_ stage.
 ...
 *
 * Note that this_ routine is *only* called when original input was a plain utf8 binary,
 ...
 * we only reach this_ if original input was a binary.
 */
...
static BIF_RETTYPE characters_to_list_trap_3(BIF_ALIST_3)
{
  ...
  return do_bif_utf8_to_list(BIF_P,
                             ...
                             num_resulting_chars,
                             ERTS_UTF8_ANALYZE_MORE, /* always this_ state here */
...
system_limit:
    ...
    if (!must_exist) {
      res = erts_atom_put((uint8_t *) bytes,
                          bin_size,
                          ERTS_ATOM_ENC_UTF8,
                          0);
                                        ...
                                        size_t extra)
{
  ...
  if ((allow_atom && is_atom(name)) ||
    ...
    erts_native_filename_put(name, encoding, (uint8_t *)name_buf);
    name_buf[need - 1] = 0;
                                     ...
                                     size_t extra_wchars)
{
  ...
  if (erts_analyze_utf8(bytes, size, &err_pos, &num_chars, nullptr) != ERTS_UTF8_OK ||
    ...
    erts_copy_utf8_to_utf16_little((uint8_t *) name_buf, bytes, num_chars);
...
static int filename_len_16bit(uint8_t *str)
{
  uint8_t *p = str;
...
void erts_copy_utf8_to_utf16_little(uint8_t *target, uint8_t *bytes, int num_chars)
{
  ...
  while (num_chars--) {
    ...
    *target++ = (uint8_t)((unipoint >> 8) & 0xFF);
  }
}
...
BIF_RETTYPE prim_file_internal_name2native_1(BIF_ALIST_1)
{
  ...
  if (is_binary(BIF_ARG_1)) {
    ...
    if (encoding != ERL_FILENAME_WIN_WCHAR) {
      /*Add 0 termination only*/
      bin_term = new_binary(BIF_P, nullptr, size + 1);
      bin_p = binary_bytes(bin_term);
    ...
    /* Determine real length and create a new_ binary */
    if (erts_analyze_utf8(bytes, size, &err_pos, &num_chars, nullptr) != ERTS_UTF8_OK ||
        erts_get_user_requested_filename_encoding() ==  ERL_FILENAME_LATIN1) {
      /* What to do now? Maybe latin1, so just take byte for byte instead */
...
BIF_RETTYPE prim_file_is_translatable_1(BIF_ALIST_1)
{
   ...
   */
  bytes = erts_get_aligned_binary_bytes(BIF_ARG_1, &temp_alloc);
  status = erts_analyze_utf8(bytes, size, &err_pos, &num_chars, nullptr);
  erts_free_aligned_binary_bytes(temp_alloc);
