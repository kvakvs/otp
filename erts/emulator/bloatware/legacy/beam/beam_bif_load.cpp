...
finish_loading_1(BIF_ALIST_1)
{
  ...
  p = (m *)erts_alloc(ERTS_ALC_T_LOADER_TMP, n * sizeof(struct m));
                 ...
                 struct m *loaded, int nloaded)
{
...
#endif
  {
     ...
     * we can commit and let them access the new_ staged code. This allows
...
error:
  ...
  if (BIF_ARG_2 == am_true) {
    ...
    /*
     * The on_load function succeded. Fix up export_ entries.
     */
    ...
    if (thing_subtag(oh->thing_word) == FUN_SUBTAG) {
      ...
      if (INSIDE((BeamInstr *) funp->fe->address)) {
        if (done_gc) {
          ...
          /*
          * Try to get rid of this_ fun by garbage collecting.
...
/*
 * Move code from current to old and null all export_ entries for the module
 */
...
is_native(BeamInstr *code)
{
  ...
  for (i = 0; i < num_functions; i++) {
    ...
    if (is_atom(name)) {
      ...
      ASSERT(is_nil(name));  /* ignore BIF stubs */
    }
  }
