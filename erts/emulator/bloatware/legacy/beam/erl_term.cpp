...
et_abort(const char *expr, const char *file, unsigned line)
{
  ...
  if (have_been_called) {
     ...
     */
    have_been_called = 1;
    erl::exit(1, "TYPE ASSERTION FAILED, file %s, line %u: %s\n", file, line, expr);
  }
...
#endif
{
  ...
  case TAG_PRIMARY_BOXED: {
    ...
    case (_TAG_HEADER_EXTERNAL_PID >> _TAG_PRIMARY_SIZE):
      return EXTERNAL_PID_DEF;

    case (_TAG_HEADER_EXTERNAL_PORT >> _TAG_PRIMARY_SIZE):
      return EXTERNAL_PORT_DEF;

    case (_TAG_HEADER_EXTERNAL_REF >> _TAG_PRIMARY_SIZE):
      return EXTERNAL_REF_DEF;

    case (_TAG_HEADER_REFC_BIN >> _TAG_PRIMARY_SIZE):
      return BINARY_DEF;

    case (_TAG_HEADER_HEAP_BIN >> _TAG_PRIMARY_SIZE):
      return BINARY_DEF;

    case (_TAG_HEADER_SUB_BIN >> _TAG_PRIMARY_SIZE):
      return BINARY_DEF;
...
/*
 * XXX: define NUMBER_CODE() here when new_ representation is used
 */
...
ET_DEFINE_CHECKED(size_t, unsigned_val, Eterm, is_small);
ET_DEFINE_CHECKED(ssize_t, signed_val, Eterm, is_small);
ET_DEFINE_CHECKED(size_t, atom_val, Eterm, is_atom);
ET_DEFINE_CHECKED(size_t, header_arity, Eterm, is_header);
ET_DEFINE_CHECKED(size_t, arityval, Eterm, is_sane_arity_value);
ET_DEFINE_CHECKED(size_t, thing_arityval, Eterm, is_thing);
ET_DEFINE_CHECKED(size_t, thing_subtag, Eterm, is_thing);
...
ET_DEFINE_CHECKED(size_t, bignum_header_arity, Eterm, _is_bignum_header);
...
ET_DEFINE_CHECKED(size_t, internal_ref_data_words, Wterm, is_internal_ref);
ET_DEFINE_CHECKED(uint32_t *, internal_ref_data, Wterm, is_internal_ref);
...
ET_DEFINE_CHECKED(size_t, external_data_words, Wterm, is_external);
ET_DEFINE_CHECKED(size_t, external_pid_data_words, Wterm, is_external_pid);
ET_DEFINE_CHECKED(size_t, external_pid_data, Wterm, is_external_pid);
ET_DEFINE_CHECKED(struct erl_node_ *, external_pid_node, Wterm, is_external_pid);
ET_DEFINE_CHECKED(size_t, external_port_data_words, Wterm, is_external_port);
ET_DEFINE_CHECKED(size_t, external_port_data, Wterm, is_external_port);
ET_DEFINE_CHECKED(struct erl_node_ *, external_port_node, Wterm, is_external_port);
ET_DEFINE_CHECKED(size_t, external_ref_data_words, Wterm, is_external_ref);
ET_DEFINE_CHECKED(uint32_t *, external_ref_data, Wterm, is_external_ref);
...
ET_DEFINE_CHECKED(size_t, external_thing_data_words, ExternalThing *, is_thing_ptr);

ET_DEFINE_CHECKED(Eterm, make_cp, UWord *, _is_taggable_pointer);
ET_DEFINE_CHECKED(UWord *, cp_val, Eterm, is_CP);
ET_DEFINE_CHECKED(size_t, catch_val, Eterm, is_catch);
ET_DEFINE_CHECKED(size_t, x_reg_offset, size_t, _is_xreg);
ET_DEFINE_CHECKED(size_t, y_reg_offset, size_t, _is_yreg);
ET_DEFINE_CHECKED(size_t, x_reg_index, size_t, _is_xreg);
ET_DEFINE_CHECKED(size_t, y_reg_index, size_t, _is_yreg);
