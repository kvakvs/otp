...
static Eterm
set_tracee_flags(Process *tracee_p, Eterm tracer, size_t d_flags, size_t e_flags)
{
  Eterm ret;
  size_t flags;
...
get_match_pseudo_process(Process *c_p, size_t heap_size)
{
  ErtsMatchPseudoProcess *mpsp;
#ifdef ERTS_SMP
...
static size_t my_size_object(Eterm t);
...
#define FENCE_PATTERN_SIZE (1*sizeof(size_t))
...
BIF_RETTYPE db_get_trace_control_word(Process *p)
{
  ...
  BIF_RET(erts_make_integer((size_t) tcw, p));
}
...
BIF_RETTYPE db_set_trace_control_word(Process *p, Eterm new_)
{
  ...
  old_tcw = (uint32_t) erts_smp_atomic32_xchg_relb(&trace_control_word,
            (erts_aint32_t) val);
  BIF_RET(erts_make_integer((size_t) old_tcw, p));
}
...
simple_term:
    ...
    ** iff the text is two size_t's and the single instruction
  ...
  ** The "program memory" is allocated like this_:
  ** text ----> +-------------+
  **            |             |
  **              ..........
  **            +-------------+
...
** and not especially nice case when this_ is used.
*/
static Eterm dpm_array_to_list(Process *psp, Eterm *arr, int arity)
{
...
/*
** Execution of the match program, this_ is Pam.
...
restart:
  ...
  for (;;) {
    ...
    if (*heap_fence != FENCE_PATTERN) {
      erl::exit(1, "Heap fence overwritten in db_prog_match after op "
               "0x%08x, overwritten with 0x%08x.", save_op, *heap_fence);
    }

    if (*stack_fence != FENCE_PATTERN) {
      erl::exit(1, "Stack fence overwritten in db_prog_match after op "
...
case_matchPushV:
    ...
    case matchProcessDump: {
      ...
      *esp++ = new_binary(build_proc, (uint8_t *)dsbufp->str,
                          dsbufp->str_len);
    ...
    case matchTrace2: {
      /*    disable         enable                                */
      size_t  d_flags  = 0,   e_flags  = 0;  /* process trace flags */
    ...
    default:
      erl::exit(1, "Internal error: unexpected opcode in match program.");
    }
  }
...
Eterm db_format_dmc_err_info(Process *p, DMCErrInfo *ei)
{
  ...
  for (tmp = ei->first; tmp != nullptr; tmp = tmp->next) {
    ...
    sev = (tmp->severity == dmcWarning) ?
          ...
          am_error;
    tlist = util::buf_to_intlist(&shp, buff, sl, NIL);
...
** newval:   The new_ value of the element.
** Can not fail.
*/
...
both_size_set:
  ...
  /* write new_ value in old dbterm, finalize will make a flat copy */
  handle->dbterm->tpl[position] = newval;
  handle->mustResize = 1;
...
static ERTS_INLINE uint8_t *db_realloc_term(DbTableCommon *tb, void *old,
    size_t old_sz, size_t new_sz, size_t offset)
{
  uint8_t *ret;

  if (erts_ets_realloc_always_moves) {
    ret = (uint8_t *)erts_db_alloc(ERTS_ALC_T_DB_TERM, (DbTable *)tb, new_sz);
    sys_memcpy(ret, old, offset);
    erts_db_free(ERTS_ALC_T_DB_TERM, (DbTable *)tb, old, old_sz);
  } else {
    ret = (uint8_t *)erts_db_realloc(ERTS_ALC_T_DB_TERM, (DbTable *)tb,
                                     old, old_sz, new_sz);
  }
...
*/
static ERTS_INLINE size_t db_alloced_size_comp(DbTerm *obj)
{
...
{
  DbTerm *db = (DbTerm *)((uint8_t *)basep + offset);
  size_t size;
...
static ERTS_INLINE size_t align_up(size_t value, size_t pow2)
{
...
*/
static size_t db_size_dbterm_comp(DbTableCommon *tb, Eterm obj)
{
  Eterm *tpl = tuple_val(obj);
  int i;
  size_t size = sizeof(DbTerm)
  ...
  return align_up(size, sizeof(size_t));
}

/* Conversion between top tuple element and pointer to compressed data
*/
static ERTS_INLINE Eterm ext2elem(Eterm *tpl, uint8_t *ext)
{
  return (((size_t)(ext - (uint8_t *)tpl)) << _TAG_PRIMARY_SIZE) | TAG_PRIMARY_HEADER;
}
static ERTS_INLINE uint8_t *elem2ext(Eterm *tpl, size_t ix)
{
  ASSERT(is_header(tpl[ix]));
  return (uint8_t *)tpl + (tpl[ix] >> _TAG_PRIMARY_SIZE);
}
...
/*
** Copy the object into a possibly new_ DbTerm,
...
void *db_store_term(DbTableCommon *tb, DbTerm *old, size_t offset, Eterm obj)
{
  ...
  if (old != 0) {
    ...
    if (size == old->size) {
      ...
      size_t new_sz = offset + sizeof(DbTerm) + sizeof(Eterm) * (size - 1);
      size_t old_sz = offset + sizeof(DbTerm) + sizeof(Eterm) * (old->size - 1);

      basep = db_realloc_term(tb, basep, old_sz, new_sz, offset);
      newp = (DbTerm *)(basep + offset);
    }
  } else {
    basep = (uint8_t *)erts_db_alloc(ERTS_ALC_T_DB_TERM, (DbTable *)tb,
...
{
  size_t new_sz = offset + db_size_dbterm_comp(tb, obj);
  uint8_t *basep;
  DbTerm *newp;
#ifdef DEBUG
  uint8_t *top;
  ...
  if (old != 0) {
    size_t old_sz = db_alloced_size_comp(old);
    ...
    basep = (uint8_t *)erts_db_alloc(ERTS_ALC_T_DB_TERM, (DbTable *)tb, new_sz);
    newp = (DbTerm *)(basep + offset);
  }
...
void db_finalize_resize(DbUpdateHandle *handle, size_t offset)
{
  ...
  DbTerm *newDbTerm;
  size_t alloc_sz = offset +
                  (tbl->common.compress ?
                   db_size_dbterm_comp(&tbl->common, make_tuple(handle->dbterm->tpl)) :
                   sizeof(DbTerm) + sizeof(Eterm) * (handle->new_size - 1));
  uint8_t *newp = (uint8_t *)erts_db_alloc(ERTS_ALC_T_DB_TERM, tbl, alloc_sz);
  uint8_t *oldp = (uint8_t *) * (handle->bp);
  ...
  *(handle->bp) = newp;
  ...
  if (tbl->common.compress) {
    ...
    if (handle->abs_vec) {
      ...
      ASSERT((uint8_t *)top <= (newp + alloc_sz));
...
void db_cleanup_offheap_comp(DbTerm *obj)
{
  ...
  for (u.hdr = obj->first_oh; u.hdr; u.hdr = u.hdr->next) {
    if ((UWord)u.voidp % sizeof(size_t) != 0) { /* unaligned ptr */
...
#else
      text->push((size_t) float_val(c)[1]);
#ifdef ARCH_64
      text->push((size_t) 0);
#else
      text->push((size_t) float_val(c)[2]);
#endif
#endif
      break;
                      ...
                      int *constant)
{
  ...
  if (!(b->flags &
    ...
    if (context->err_info != nullptr) {
      ...
      char buff[100];
      erts_snprintf(buff, sizeof(buff),
                    "Function %%T/%d cannot be called in this_ context.",
                    (int)a - 1);
      RETURN_TERM_ERROR(buff, p[1], context, *constant);
    } else {
      return retFail;
  ...
  *constant = 0;
  ...
  default:
    erl::exit(1, "ets:match() internal error, "
...
static int match_compact(ErlHeapFragment *expr, DMCErrInfo *err_info)
{
  ...
  while (i--) {
    if (is_thing(*p)) {
      ...
      *p = erts_atom_put((uint8_t *) buff, strlen(buff),
...
static size_t my_size_object(Eterm t)
{
  ...
  case TAG_PRIMARY_BOXED:
    ...
    if (arityval(*tuple_val(t)) == 1 && is_tuple(tmp = tuple_val(t)[1])) {
      ...
      erl::exit(1, "Internal error, sizing unrecognized object in "
    ...
    if (BOXED_IS_TUPLE(t)) {
      if (arityval(*tuple_val(t)) == 1 &&
        ...
        *savep++ = make_arityval(n);
        ...
        erl::exit(1, "Trying to constant-copy non constant expression "
                      ...
                      int all, DbTerm *obj, Eterm **hpp, size_t extra)
{
  ...
  if (tb->compress) {
    ...
    base = HALFWORD_HEAP ? obj->tpl : nullptr;
  }
