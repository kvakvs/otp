 ...
 *              this_ utility. The only task the callback-module has to
...
{
  void *res = erts_sys_alloc(0, nullptr, size);
...
{
  void *res = erts_sys_realloc(0, nullptr, ptr, size);
...
#endif
    res = erts_sys_alloc(0, nullptr, size);
...
#endif
    res = erts_sys_realloc(0, nullptr, ptr, size);
...
{
  return crr->cpool.abandoned.next != nullptr;
}
...
unlink_abandoned_carrier(Carrier_t *crr)
{
  ...
  crr->cpool.abandoned.next = nullptr;
  crr->cpool.abandoned.prev = nullptr;
}
...
fix_cpool_alloc(Allctr_t *allctr, ErtsAlcType_t type, size_t size)
{
  ...
  if (res) {
    ...
    fix->u.cpool.used++;
    fix_cpool_check_shrink(allctr, type, fix, nullptr);
    return res;
  }
               ...
               Carrier_t **busy_pcrr_pp)
{
  ...
  if ((!busy_pcrr_pp || !*busy_pcrr_pp)
    ...
    *((void **) p) = fix->list;
    ...
    if (IS_SBC_BLK(blk)) {
      destroy_carrier(allctr, blk, nullptr);
    } else {
  ...
  if (ddq->head.next.thr_progress_reached
    ...
    if (erts_atomic_read_nob(&ddq->tail.data.um_refc[um_refc_ix]) == 0) {
      ...
      if (ddq->head.unref_end != (ErtsAllctrDDBlock_t *) ilast) {
        ...
        ddq->head.next.thr_progress = erts_thr_progress_later(nullptr);
...
doit:
      ...
      /* Carrier migrated; need to redirect block to new_ owner... */
...
dequeue:
    ...
    if (IS_FREE_LAST_MBC_BLK(blk)) {
       ...
       * if this_ carrier is pulled from dc_list by cpool_fetch()
       */
...
#define ERTS_ALCU_HANDLE_DD_IN_OP(Allctr, Locked)     \
    handle_delayed_dealloc((Allctr), (Locked), 1,       \
                           ERTS_ALCU_DD_OPS_LIM_LOW, nullptr, nullptr, nullptr)
...
mbc_alloc_block(Allctr_t *allctr, size_t size, size_t *blk_szp)
{
  ...
  *blk_szp = get_blk_sz = UMEMSZ2BLKSZ(allctr, size);
  ...
  blk = (*allctr->get_free_block)(allctr, get_blk_sz, nullptr, 0);
            ...
            Carrier_t **busy_pcrr_pp)
{
  ...
  if (old_blk_sz == blk_sz) {
    ...
    STAT_MBC_BLK_FREE(allctr, crr, nullptr, old_blk_sz, alcu_flgs);
    STAT_MBC_BLK_ALLOC(allctr, crr, blk_sz, alcu_flgs);
...
#ifdef ERTS_SMP
    check_abandon_carrier(allctr, nxt_blk, nullptr);
#endif
  ...
  if (cand_blk_sz < get_blk_sz) {
    /* We wont fit in cand_blk get a new_ one */
#ifdef ERTS_SMP
realloc_move:
#endif
...
move_into_new_blk:
  ...
  if (new_blk) {
    ...
    /*
     * Copy user-data then update new_ blocks in mbc_alloc_finalize().
 ...
 * threads except in the alloc_SUITE:cpool case. In this_
...
cpool_insert(Allctr_t *allctr, Carrier_t *crr)
{
  ...
  /* Write pointers to this_ element in successor and predecessor */
...
cpool_delete(Allctr_t *allctr, Allctr_t *prev_allctr, Carrier_t *crr)
{
  ...
  /* Write pointers past this_ element in predecessor and successor */
  ...
  /* Repleace mod markers with delete markers on this_ element */
  ...
  crr->cpool.thr_prgr = erts_thr_progress_later(nullptr);
                              ...
                              int *need_more_work)
{
  ...
  if (crr) {
    ...
    if (!crr) {
      allctr->cpool.dc_list.last = nullptr;
    } else {
      crr->prev = nullptr;
...
schedule_dealloc_carrier(Allctr_t *allctr, Carrier_t *crr)
{
  ...
  if (allctr != orig_allctr) {
     ...
     *
     * The receiver will recognize that this_ is a carrier to
  ...
  if (check_pending_dealloc) {
    check_pending_dealloc_carrier(allctr, nullptr, nullptr, nullptr);
  }
...
create_carrier(Allctr_t *allctr, size_t umem_sz, UWord flags)
{
  ...
  if ((flags & (CFLG_MBC | CFLG_NO_CPOOL)) == CFLG_MBC
    ...
    if (crr) {
      ...
      blk = (*allctr->get_free_block)(allctr, blk_sz, nullptr, 0);
      ASSERT(blk);
      return blk;
    }
  }
...
#define bld_string  erts_bld_string
               ...
               size_t *szp)
    {
      ...
      if (print_to_p || hpp) {
        if (sz_only) {
          cpool_read_stat(allctr, nullptr, &csz, nullptr, &bsz);
        } else {
...
#undef PRINT_CC_5
                              ...
                              size_t **hpp, size_t *szp)
    {
      ...
      if (hpp || szp) {
        res = NIL;
        ensure_atoms_initialized(nullptr);
        add_2tup(hpp, szp, &res,
                 am.sac,
    ...
    {
      do_erts_alcu_free(type, extra, p, nullptr);
    }
    ...
    erts_alcu_free_thr_spec(ErtsAlcType_t type, void *extra, void *p)
    {
      ...
      do_erts_alcu_free(type, allctr, p, nullptr);
      ...
      if (size < allctr->sbc_threshold) {
        if (IS_MBC_BLK(blk)) {
          ...
          if (100 * diff_sz_val < allctr->sbc_move_threshold * crr_sz_val)
            /* Data won't be copied into a new_ carrier... */
          {
            goto do_carrier_resize;
...
do_carrier_resize:
...
#endif
          res = new_blk ? BLK2UMEM(new_blk) : nullptr;
        } else if (alcu_flgs & ERTS_ALCU_FLG_FAIL_REALLOC_MOVE) {
          return nullptr;
        } else {
    ...
    erts_alcu_realloc_ts(ErtsAlcType_t type, void *extra, void *ptr, size_t size)
    {
      ...
      void *res;
      erts_mtx_lock(&allctr->mutex);
      res = do_erts_alcu_realloc(type, extra, ptr, size, 0, nullptr);
      erts_mtx_unlock(&allctr->mutex);
      DEBUG_CHECK_ALIGNMENT(res);
      return res;
    }
                               ...
                               void *ptr, size_t size)
    {
      ...
      res = do_erts_alcu_realloc(type, allctr, ptr, size, 0, nullptr);
                                  ...
                                  void *ptr, size_t size)
    {
      ...
      if (!res) {
        ...
        do_erts_alcu_free(type, allctr, ptr, nullptr);
...
# endif
      ...
      allctr->cpool.dc_list.first = nullptr;
      allctr->cpool.dc_list.last = nullptr;
...
error:
    ...
    erts_alcu_stop(Allctr_t *allctr)
    {
      ...
      while (allctr->sbc_list.first) {
        destroy_carrier(allctr, SBC2BLK(allctr, allctr->sbc_list.first), nullptr);
      }

      while (allctr->mbc_list.first) {
        destroy_carrier(allctr, MBC_TO_FIRST_BLK(allctr, allctr->mbc_list.first), nullptr);
      }
    ...
    erts_alcu_test(UWord op, UWord a1, UWord a2)
    {
      switch (op) {
      case 0x000:
        return (UWord) BLK_SZ((Block_t *) a1);

      case 0x001:
        return (UWord) BLK_UMEM_SZ((Block_t *) a1);

      case 0x002:
        return (UWord) IS_PREV_BLK_FREE((Block_t *) a1);

      case 0x003:
        return (UWord) IS_FREE_BLK((Block_t *) a1);

      case 0x004:
        return (UWord) IS_LAST_BLK((Block_t *) a1);

      case 0x005:
        return (UWord) UMEM2BLK((void *) a1);

      case 0x006:
        return (UWord) BLK2UMEM((Block_t *) a1);

      case 0x007:
        return (UWord) IS_SB_CARRIER((Carrier_t *) a1);

      case 0x008:
        return (UWord) IS_SBC_BLK((Block_t *) a1);

      case 0x009:
        return (UWord) IS_MB_CARRIER((Carrier_t *) a1);

      case 0x00a:
        return (UWord) IS_MSEG_CARRIER((Carrier_t *) a1);

      case 0x00b:
        return (UWord) CARRIER_SZ((Carrier_t *) a1);

      case 0x00c:
        return (UWord) SBC2BLK((Allctr_t *) a1,
                               (Carrier_t *) a2);

      case 0x00d:
        return (UWord) BLK_TO_SBC((Block_t *) a2);

      case 0x00e:
        return (UWord) MBC_TO_FIRST_BLK((Allctr_t *) a1,
                                        (Carrier_t *) a2);

      case 0x00f:
        return (UWord) FIRST_BLK_TO_MBC((Allctr_t *) a1,
                                        (Block_t *) a2);

      case 0x010:
        return (UWord)((Allctr_t *) a1)->mbc_list.first;

      case 0x011:
        return (UWord)((Allctr_t *) a1)->mbc_list.last;

      case 0x012:
        return (UWord)((Allctr_t *) a1)->sbc_list.first;

      case 0x013:
        return (UWord)((Allctr_t *) a1)->sbc_list.last;

      case 0x014:
        return (UWord)((Carrier_t *) a1)->next;

      case 0x015:
        return (UWord)((Carrier_t *) a1)->prev;

      case 0x016:
        return (UWord) ABLK_HDR_SZ;

      case 0x017:
        return (UWord)((Allctr_t *) a1)->min_block_size;

      case 0x018:
        return (UWord) NXT_BLK((Block_t *) a1);

      case 0x019:
        return (UWord) PREV_BLK((Block_t *) a1);

      case 0x01a:
        return (UWord) IS_MBC_FIRST_BLK((Allctr_t *)a1, (Block_t *) a2);

      case 0x01b:
        return (UWord) sizeof(Unit_t);

      case 0x01c:
        return (unsigned long) BLK_TO_MBC((Block_t *) a1);

      case 0x01d:
        ((Allctr_t *) a1)->add_mbc((Allctr_t *)a1, (Carrier_t *)a2);
        break;

      case 0x01e:
        ((Allctr_t *) a1)->remove_mbc((Allctr_t *)a1, (Carrier_t *)a2);
        break;
#ifdef ERTS_SMP

      case 0x01f:
        return (UWord) sizeof(ErtsAlcCrrPool_t);
      ...
      case 0x023:
        return (UWord) cpool_is_empty((Allctr_t *) a1);

      case 0x024:
        return (UWord) cpool_dbg_is_in_pool((Allctr_t *) a1, (Carrier_t *) a2);
#else
