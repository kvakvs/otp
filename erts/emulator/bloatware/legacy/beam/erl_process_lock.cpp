 ...
 * aquire.
 *
 * Note: We need the pix lock during this_ operation. Wait
...
/*
 * Transfer 'trnsfr_lcks' held by this_ executing thread to other
 ...
 *
 * For example, if we want to lock 10111, and 00100 was already locked, this_
 ...
 *
 * If successful, this_ returns 0 and all locks in 'need_locks' are held.
 *
               ...
               ErtsProcLocks olflgs)
{
  ...
  /* Acquire a waiter object on which this_ thread can wait. */
  wtr = tse_fetch(pix_lock);

  /* Record which locks this_ waiter needs. */
...
void erts_lcnt_proc_lock_post_x(erts_proc_lock_t *lock, ErtsProcLocks locks, char *file,
                                unsigned int line)
{
  if (erts_lcnt_rt_options & ERTS_LCNT_OPT_PROCLOCK) {
    if (locks & ERTS_PROC_LOCK_MAIN) {
