...
/* Pseudo export_ entries. Never filled in with data, only used to
...
#endif
  erts_bif_trace_init();
  erts_system_monitor_clear(nullptr);
  erts_system_profile_clear(nullptr);
...
#define ERTS_ENQ_TRACE_MSG(FPID, TPROC, MSG, BP) \
    erts_queue_message((TPROC), nullptr, (BP), (MSG), NIL, NIL)
#else
#define ERTS_ENQ_TRACE_MSG(FPID, TPROC, MSG, BP) \
    erts_queue_message((TPROC), nullptr, (BP), (MSG), NIL)
#endif
#endif
...
#else
#define ERTS_NULL_TRACER_REF nullptr
...
Eterm
erts_set_system_seq_tracer(Process *c_p, ErtsProcLocks c_p_locks, Eterm new_)
{
  Eterm old;

  if (new_ != am_false && !is_valid_tracer(new_)) {
    return THE_NON_VALUE;
  }

  erts_smp_rwmtx_rwlock(&sys_trace_rwmtx);
  old = system_seq_tracer;
  system_seq_tracer = new_;

#ifdef DEBUG_PRINTOUTS
  erts_fprintf(stderr, "set seq tracer new_=%T old=%T\n", new_, old);
#endif
  erts_smp_rwmtx_rwunlock(&sys_trace_rwmtx);
  return old;
}
                      ...
                      Eterm message)
{
  ...
  buffer = (uint8_t *) erts_alloc(ERTS_ALC_T_TMP, size);
  ...
  if (!(ptr <= buffer + size)) {
    erl::exit(1, "Internal error in do_send_to_port: %d\n", ptr - buffer);
  }
...
/* If (c_p != nullptr), a fake schedule out/in message pair will be sent,
...
if (*tracee_flags & F_TIMESTAMP) {
   ...
   * but this_ message does not contain a timestamp.
...
profile_send(Eterm from, Eterm message)
{
  ...
  if (is_internal_port(profiler)) {
    ...
    erts_queue_message(profile_p, nullptr, bp, msg, NIL
#ifdef USE_VM_PROBES
                       , NIL
#endif
                      );
 ...
 *
 * 'c_p' is the currently executing process, may be nullptr.
 */
static void
               ...
               int no_fake_sched)
{
  ...
  if (is_internal_pid(ERTS_TRACER_PROC(tracee))) {
    ...
    send_to_port(no_fake_sched ? nullptr : tracee,
                 msg,
                         ...
                         Eterm receiver, Process *process, Eterm exitfrom)
{
  ...
  default:
    erl::exit(1, "invalid type in seq_trace_output_generic: %d:\n", type);
...
#undef LOCAL_HEAP_SIZE
    ...
    erts_smp_mtx_unlock(&smq_mtx);
#else
    erts_queue_message(tracer, nullptr, bp, mess, NIL
#ifdef USE_VM_PROBES
                       , NIL
#endif
...
/*
 * This function implements the new_ call trace.
 *
                ...
                Eterm *args, int local, Eterm *tracer_pid)
{
  ...
  if (is_internal_port(*tracer_pid)) {
    ...
    /* BEGIN this_ code should be the same for port and pid trace */
    return_flags = 0;
    ...
    /* END this_ code should be the same for port and pid trace */
...
#ifdef DEBUG
    size = 0;
    (void) erts_bld_atom_uword_2tup_list(nullptr,
                                         &size,
                                         sizeof(values) / sizeof(*values),
                                         tags,
                                         values);
...
monitor_long_schedule_proc(Process *p, BeamInstr *in_fp, BeamInstr *out_fp, Uint time)
{
   ...
   * = 20 + (in_fp != nullptr) ? 4 : 0 + (out_fp != nullptr) ? 4 : 0 + size of Timeout
   */
  hsz = 20 + ((in_fp != nullptr) ? 4 : 0) + ((out_fp != nullptr) ? 4 : 0);
  (void) erts_bld_uint(nullptr, &hsz, time);
  hp = ERTS_ALLOC_SYSMSG_HEAP(hsz, &bp, &off_heap, monitor_p);
  tmo = erts_bld_uint(&hp, nullptr, time);
...
#else
  erts_queue_message(monitor_p, nullptr, bp, msg, NIL
#ifdef USE_VM_PROBES
                     , NIL
#endif
                    );
...
#define LOCAL_HEAP_SIZE (4 + 7)
  ...
  msg = TUPLE6(hp, am_profile, am_scheduler, scheduler_id, state,
               make_small(active_sched), timestamp);
  hp += 7;
...
#define LOCAL_HEAP_SIZE (4 + 7)
  ...
  timestamp = TUPLE3(hp, make_small(Ms), make_small(s), make_small(us));
  hp += 4;
  msg = TUPLE6(hp, am_profile, am_scheduler, scheduler_id, state, no_schedulers, timestamp);
  hp += 7;
#ifndef ERTS_SMP
  profile_send(NIL, msg);
  UnUseTmpHeapNoproc(LOCAL_HEAP_SIZE);
...
#define LOCAL_HEAP_SIZE (5+6)
    ...
    /* No fake schedule */
    send_to_port(nullptr, mess, &ERTS_TRACER_PROC(p), &ERTS_TRACE_FLAGS(p));
    UnUseTmpHeapNoproc(LOCAL_HEAP_SIZE);
#undef LOCAL_HEAP_SIZE
    erts_smp_mtx_unlock(&smq_mtx);
  } else {
...
#define LOCAL_HEAP_SIZE (5+5)
    ...
    /* No fake schedule */
    send_to_port(nullptr, mess, &ERTS_TRACER_PROC(t_p), &ERTS_TRACE_FLAGS(t_p));
    UnUseTmpHeapNoproc(LOCAL_HEAP_SIZE);
#undef LOCAL_HEAP_SIZE
    erts_smp_mtx_unlock(&smq_mtx);
  } else {
...
#define LOCAL_HEAP_SIZE (5+6)
    ...
    if (IS_TRACED_FL(p, F_TRACE_SCHED_NO)) {
      ...
      if (esd) {
        sched_id = make_small(esd->no);
      } else {
        sched_id = am_undefined;
      }
...
#undef LOCAL_HEAP_SIZE
    ...
    if (IS_TRACED_FL(p, F_TRACE_SCHED_NO)) {
      ws = 6;  /* Make place for scheduler id */
    }
...
#define LOCAL_HEAP_SIZE (4 + 6)
  ...
  timestamp = TUPLE3(hp, make_small(Ms), make_small(s), make_small(us));
  hp += 4;
  msg = TUPLE5(hp, am_profile, p->common.id, status, count, timestamp);
  hp += 6;
...
#define LOCAL_HEAP_SIZE (4 + 6 + 4)
  ...
  timestamp = TUPLE3(hp, make_small(Ms), make_small(s), make_small(us));
  hp += 4;
  msg = TUPLE5(hp, am_profile, p->common.id, status, where, timestamp);
  hp += 6;
#ifndef ERTS_SMP
  profile_send(p->common.id, msg);
  UnUseTmpHeapNoproc(LOCAL_HEAP_SIZE);
...
sys_msg_dispatcher_func(void *unused)
{
  ...
  ErtsSysMsgQ *local_sys_message_queue = nullptr;
  int wait = 0;
  ...
  erts_thr_progress_register_managed_thread(nullptr, &callbacks, 0);
  ...
  while (1) {
    ...
    sys_message_queue = nullptr;
    sys_message_queue_end = nullptr;

    erts_smp_mtx_unlock(&smq_mtx);

    if (end_wait) {
      erts_thr_progress_finalize_wait(nullptr);
      erts_thr_progress_active(nullptr, 1);
    }
    ...
    for (smqp = local_sys_message_queue; smqp; smqp = smqp->next) {
      ...
      if (erts_thr_progress_update(nullptr)) {
      ...
      if (is_internal_pid(receiver)) {
        proc = erts_pid2proc(nullptr, 0, receiver, proc_locks);
...
queue_proc_msg:
        ...
        proc = erts_whereis_process(nullptr, 0, receiver, proc_locks, 0);
