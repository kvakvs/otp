...
BIF_RETTYPE open_port_2(BIF_ALIST_2)
{
  ...
  if (!port) {
    ...
    BIF_P->fvalue = erts_atom_put((uint8_t *) str, strlen(str), ERTS_ATOM_ENC_LATIN1, 1);
  ...
  *err_nump = 0;
    ...
    while (1) {
      if (is_tuple_arity(*nargs, 2)) {
        ...
        if (option == am_packet) {
          ...
          if ((a0 = erts_convert_filename_to_native(*tp, nullptr, 0, ERTS_ALC_T_TMP, 1, 1,
                    nullptr)) == nullptr) {
            goto badarg;
          }
  ...
  if (is_atom(name) || (i = is_string(name))) {
    ...
    if (*tp == am_spawn || *tp == am_spawn_driver || *tp == am_spawn_executable) {  /* A process port */
      ...
      if ((name_buf = erts_convert_filename_to_encoding(name, nullptr, 0, ERTS_ALC_T_TMP, 0, 1, encoding,
                      nullptr, 0))
          == nullptr) {
        goto badarg;
      }
  ...
  if (edir != NIL) {
    if ((opts.wd = erts_convert_filename_to_native(edir, nullptr, 0, ERTS_ALC_T_TMP, 0, 1,
                   nullptr)) == nullptr) {
      goto badarg;
    }
  }
...
static char **convert_args(Eterm l)
{
  ...
  while (is_list(l)) {
    ...
    if ((b = erts_convert_filename_to_native(str, nullptr, 0, ERTS_ALC_T_TMP, 1, 1,
             nullptr)) == nullptr) {
      int j;
...
static uint8_t *convert_environment(Process *p, Eterm env)
{
   ...
   * gets deallocated. Do NOT return directly from this_ function.
   */
  ...
  if (pca->string_as_bin) {
    ...
    if (hpp) {
      ...
      if (make_subbin) {
        ...
        bin->size = len;
        bin->offs = pca->bin_offs + ((uint8_t *)str - pca->aligned_ptr);
        bin->orig = pca->orig;
                ...
                const char *buf, int len, const char *prefix, int plen)
{
  ...
  uint8_t *bin_ptr = binary_bytes(bin);
...
BIF_RETTYPE decode_packet_3(BIF_ALIST_3)
{
  ...
  uint8_t *bin_ptr;           /*| orig: original binary                     */
  uint8_t bin_bitsz;          /*| bin: BIF_ARG_2, may be sub-binary of orig */
...
next_option:
  ...
  if (pca.bin_bitoffs != 0) {
    pca.aligned_ptr = (uint8_t *)erts_alloc(ERTS_ALC_T_TMP, pca.bin_sz);
