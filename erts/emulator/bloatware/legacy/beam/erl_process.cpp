...
#define ERTS_DELAYED_WAKEUP_REDUCTIONS ((uint64_t) vm::CONTEXT_REDS/2)

#define ERTS_RUNQ_CHECK_BALANCE_REDS_PER_SCHED (2000*vm::CONTEXT_REDS)
#define ERTS_RUNQ_CALL_CHECK_BALANCE_REDS \
  (ERTS_RUNQ_CHECK_BALANCE_REDS_PER_SCHED/2)

#define ERTS_PROC_MIN_CONTEXT_SWITCH_REDS_COST (vm::CONTEXT_REDS/10)
...
#define ERTS_EMPTY_RUNQ_PORTS(RQ) \
    (RUNQ_READ_LEN(&(RQ)->ports.info.len) == 0 && (RQ)->misc.start == nullptr)
...
erts_sched_wall_time_request(Process *c_p, int set, int enable)
{
  ...
  swtrp->enable = enable;
  swtrp->proc = c_p;
  swtrp->ref = STORE_NC(&hp, nullptr, ref);
...
erts_psd_set_init(Process *p, ErtsProcLocks plocks, int ix, void *data)
{
  ...
  int refc = 0;
  ErtsPSD *psd = (ErtsPSD *)erts_alloc(ERTS_ALC_T_PSD, sizeof(ErtsPSD));
  int i;
                     ...
                     int waiting)
{
  ...
  while (1) {
    auto mawp = (erts_misc_aux_work_t *)erts_thr_q_dequeue(q);
...
handle_delayed_dealloc(ErtsAuxWorkData *awdp, erts_aint32_t aux_work, int waiting)
{
  ...
  if (need_thr_progress) {
    ...
    awdp->dd.completed_callback((void *)awdp->dd.completed_arg);
    awdp->dd.completed_callback = nullptr;
    awdp->dd.completed_arg = nullptr;
  }
...
setup_completed_dealloc(void *vproc)
{
  ...
  awdp->dd.completed_arg = (void (*)(void *))vproc;
}
...
erts_debug_wait_deallocations(Process *c_p)
{
  ...
  return 1;
#else
  /* Only one process at a time can do this_ */
...
aux_thread(void *unused)
{
  ...
  erts_thr_progress_register_managed_thread(nullptr, &callbacks, 1);
  init_aux_work_data(awdp, nullptr, nullptr);
  awdp->ssi = ssi;
  ...
  while (1) {
    ...
    if (aux_work) {
      if (!thr_prgr_active) {
        erts_thr_progress_active(nullptr, thr_prgr_active = 1);
      }

      aux_work = handle_aux_work(awdp, aux_work, 1);

      if (aux_work && erts_thr_progress_update(nullptr)) {
        erts_thr_progress_leader_update(nullptr);
      }
    }

    if (!aux_work) {
      if (thr_prgr_active) {
        erts_thr_progress_active(nullptr, thr_prgr_active = 0);
      }
      ...
      erts_thr_progress_finalize_wait(nullptr);
    }
...
sys_aux_work:
      ...
      /*
       * If we got new_ I/O tasks we aren't allowed to
...
wake_scheduler(ErtsRunQueue *rq)
{
   ...
   * so all code *should* handle this_ without having
...
set_no_used_runqs(int used)
{
  ...
  while (1) {
    erts_aint32_t act, new_;
    new_ = (used & ERTS_NO_RUNQS_MASK) << ERTS_NO_USED_RUNQS_SHIFT;
    new_ |= exp & ERTS_NO_RUNQS_MASK;
    act = erts_smp_atomic32_cmpxchg_nob(&balance_info.no_runqs, new_, exp);
...
set_no_active_runqs(int active)
{
  ...
  while (1) {
    ...
    new_ = exp & (ERTS_NO_RUNQS_MASK << ERTS_NO_USED_RUNQS_SHIFT);
    new_ |= active & ERTS_NO_RUNQS_MASK;
    act = erts_smp_atomic32_cmpxchg_nob(&balance_info.no_runqs, new_, exp);
...
wake_scheduler_on_empty_runq(ErtsRunQueue *crq)
{
  ...
  if (active_ix < balance_ix) {
    /* Try to activate a new_ run queue and wake its scheduler */
...
dequeue_process(ErtsRunQueue *runq, int prio_q, erts_aint32_t *statep)
{
  ...
  if (p) {
    unqueue_process(runq, rpq, rqi, prio, nullptr, p);
  }
...
init_migration_paths(void) {
  ...
  for (qix = 0; qix < erts_no_run_queues; qix++) {
    ...
    mps->mpath[qix].misc_evac_runq = nullptr;

    for (pix = 0; pix < ERTS_NO_PRIO_LEVELS; pix++) {
      mps->mpath[qix].prio[pix].limit.this_ = -1;
      mps->mpath[qix].prio[pix].limit.other = -1;
      mps->mpath[qix].prio[pix].runq = nullptr;
...
retire_mpaths(ErtsMigrationPaths * mps) {
  ...
  if (!mpaths.retired.first) {
    mpaths.retired.last = nullptr;
  }

  mps->thr_prgr = erts_thr_progress_later(nullptr);
  mps->next = nullptr;
...
#  error check_balance() assumes ERTS_MAX_PROCESS < (1 << 27)
  ...
  get_no_runqs(nullptr, &blnc_no_rqs);
...
setup_migration_paths:
    ...
    for (pix = 0; pix < ERTS_NO_PRIO_LEVELS; pix++) {
      ...
      for (qix = 0; qix < blnc_no_rqs; qix++) {
        ...
        if (pix == 2) {
          erts_fprintf(stderr, "%d ", len_diff);
        }
  ...
  for (qix = 0; qix < blnc_no_rqs; qix++) {
    ...
    for (pix = 0; pix < ERTS_NO_PRIO_LEVELS; pix++) {
      if (!(ERTS_CHK_RUNQ_FLG_EMIGRATE(flags, pix)
        ...
        mp->prio[pix].limit.this_ = -1;
        mp->prio[pix].limit.other = -1;
        mp->prio[pix].runq = nullptr;
        mp->prio[pix].flags = 0;
  ...
  for (qix = blnc_no_rqs; qix < erts_no_schedulers; qix++) {
    ...
    for (pix = 0; pix < ERTS_NO_PRIO_LEVELS; pix++) {
      nmp->prio[pix].limit.this_ = omp->prio[pix].limit.this_;
...
change_no_used_runqs(int used) {
  ...
  for (qix = 0; qix < used; qix++) {
    ...
    nmp->misc_evac_runq = nullptr;

    for (pix = 0; pix < ERTS_NO_PRIO_LEVELS; pix++) {
      nmp->prio[pix].limit.this_ = -1;
      nmp->prio[pix].limit.other = -1;
      nmp->prio[pix].runq = nullptr;
...
#define ERTS_WAKEUP_OTHER_LIMIT_VERY_HIGH (200*vm::CONTEXT_REDS)
#define ERTS_WAKEUP_OTHER_LIMIT_HIGH (50*vm::CONTEXT_REDS)
#define ERTS_WAKEUP_OTHER_LIMIT_MEDIUM (10*vm::CONTEXT_REDS)
#define ERTS_WAKEUP_OTHER_LIMIT_LOW (vm::CONTEXT_REDS)
#define ERTS_WAKEUP_OTHER_LIMIT_VERY_LOW (vm::CONTEXT_REDS/10)
...
#define ERTS_WAKEUP_OTHER_FIXED_INC (vm::CONTEXT_REDS/10)

/* Legacy */

#define ERTS_WAKEUP_OTHER_LIMIT_VERY_HIGH_LEGACY (200*vm::CONTEXT_REDS)
#define ERTS_WAKEUP_OTHER_LIMIT_HIGH_LEGACY (50*vm::CONTEXT_REDS)
#define ERTS_WAKEUP_OTHER_LIMIT_MEDIUM_LEGACY (10*vm::CONTEXT_REDS)
#define ERTS_WAKEUP_OTHER_LIMIT_LOW_LEGACY (vm::CONTEXT_REDS)
#define ERTS_WAKEUP_OTHER_LIMIT_VERY_LOW_LEGACY (vm::CONTEXT_REDS/10)

#define ERTS_WAKEUP_OTHER_DEC_LEGACY 10
#define ERTS_WAKEUP_OTHER_FIXED_INC_LEGACY (vm::CONTEXT_REDS/10)
...
init_aux_work_data(ErtsAuxWorkData * awdp, ErtsSchedulerData * esdp, char *dawwp) {
  ...
  awdp->esdp = esdp;
  awdp->ssi = esdp ? esdp->ssi : nullptr;
#ifdef ERTS_SMP
...
#endif
  awdp->async_ready.queue = nullptr;
#endif
#ifdef ERTS_SMP
  awdp->delayed_wakeup.next = ERTS_DELAYED_WAKEUP_INFINITY;

  if (!dawwp) {
    awdp->delayed_wakeup.job = nullptr;
    awdp->delayed_wakeup.sched2jix = nullptr;
    awdp->delayed_wakeup.jix = -1;
  } else {
    int i;
...
init_scheduler_data(ErtsSchedulerData * esdp, int num,
  ...
  esdp->match_pseudo_process = nullptr;
  esdp->free_process = nullptr;
#endif
  esdp->x_reg_array = (Eterm *)erts_alloc_permanent_cache_aligned(ERTS_ALC_T_BEAM_REGISTER,
                      ERTS_X_REGS_ALLOCATED * sizeof(Eterm));
  esdp->f_reg_array = (FloatDef *)erts_alloc_permanent_cache_aligned(ERTS_ALC_T_BEAM_REGISTER,
                      vm::MAX_REG * sizeof(FloatDef));
#if !HEAP_ON_C_STACK
...
erts_init_scheduling(int no_schedulers, int no_schedulers_online
  ...
  erts_aligned_run_queues =
    (ErtsAlignedRunQueue *)erts_alloc_permanent_cache_aligned(ERTS_ALC_T_RUNQS, size_runqs);
#ifdef ERTS_SMP
#ifdef ERTS_DIRTY_SCHEDULERS
  ...
  for (ix = -(ERTS_NUM_DIRTY_RUNQS); ix < n; ix++) {
    ...
    rq->procs.pending_exiters = nullptr;
    ...
    for (pix = 0; pix < ERTS_NO_PROC_PRIO_LEVELS; pix++) {
      ...
      if (pix < ERTS_NO_PROC_PRIO_LEVELS - 1) {
        rq->procs.prio[pix].first = nullptr;
        rq->procs.prio[pix].last = nullptr;
      }
    }
    ...
    rq->ports.info.max_len = 0;
    rq->ports.info.reds = 0;
    rq->ports.start = nullptr;
    rq->ports.end = nullptr;
  ...
  aligned_sched_sleep_info =
    (ErtsAlignedSchedulerSleepInfo *)erts_alloc_permanent_cache_aligned(
  ...
  for (ix = 0; ix < no_ssi; ix++) {
    ...
    ssi->event = nullptr; /* initialized in sched_thread_func */
#endif
  ...
  for (ix = 0; ix < no_dirty_cpu_schedulers; ix++) {
    ...
    erts_smp_atomic32_init_nob(&ssi->flags, 0);
    ssi->event = nullptr; /* initialized in sched_dirty_cpu_thread_func */
    erts_atomic32_init_nob(&ssi->aux_work, 0);
  }
  ...
  for (ix = 0; ix < no_dirty_io_schedulers; ix++) {
    ...
    erts_smp_atomic32_init_nob(&ssi->flags, 0);
    ssi->event = nullptr; /* initialized in sched_dirty_io_thread_func */
    erts_atomic32_init_nob(&ssi->aux_work, 0);
  }
  ...
  erts_aligned_scheduler_data =
    (ErtsAlignedSchedulerData *)erts_alloc_permanent_cache_aligned(ERTS_ALC_T_SCHDLR_DATA,
  ...
  aux_thread_aux_work_data =
    (ErtsAuxWorkData *)erts_alloc_permanent_cache_aligned(ERTS_ALC_T_SCHDLR_DATA,
...
#endif
  schdlr_sspnd.msb.procs = nullptr;
  ...
  ERTS_VERIFY_UNUSED_TEMP_ALLOC(nullptr);
#endif
#endif
...
schedule_process(Process * p, erts_aint32_t in_state, ErtsProcLocks locks) {
  ...
  if (enqueue != ERTS_ENQUEUE_NOT)
    add2runq(enqueue > 0 ? p : make_proxy_proc(nullptr, p, enq_prio),
             state,
             enq_prio);
}
...
suspend_scheduler(ErtsSchedulerData * esdp) {
  ...
  erts_aint32_t aux_work;
  int thr_prgr_active = 1;
  ErtsStuckBoundProcesses sbp = {nullptr, nullptr};
  int *ss_onlinep;
  int *ss_curr_onlinep;
...
erts_is_multi_scheduling_blocked(void) {
  int res;
  erts_smp_mtx_lock(&schdlr_sspnd.mtx);
  res = schdlr_sspnd.msb.procs != nullptr;
  erts_smp_mtx_unlock(&schdlr_sspnd.mtx);
  return res;
}
...
sched_dirty_cpu_thread_func(void *vesdp) {
  ...
  callbacks.finalize_wait = nullptr;
  ...
  erts_tsd_set(sched_data_key, vesdp);
#if ERTS_USE_ASYNC_READY_Q
  esdp->aux_work_data.async_ready.queue = nullptr;
#endif
...
handle_pend_sync_suspend(Process * suspendee,
  ...
  if (suspender) {
    ...
    if (suspendee_alive) {
      erts_suspend(suspendee, suspendee_locks, nullptr);
...
pid2proc_not_running(Process * c_p, ErtsProcLocks c_p_locks,
  ...
  if (c_p->suspendee == pid) {
     ...
     * In this_ case we have to yield, be suspended by
 ...
 *   yield (ERTS_BIF_YIELD[0-3]()). c_p might in this_ case have been
 *   suspended.
 */
Process *
...
erts_pid2proc_suspend(Process * c_p, ErtsProcLocks c_p_locks,
  ...
  if (rp) {
    erts_suspend(rp, pid_locks, nullptr);
  }
...
suspend_process_2(BIF_ALIST_2) {
  ...
  if (asynchronous) {
    ...
    if (BIF_P->suspendee == BIF_ARG_1) {
       ...
       * we might deadlock. In this_ case we have to yield,
      ...
      if (do_bif_suspend_process(BIF_P, smon, suspendee)) {
         ...
         * will be resumed and this_ BIF will be called again.
...
erts_resume_processes(ErtsProcList * list) {
  ...
  while (plp) {
    ...
    proc = erts_pid2proc(nullptr, 0, plp->pid, ERTS_PROC_LOCK_STATUS);
...
erts_set_process_priority(Process * p, Eterm value) {
  ...
  switch (value) {
  case am_max:
    nprio = (erts_aint32_t) PRIORITY_MAX;
    break;

  case am_high:
    nprio = (erts_aint32_t) PRIORITY_HIGH;
    break;

  case am_normal:
    nprio = (erts_aint32_t) PRIORITY_NORMAL;
    break;

  case am_low:
    nprio = (erts_aint32_t) PRIORITY_LOW;
    break;
 ...
 * replaced by a new_ process. 'calls' is the number of reduction
 * steps the current process consumed.
 * schedule() returns the new_ process, and the new_ process'
...
Process *schedule(Process * p, int calls) {
  ...
  if (p != nullptr && DTRACE_ENABLED(process_unscheduled)) {
   ...
   */
  if (!p) { /* nullptr in the very first schedule() call */
...
check_activities_to_run: {
    ...
    if (erts_proclist_fetch(&pnd_xtrs, nullptr)) {
      rq->procs.pending_exiters = nullptr;
      erts_smp_runq_unlock(rq);
...
continue_check_activities_to_run_known_flags:
    ...
    if (RUNQ_READ_LEN(&rq->ports.info.len)) {
      ...
      if ((!erts_eager_check_io
         ...
         *
         * One could argue that this_ is a strange behavior. The
         * reason for doing it this_ way is that it is similar
...
pick_next_process: {
      ...
      while (1) {
        erts_aint32_t exp, new_, tmp;
        tmp = new_ = exp = state;
        new_ &= psflg_band_mask;
        ...
        if (!(state & (ERTS_PSFLG_RUNNING
          ...
          if (tmp != ERTS_PSFLG_SUSPENDED) {
            if (state & ERTS_PSFLG_ACTIVE_SYS) {
              new_ |= ERTS_PSFLG_RUNNING_SYS;
            } else {
              new_ |= ERTS_PSFLG_RUNNING;
            }
          }
        }

        state = erts_smp_atomic32_cmpxchg_relb(&p->state, new_, exp);
...
fetch_sys_task(Process * c_p, erts_aint32_t state, int *qmaskp, int *priop) {
  ErtsProcSysTaskQs *unused_qs = nullptr;
  int qbit, qmask;
...
erts_internal_request_system_task_3(BIF_ALIST_3) {
  ...
  ErtsProcSysTaskQs *stqs, *free_stqs = nullptr;
  ErtsProcSysTask *st = nullptr;
...
alloc_qs:
    ...
    stqs->qmask = 1 << prio;
    stqs->ncount = 0;
    stqs->q[PRIORITY_MAX] = nullptr;
    stqs->q[PRIORITY_HIGH] = nullptr;
    stqs->q[PRIORITY_NORMAL] = nullptr;
    stqs->q[PRIORITY_LOW] = nullptr;
    stqs->q[prio] = st;
  }
   ...
   * lock on process.
   */
  schedule_process_sys_task(rp, rp_state, nullptr);
...
save_gc_task(Process * c_p, ErtsProcSysTask * st, int prio) {
  ...
  if (!qs) {
    ...
    qs->qmask = 1 << prio;
    qs->ncount = 0;
    qs->q[PRIORITY_MAX] = nullptr;
    qs->q[PRIORITY_HIGH] = nullptr;
    qs->q[PRIORITY_NORMAL] = nullptr;
    qs->q[PRIORITY_LOW] = nullptr;
    qs->q[prio] = st;
...
erts_set_gc_state(Process * c_p, int enable) {
  ...
  (void) ERTS_PROC_SET_DELAYED_GC_TASK_QS(c_p, ERTS_PROC_LOCK_MAIN, nullptr);
...
erl_create_process(Process * parent, /* Parent of process (default group leader). */
  ...
  p = alloc_process(rq, state); /* All proc locks are locked by this_ thread
             on success */
  ...
  if (so->flags & SPO_USE_ARGS) {
    ...
    p->max_gen_gcs    = (uint16_t) erts_smp_atomic32_read_nob(&erts_max_gen_gcs);
  }
  ...
  p->old_hend = p->old_htop = p->old_heap = nullptr;
  p->high_water = p->heap;
  p->gen_gcs = 0;
...
#ifdef ERTS_SMP
  p->common.u.alive.ptimer = nullptr;
#else
  sys_memset(&p->common.u.alive.tm, 0, sizeof(ErlTimer));
#endif

  p->common.u.alive.reg = nullptr;
  ERTS_P_LINKS(p) = nullptr;
  ERTS_P_MONITORS(p) = nullptr;
  p->nodes_monitors = nullptr;
  p->suspend_monitors = nullptr;
  ...
  /*
   * Check if this_ process should be initially linked to its parent.
   */
  ...
  /*
   * Test whether this_ process should be initially monitored by its parent.
   */
  ...
  VERBOSE(DEBUG_PROCESSES, ("Created a new_ process: %T\n", p->common.id));
...
void erts_init_empty_process(Process * p) {
  ...
  p->psd = nullptr;
  ERTS_P_MONITORS(p) = nullptr;
  ERTS_P_LINKS(p) = nullptr;         /* List of links */
  p->nodes_monitors = nullptr;
  p->suspend_monitors = nullptr;
  p->msg.first = nullptr;
  p->msg.last = &p->msg.first;
...
erts_debug_verify_clean_empty_process(Process * p) {
  ...
  ASSERT(p->stop == nullptr);
  ASSERT(p->hend == nullptr);
  ASSERT(p->heap == nullptr);
  ...
  ASSERT(p->common.u.alive.reg == nullptr);
  ASSERT(p->heap_sz == 0);
  ASSERT(p->high_water == nullptr);
  ASSERT(p->old_hend == nullptr);
  ASSERT(p->old_htop == nullptr);
  ASSERT(p->old_heap == nullptr);

  ASSERT(ERTS_P_MONITORS(p) == nullptr);
  ASSERT(ERTS_P_LINKS(p) == nullptr);
  ASSERT(p->nodes_monitors == nullptr);
  ASSERT(p->suspend_monitors == nullptr);
  ASSERT(p->msg.first == nullptr);
  ASSERT(p->msg.len == 0);
  ASSERT(p->u.bif_timers == nullptr);
  ASSERT(p->dictionary == nullptr);
  ASSERT(p->catches == 0);
  ASSERT(p->cp == nullptr);
  ASSERT(p->i == nullptr);
...
#ifdef ERTS_SMP
  ASSERT(p->msg_inq.first == nullptr);
  ASSERT(p->msg_inq.len == 0);
  ASSERT(p->suspendee == NIL);
  ASSERT(p->pending_suspenders == nullptr);
  ASSERT(p->pending_exit.reason == THE_NON_VALUE);
  ASSERT(p->pending_exit.bp == nullptr);
#endif

  /* Thing that erts_cleanup_empty_process() cleans up */

  ASSERT(p->off_heap.first == nullptr);
  ASSERT(p->off_heap.overhead == 0);

  ASSERT(p->mbuf == nullptr);
}
...
delete_process(Process * p) {
   ...
   * we'll notice.
   */
  p->off_heap.first = (erl_off_heap_header *)((void *) 0x8DEFFACD);
...
set_proc_exiting(Process * p,
   ...
   * save the stack trace since this_ process irreversibly is going to
   * exit.
   */
...
handle_pending_exiters(ErtsProcList * pnd_xtrs) {
  ...
  while (plp) {
    ...
    Process *p = erts_pid2proc(nullptr, 0, plp->pid, ERTS_PROC_LOCKS_ALL);
...
save_pending_exiter(Process * p) {
  ...
  rq = erts_get_runq_current(nullptr);
...
send_exit_message(Process * to, ErtsProcLocks * to_locksp,
  ...
  if (token == NIL
    ...
    seq_trace_output(token, mess, SEQ_TRACE_SEND, to->common.id, nullptr);
 ...
 * process is to be exited and this_ is what is described below.
 *
 ...
 * schedules a new_ process will set the receiving process in the exiting
  ...
  if ((state & ERTS_PSFLG_TRAP_EXIT)
    ...
    if (!(state & (ERTS_PSFLG_EXITING | ERTS_PSFLG_PENDING_EXIT))) {
      ...
      if (rp == c_p && (*rp_locks & ERTS_PROC_LOCK_MAIN)) {
        ...
        set_proc_exiting(c_p, state, rsn, nullptr);
...
set_pending_exit:
     ...
     *    so we drop this_ signal.
     *
     *    NOTE: dropping this_ exit signal is based on the assumption
  ...
  if (mon->type == MON_ORIGIN) {
    ...
    if (is_atom(mon->pid)) { /* remote by name */
      ...
      if (dep) {
        ...
        if (rmon) {
          ErtsDSigData dsd;
          int code = erts_dsig_prepare(&dsd, dep, nullptr,
                                       ERTS_DSP_NO_LOCK, 0);
      ...
      if (is_internal_pid(mon->pid)) { /* local by pid or name */
        rp = erts_pid2proc(nullptr, 0, mon->pid, ERTS_PROC_LOCK_LINK);
    ...
    if (is_internal_port(mon->pid)) {
      ...
      rp = erts_pid2proc(nullptr, 0, mon->pid, rp_locks);
...
static void doit_exit_link(ErtsLink * lnk, void *vpcontext) {
  ...
  case LINK_PID:
    if (is_internal_port(item)) {
      ...
      ErtsProcLocks rp_locks = (ERTS_PROC_LOCK_LINK
                                | ERTS_PROC_LOCKS_XSIG_SEND);
      rp = erts_pid2proc(nullptr, 0, item, rp_locks);

      if (rp) {
        rlnk = erts_remove_link(&ERTS_P_LINKS(rp), p->common.id);

        /* If rlnk == nullptr, we got unlinked while exiting,
        i.e., do nothing... */
        if (rlnk) {
          int xres;
          erts_destroy_link(rlnk);
          xres = send_exit_signal(nullptr,
                                  p->common.id,
                                  rp,
                                  &rp_locks,
                                  reason,
...
/* this_ function fishishes a process and propagates exit messages - called
...
erts_do_exit_process(Process * p, Eterm reason) {
  ...
  /*
   * The p->u.bif_timers of this_ process can *not* be used anymore;
   * will be overwritten by misc termination data.
   */
  p->u.terminate = nullptr;
...
erts_continue_exit_process(Process * p) {
  ...
  if (state & ERTS_PSFLG_ACTIVE_SYS) {
    if (cleanup_sys_tasks(p, state, vm::CONTEXT_REDS) >= vm::CONTEXT_REDS / 2) {
      goto yield;
    }
  }
  ...
  if (p->common.u.alive.reg) {
    (void) erts_unregister_name(p, ERTS_PROC_LOCK_MAIN, nullptr, THE_NON_VALUE);
  ...
  /*
   * From this_ point on we are no longer allowed to yield
   * this_ process.
   */
#ifdef DEBUG
  yield_allowed = 0;
...

  {
    ...
    ASSERT(p->scheduler_data->free_process == nullptr);

    p->scheduler_data->current_process = nullptr;
  ...
  /*
   * All "erlang resources" have to be deallocated before this_ point,
  ...
  dep = ((p->flags & F_DISTRIBUTION)
         ? ERTS_PROC_SET_DIST_ENTRY(p, ERTS_PROC_LOCKS_ALL, nullptr)
         : nullptr);
  scb = ERTS_PROC_SET_SAVED_CALLS_BUF(p, ERTS_PROC_LOCKS_ALL, nullptr);
  pbt = ERTS_PROC_SET_CALL_TIME(p, ERTS_PROC_LOCKS_ALL, nullptr);
  nif_export = ERTS_PROC_SET_NIF_TRAP_EXPORT(p, ERTS_PROC_LOCKS_ALL, nullptr);
 ...
 * 2) All shedulers detect this_ and set the flag halt_in_progress
 ...
 *
 * A BIF that calls this_ should make sure to schedule out to never come back:
 *    erl_halt((int)(- code));
