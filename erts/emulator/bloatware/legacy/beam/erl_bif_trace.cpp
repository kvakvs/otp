...
erts_trace_flag2bit(Eterm flag)
{
  ...
  case am_receive:
    return F_TRACE_RECEIVE;

  case am_set_on_spawn:
    return F_TRACE_SOS;

  case am_procs:
    return F_TRACE_PROCS;

  case am_set_on_first_spawn:
    return F_TRACE_SOS1;

  case am_set_on_link:
    return F_TRACE_SOL;

  case am_set_on_first_link:
    return F_TRACE_SOL1;
  ...
  case am_running:
    return F_TRACE_SCHED;

  case am_exiting:
    return F_TRACE_SCHED_EXIT;

  case am_garbage_collection:
    return F_TRACE_GC;

  case am_call:
    return  F_TRACE_CALLS;

  case am_arity:
    return F_TRACE_ARITY_ONLY;

  case am_return_to:
    return F_TRACE_RETURN_TO;

  case am_silent:
    return F_TRACE_SILENT;

  case am_scheduler_id:
    return F_TRACE_SCHED_NO;

  case am_running_ports:
    return F_TRACE_SCHED_PORTS;

  case am_running_procs:
    return F_TRACE_SCHED_PROCS;

  case am_ports:
    return F_TRACE_PORTS;
                 ...
                 Uint *pMask, Eterm *pTracer, int *pCpuTimestamp)
{
  ...
  if (pTracer && tracer != NIL) {
    *pTracer       = tracer;
  }

  if (pCpuTimestamp && cpu_timestamp) {
    *pCpuTimestamp = cpu_timestamp;
  }
        ...
        if (on) {
          if (!erts_cpu_timestamp) {
            ...
            /*
               ...
               on a different machine than this_. We check
    ...
    if (pid_spec == am_all || pid_spec == am_existing) {
      ...
      if (ports || mods) {
        ...
        for (i = 0; i < max; i++) {
          ...
          /* This could be a reason to modify this_ function or make a new_ one. */
        }
      }
    }
  ...
  if ((ERTS_TRACE_FLAGS(tracee_port) & TRACEE_FLAGS)
    ...
    if (is_internal_port(ERTS_TRACER_PROC(tracee_port))) {
      if (!erts_is_valid_tracer_port(ERTS_TRACER_PROC(tracee_port))) {
         ...
         * - discard it and approve the new_. */
        goto remove_tracer;
      } else {
        return 1;
      }
...
#if defined(DEBUG)
    /*
     * Check num_flags if this_ assertion fires.
     */
                              ...
                              Eterm   *call_time)       /* out */
{
  ...
  /* First look for an export_ entry */
  e.code[0] = mfa[0];
  e.code[1] = mfa[1];
  e.code[2] = mfa[2];
...
trace_info_func(Process *p, Eterm func_spec, Eterm key)
{
  ...
  case am_all: {
    ...
    hp += 2;
    t = TUPLE2(hp, am_call_time, ct);
    hp += 3;
    retval = CONS(hp, t, retval);
    hp += 2;
    t = TUPLE2(hp, am_meta_match_spec, match_spec_meta);
    hp += 3;
    retval = CONS(hp, t, retval);
    hp += 2;
    t = TUPLE2(hp, am_meta, meta);
    hp += 3;
    retval = CONS(hp, t, retval);
    hp += 2;
    t = TUPLE2(hp, am_match_spec, match_spec);
    hp += 3;
    retval = CONS(hp, t, retval);
    hp += 2;
    t = TUPLE2(hp, am_traced, traced);
    hp += 3;
    retval = CONS(hp, t, retval);
    hp += 2;
  }
...
trace_info_on_load(Process *p, Eterm key)
{
  ...
  case am_all: {
    ...
    r = CONS(hp, t, r);
    hp += 2;
    t = TUPLE2(hp, am_meta_match_spec, meta_match_spec);
    hp += 3;
    r = CONS(hp, t, r);
    hp += 2;
    t = TUPLE2(hp, am_meta,
               (erts_default_trace_pattern_flags.meta
                ? erts_default_meta_tracer_pid : am_false));
    hp += 3;
    r = CONS(hp, t, r);
    hp += 2;
    t = TUPLE2(hp, am_match_spec, match_spec);
...
erts_finish_breakpointing(void)
{
  ...
  case 0:
    ...
    /*
     * At this_ point, in all functions that are to be breakpointed,
     * a pointer to a GenericBp struct has already been added,
     *
     * Insert the new_ breakpoints (if any) into the
...
static Eterm system_monitor_get(Process *p)
{
  ...
  if (system_monitor == NIL) {
    ...
    if (long_gc != NIL) {
      Eterm t = TUPLE2(hp, am_long_gc, long_gc);
      hp += 3;
      res = CONS(hp, t, res);
      hp += 2;
    }
    ...
    if (long_schedule != NIL) {
      Eterm t = TUPLE2(hp, am_long_schedule, long_schedule);
      hp += 3;
      res = CONS(hp, t, res);
      hp += 2;
    }
    ...
    if (large_heap != NIL) {
      Eterm t = TUPLE2(hp, am_large_heap, large_heap);
      hp += 3;
      res = CONS(hp, t, res);
      hp += 2;
    }
    ...
    if (erts_system_monitor_flags.busy_port) {
      res = CONS(hp, am_busy_port, res);
      hp += 2;
    }

    if (erts_system_monitor_flags.busy_dist_port) {
      res = CONS(hp, am_busy_dist_port, res);
      hp += 2;
    }
    ...
    for (long_gc = 0, long_schedule = 0, large_heap = 0,
      ...
      if (is_tuple(t)) {
        ...
        if (tp[1] == am_long_gc) {
          ...
          if (! term_to_Uint(tp[2], &long_schedule)) {
            goto error;
          }
          ...
          if (! term_to_Uint(tp[2], &large_heap)) {
            goto error;
          }
...
static Eterm system_profile_get(Process *p)
{
  ...
  if (system_profile == NIL) {
    ...
    if (erts_system_profile_flags.runnable_ports) {
      res = CONS(hp, am_runnable_ports, res);
      hp += 2;
    }

    if (erts_system_profile_flags.runnable_procs) {
      res = CONS(hp, am_runnable_procs, res);
      hp += 2;
    }

    if (erts_system_profile_flags.scheduler) {
      res = CONS(hp, am_scheduler, res);
      hp += 2;
    }

    if (erts_system_profile_flags.exclusive) {
      res = CONS(hp, am_exclusive, res);
      hp += 2;
    }
