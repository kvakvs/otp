...
static Export *alloc_info_trap = nullptr;
static Export *alloc_sizes_trap = nullptr;
...
{
  auto pmlc = (MonListContext *)vpmlc;
  Eterm tup;
...
static void do_calc_lnk_size(ErtsLink *lnk, void *vpsz)
{
  ...
  *psz += IS_CONST(lnk->pid) ? 0 : NC_HEAP_SIZE(lnk->pid);
  ...
  if (lnk->type != LINK_NODE && ERTS_LINK_ROOT(lnk) != nullptr) {
    /* Node links use this_ pointer as ref counter... */
...
{
  auto pllc = (LnkListContext *)vpllc;
  Eterm tup;
  ...
  } else if (ERTS_LINK_ROOT(lnk) != nullptr) {
    old_res = pllc->res;
    pllc->res = NIL;
...
#else
  (void) erts_schedulers_state(&total, &online, &active, nullptr, nullptr, nullptr, 0);
#endif
#endif
...
do {                  \
    if ((MICP)->mi_i >= (MICP)->mi_max) {       \
        (MICP)->mi = ((MICP)->mi ? (MonitorInfo*)erts_realloc(ERTS_ALC_T_TMP,   \
            (MICP)->mi,   \
            ((MICP)->mi_max+MI_INC) \
            * sizeof(MonitorInfo))  \
                      : (MonitorInfo*)erts_alloc(ERTS_ALC_T_TMP,      \
...
{
  auto micp = (MonitorInfoCollection *)vmicp;
...
do {                  \
  ...
  (SMICP)->smi = ((SMICP)->smi          \
                        ? (ErtsSuspendMonitor **)erts_realloc(ERTS_ALC_T_TMP,     \
               (SMICP)->smi,      \
               ((SMICP)->smi_max    \
          + ERTS_SMI_INC)     \
               * sizeof(ErtsSuspendMonitor *))  \
                        : (ErtsSuspendMonitor **)erts_alloc(ERTS_ALC_T_TMP,     \
             ERTS_SMI_INC     \
             * sizeof(ErtsSuspendMonitor *)));  \
...
{
  auto smicp = (ErtsSuspendMonitorInfoCollection *)vsmicp;
  ...
  while (is_list(list)) {
    ...
    if (res_elem_ix_ix >= res_elem_ix_sz) {
      if (res_elem_ix != &def_res_elem_ix_buf[0])
        res_elem_ix =
          (int *)erts_realloc(ERTS_ALC_T_TMP,
                              res_elem_ix,
                 ...
                 int always_wrap)
{
  ...
  /*
   * Q: Why this_ always_wrap argument?
   *
        ...
        if (rp != BIF_P) {
          if (is_value(msg)) {
            ...
            msg = erts_msg_distext2heap(
                    nullptr, nullptr, &hfp, &ERL_MESSAGE_TOKEN(mq[i].msgp),
  ...
  case am_garbage_collection: {
    ...
    t = TUPLE2(hp, am_min_heap_size, make_small(MIN_HEAP_SIZE(rp)));
    hp += 3;
    res = CONS(hp, t, res);
    hp += 2;
    t = TUPLE2(hp, am_min_bin_vheap_size, make_small(MIN_VHEAP_SIZE(rp)));
    hp += 3;
    res = CONS(hp, t, res);
    hp += 2;
    break;
...
BIF_RETTYPE system_info_1(BIF_ALIST_1)
{
  ...
  if (is_tuple(BIF_ARG_1)) {
    ...
    res = erts_allocated_areas(nullptr, nullptr, BIF_P);
    BIF_RET(res);
    ...
    tup = TUPLE2(hp, am_min_heap_size, make_small(H_MIN_SIZE));
    hp += 3;
    res = CONS(hp, tup, res);
    hp += 2;

    tup = TUPLE2(hp, am_min_bin_vheap_size, make_small(BIN_VH_MIN_SIZE));
    hp += 3;
    res = CONS(hp, tup, res);
    hp += 2;
...
BIF_RETTYPE is_process_alive_1(BIF_ALIST_1)
{
  if (is_internal_pid(BIF_ARG_1)) {
    ...
    if (external_pid_dist_entry(BIF_ARG_1) == erts_this_dist_entry) {
      BIF_RET(am_false);  /* A pid from an old incarnation of this_ node */
    } else {
      BIF_ERROR(BIF_P, BADARG);
    }
  } else {
...
BIF_RETTYPE process_display_2(BIF_ALIST_2)
{
  ...
  erts_stack_dump(ERTS_PRINT_STDERR, nullptr, rp);
#ifdef ERTS_SMP
...
/* this_ is a general call which return some possibly useful information */
...
BIF_RETTYPE statistics_1(BIF_ALIST_1)
{
  ...
  if (BIF_ARG_1 == am_scheduler_wall_time) {
    ...
    res = erts_run_queues_len(nullptr);
...
BIF_RETTYPE erts_debug_get_internal_state_1(BIF_ALIST_1)
{
  ...
  if (is_atom(BIF_ARG_1)) {
    if (ERTS_IS_ATOM_STR("reds_left", BIF_ARG_1)) {
      ...
      erts_smp_thr_progress_block();
      erts_smp_proc_lock(BIF_P, ERTS_PROC_LOCK_MAIN);
      res = erts_memory(nullptr, nullptr, BIF_P, THE_NON_VALUE);
      erts_smp_thr_progress_unblock();
      BIF_RET(res);
      ...
      if (ERTS_IS_ATOM_STR("process_status", tp[1])) {
        ...
        if (is_binary(bin)) {
          ...
          if (*binary_val(real_bin) == HEADER_PROC_BIN) {
            ...
            SzTerm = erts_bld_uint(&hp, nullptr, pb->size);
...
#endif
          xres = erts_send_exit_signal(nullptr, /* nullptr in order to
                                        ...
                                        Eterm res)
{
  ...
  af    = erts_atom_put((uint8_t *)stats->file, strlen(stats->file), ERTS_ATOM_ENC_LATIN1, 1);
...
static Eterm lcnt_build_lock_term(Eterm **hpp, size_t *szp, erts_lcnt_lock_t *lock, Eterm res)
{
  ...
  type  = erts_atom_put((uint8_t *)ltype, strlen(ltype), ERTS_ATOM_ENC_LATIN1, 1);
  name  = erts_atom_put((uint8_t *)lock->name, strlen(lock->name), ERTS_ATOM_ENC_LATIN1, 1);

  if (lock->flag & ERTS_LCNT_LT_ALLOC) {
    /* use allocator types names as id's for allocator locks */
    ltype = (char *) ERTS_ALC_A2AD(signed_val(lock->id));
    id    = erts_atom_put((uint8_t *)ltype, strlen(ltype), ERTS_ATOM_ENC_LATIN1, 1);
...
static Eterm lcnt_build_result_term(Eterm **hpp, size_t *szp, erts_lcnt_data_t *data, Eterm res)
{
  ...
  adur = erts_atom_put((uint8_t *)str_duration, strlen(str_duration), ERTS_ATOM_ENC_LATIN1, 1);
  tdur = erts_bld_tuple(hpp, szp, 2, adur, tdt);

  /* lock tuple */

  aloc = erts_atom_put((uint8_t *)str_locks, strlen(str_locks), ERTS_ATOM_ENC_LATIN1, 1);

  for (lock = data->current_locks->head; lock != nullptr ; lock = lock->next) {
    lloc = lcnt_build_lock_term(hpp, szp, lock, lloc);
  }

  for (lock = data->deleted_locks->head; lock != nullptr ; lock = lock->next) {
    lloc = lcnt_build_lock_term(hpp, szp, lock, lloc);
  }
...
BIF_RETTYPE erts_debug_lock_counters_1(BIF_ALIST_1)
{
  ...
  if (BIF_ARG_1 == am_enabled) {
    ...
    szp = &hsize;
    lcnt_build_result_term(nullptr, szp, data, NIL);
...
{
  Eterm type = erts_atom_put((uint8_t *) os_type, strlen(os_type), ERTS_ATOM_ENC_LATIN1, 1);
  Eterm flav;
  int major, minor, build;
  char *buf = (char *)erts_alloc(ERTS_ALC_T_TMP, 1024); /* More than enough */
  Eterm *hp;

  os_flavor(buf, 1024);
  flav = erts_atom_put((uint8_t *) buf, strlen(buf), ERTS_ATOM_ENC_LATIN1, 1);
  erts_free(ERTS_ALC_T_TMP, (void *) buf);
  hp = (Eterm *)erts_alloc(ERTS_ALC_T_LL_TEMP_TERM, (3 + 4) * sizeof(Eterm));
