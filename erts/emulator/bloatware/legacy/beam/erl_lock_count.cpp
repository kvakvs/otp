...
static char *lcnt_lock_type(Uint16 flag)
{
  ...
  case ERTS_LCNT_LT_SPINLOCK:
    return "spinlock";

  case ERTS_LCNT_LT_RWSPINLOCK:
    return "rw_spinlock";

  case ERTS_LCNT_LT_MUTEX:
    return "mutex";

  case ERTS_LCNT_LT_RWMUTEX:
    return "rw_mutex";

  case ERTS_LCNT_LT_PROCLOCK:
    return "proclock";
...
{
  if ((flag & ERTS_LCNT_LO_WRITE) && (flag & ERTS_LCNT_LO_READ)) {
    return "rw";
  }

  if (flag & ERTS_LCNT_LO_READ) {
    return "r ";
  }

  if (flag & ERTS_LCNT_LO_WRITE) {
    return " w";
  }
...
static void print_lock_x(erts_lcnt_lock_t *lock, Uint16 flag, char *action)
{
  ...
  if (strcmp(lock->name, "run_queue") != 0) {
    return;
  }
...
static erts_lcnt_lock_stats_t *lcnt_get_lock_stats(erts_lcnt_lock_t *lock, char *file,
    unsigned int line)
{
  unsigned int i;
...
void erts_lcnt_init()
{
  ...
  /* init lock */
  if (ethr_mutex_init(&lcnt_data_lock) != 0) {
    abort();
  }
...
void erts_lcnt_lock_opt(erts_lcnt_lock_t *lock, Uint16 option)
{
  ...
  if (erts_lcnt_rt_options & ERTS_LCNT_OPT_SUSPEND) {
    return;
  }
