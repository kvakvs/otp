  ...
  if (is_list(item) || is_nil(item)) {
    ...
    *hp++ = make_arityval(size);
    ...
    while (is_list(item)) {
      ...
      while (idx > 0 && (c = CMP_TERM(kv[1], ks[idx - 1])) < 0) {
        idx--;
      }
...
Eterm erts_maps_put(Process *p, Eterm key, Eterm value, Eterm map)
{
  ...
  *hp++  = value;
   ...
   * this_ will work out fine once we get the size word
   * in the header.
   */
