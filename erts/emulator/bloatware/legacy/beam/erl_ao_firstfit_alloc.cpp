...
/* Set new_ possibly lower 'max_sz' of node and propagate change toward root
*/
...
static Eterm info_options(Allctr_t *, const char *, int *, void *, size_t **, size_t *);
...
aoff_unlink_free_block(Allctr_t *allctr, Block_t *blk)
{
  ...
  /* Update the carrier tree with a potentially new_ (lower) max_sz
   */
  if (crr->root) {
...
aoff_link_free_block(Allctr_t *allctr, Block_t *block)
{
  ...
  /* Update the carrier tree with a potentially new_ (larger) max_sz
  */
             ...
             size_t *szp)
{
  ...
  if (hpp || szp) {
    ...
    if (!atoms_initialized)
      erl::exit(1, "%s:%d: Internal error: Atoms not initialized",
               __FILE__, __LINE__);;
...
erts_aoffalc_test(UWord op, UWord a1, UWord a2)
{
  switch (op) {
  case 0x500:
    return (UWord)((AOFFAllctr_t *) a1)->flavor == AOFF_AOBF;
  ...
  case 0x502:
    return (UWord)((AOFF_RBTree_t *) a1)->parent;

  case 0x503:
    return (UWord)((AOFF_RBTree_t *) a1)->left;

  case 0x504:
    return (UWord)((AOFF_RBTree_t *) a1)->right;

  case 0x505:
    return (UWord) LIST_NEXT(a1);

  case 0x506:
    return (UWord) IS_BLACK((AOFF_RBTree_t *) a1);

  case 0x507:
    return (UWord) IS_TREE_NODE((AOFF_RBTree_t *) a1);

  case 0x508:
    return (UWord) 0; /* IS_BF_ALGO */

  case 0x509:
    return (UWord)((AOFF_RBTree_t *) a1)->max_sz;

  case 0x50a:
    return (UWord)((AOFFAllctr_t *) a1)->flavor == AOFF_BF;
