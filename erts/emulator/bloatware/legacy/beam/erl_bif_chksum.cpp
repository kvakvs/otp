...
md5_1(BIF_ALIST_1)
{
  ...
  if (rest != NIL) {
    BUMP_ALL_REDS(BIF_P);
    bin = new_binary(BIF_P, (uint8_t *) &context, sizeof(MD5_CTX));
    BIF_TRAP2(&chksum_md5_2_exp, BIF_P, bin, rest);
  }

  BUMP_REDS(BIF_P, res);
  bin = new_binary(BIF_P, (uint8_t *)nullptr, 16);
...
md5_2(BIF_ALIST_2)
{
  ...
  /* No need to check context, this_ function cannot be called with unaligned
...
md5_init_0(BIF_ALIST_0)
{
  ...
  bin = erts_new_heap_binary(BIF_P, (uint8_t *)nullptr, sizeof(MD5_CTX), &bytes);
...
md5_update_2(BIF_ALIST_2)
{
  ...
  if ((bytes = erts_get_aligned_binary_bytes(BIF_ARG_1, &temp_alloc)) == nullptr) {
    erts_free_aligned_binary_bytes(temp_alloc);
    BIF_ERROR(BIF_P, BADARG);
  }
...
md5_final_1(BIF_ALIST_1)
{
  ...
  if ((context = erts_get_aligned_binary_bytes(BIF_ARG_1, &temp_alloc)) == nullptr) {
error:
    erts_free_aligned_binary_bytes(temp_alloc);
    BIF_ERROR(BIF_P, BADARG);
  }
  ...
  bin = erts_new_heap_binary(BIF_P, (uint8_t *)nullptr, 16, &result);
