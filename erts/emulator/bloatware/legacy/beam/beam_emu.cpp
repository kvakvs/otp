 ...
 * This is safe to do, since this_ space is only activly used when
...
void process_main(void)
{
  ...
  /*
   * Note: In this_ function, we attempt to place rarely executed code towards
    ...
    OpCase(catch_end_y): {
      ...
      if (is_non_value(r(0))) {
        if (x(1) == am_throw) {
          ...
          if (E - HTOP < 3 || c_p->mbuf) {
            /* Force GC in case add_stacktrace()
...
loop_rec__:
      ...
      if (is_non_value(ERL_MESSAGE_TERM(msgp))) {
        ...
        /* TODO: Add DTrace probe for this_ bad message situation? */
    ...
    OpCase(i_wait_timeout_locked_fs): {
      ...
      if (timeout_value != make_small(0)) {
        ...
        if (is_small(timeout_value) && signed_val(timeout_value) > 0 &&
           ...
           * c_p->def_arg_reg[0].  Note that it is safe to use this_
        ...
        /*
         * Prepare to wait indefinitely for a new_ message to arrive
         * (or the time set above if falling through from above).
         *
         * When a new_ message arrives, control will be transferred
...
do_put_tuple: {
    ...
    OpCase(i_has_map_fields_fsI): {
      ...
      /* this_ instruction assumes Arg1 is a map,
...
#undef PUT_TERM_REG
    ...
    OpCase(bif1_fbsd): {
      typedef Eterm(*bf_t)(Process *, Eterm *);
      bf_t bf;
      Eterm tmp_reg[1];
      Eterm result;
    ...
    OpCase(bif1_body_bsd): {
      ...
      reg[0] = tmp_reg[0];
      SWAPOUT;
      I = handle_error(c_p, I, reg, (BifFunction)bf);
...
big_shift:
        ...
        if (bignum_header_is_neg(*big_val(tmp_arg2))) {
           ...
           * the sign of N. Since we don't believe this_ case
...
context_switch2:   /* Entry for fun calls. */
...

    {
      ...
      if (c_p->arity > c_p->max_arg_reg) {
        /*
         * Yes, this_ is an expensive operation, but you only pay it the first
    ...
    OpCase(normal_exit): {
      ...
      c_p->arity = 0;    /* In case this_ process will ever be garbed again. */
      ERTS_SMP_UNREQ_PROC_MAIN_LOCK(c_p);
...
find_func_info: {
  ...
  /*
   * At this_ point, I points to the code[3] in the export_ entry for
...
do_case_end:
    ...
    /*
     * Construction of binaries using new_ instructions.
     */
    {
      Eterm new_binary;
      Eterm num_bits_term;
...
do_bs_init_bits_known:
      ...
      if (num_bytes <= ERL_ONHEAP_BIN_LIMIT) {
        ...
        hb->size = num_bytes;
        erts_current_bin = (uint8_t *) hb->data;
...
store_bs_add_result:
          ...
          if (MY_IS_SSMALL((ssize_t) tmp_arg1)) {
            ...
            /*
             * May generate a heap fragment, but in this_
    ...
    OpCase(bs_put_string_II): {
      BeamInstr *next;
      PreFetch(2, next);
      erts_new_bs_put_string(ERL_BITS_ARGS_2((uint8_t *) Arg(1), Arg(0)));
      NextPF(2, next);
    }
...
do_bs_get_utf16: {
...

    {
      ...
      /* Unfortunately, inlining can generate this_ instruction. */
...
L_hipe_mode_switch:
      /* XXX: this_ abuse of def_arg_reg[] is horrid! */
      SWAPOUT;
      c_p->fcalls = FCALLS;
...
init_emulator: {
      ...
      /*
       * Enter all BIFs into the export_ table.
       */
...
/*
 * Mapping from the error code 'class_ tag' to atoms.
 */
 ...
 *
 * When a new_ exception is raised, the current stack trace information
...
handle_error(Process *c_p, BeamInstr *pc, Eterm *reg, BifFunction bf)
{
  ...
  /*
   * Check if we have an arglist for the top level call. If so, this_
   ...
   * main reason for doing this_ separately is to allow throws to later
  ...
  if ((c_p->catches > 0 || IS_TRACED_FL(c_p, F_EXCEPTION_TRACE))
    ...
    exception class_, term and trace, respectively. (If the
    ...
    if (is_catch(*ptr)) {
      ...
      if (*cp_val(*prev) == i_return_trace) {
        ...
        while (++ptr, ptr < STACK_START(c_p) && is_not_CP(*ptr)) {
          if (is_catch(*ptr) && active_catches) {
            goto found_catch;
          }
        }
...
{
  /* Add a stacktrace if this_ is an error. */
...
apply_setup_error_handler(Process *p, Eterm module, Eterm function, size_t arity, Eterm *reg)
{
  ...
  if ((ep = erts_active_export_entry(erts_proc_get_error_handler(p),
    ...
    /*
     * Always copy args from registers to a new_ list; this_ ensures
     * that we have the same behaviour whether or not this_ was
...
error2:
   ...
   * (currently implemented using tuples, but this_ might change).
   */
  this_ = THE_NON_VALUE;
  ...
  /*
   * Get the index into the export_ table, or failing that the export_
...
error2:
  ...
  /*
   * At this_ point, arguments are known to be good.
   */
  ...
  if (is_fun_header(hdr)) {
    ...
    if (actual_arity == arity + num_free) {
      ...
      if (actual_arity >= 0) {
        ...
        /*
         * No current code for this_ module. Call the error_handler module
...
update_map_assoc(Process *p, Eterm *reg, Eterm map, BeamInstr *I)
{
  ...
  /*
   * If the old map is empty, create a new_ map.
   */
  ...
  for (;;) {
    ...
    } else {    /* Replace or insert new_ */
      GET_TERM(new_p[1], *hp++);

      if (c > 0) {  /* If new_ new_ key */
        *kp++ = new_key;
      } else {    /* If replacement */
        *kp++ = key;
        old_keys++, old_vals++, num_old--;
  ...
  /*
   * At this_ point, we have run out of either old keys and values,
  ...
  if (n > 0) {
     ...
     * are still new_ keys and values in the update list that
     * must be copied.
     */
    ASSERT(num_old == 0);
