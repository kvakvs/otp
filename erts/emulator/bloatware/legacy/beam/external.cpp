...
/* Does ssize_t fit in int32_t?
 */
#define IS_SSMALL32(x) (((size_t) (((x) >> (32-1)) + 1)) < 2)
...
static uint8_t *enc_term(ErtsAtomCacheMap *, Eterm, uint8_t *, uint32_t,
                         struct erl_off_heap_header **off_heap);
struct TTBEncodeContext_;
static int enc_term_int(struct TTBEncodeContext_ *, ErtsAtomCacheMap *acmp, Eterm obj, uint8_t *ep,
                        uint32_t dflags,
                        struct erl_off_heap_header **off_heap, ssize_t *reds, uint8_t **res);
static size_t is_external_string(Eterm obj, int *p_is_string);
...
struct B2TContext_t;
static uint8_t *dec_term(ErtsDistExternal *, Eterm **, uint8_t *, ErlOffHeap *, Eterm *,
                         struct B2TContext_t *);
static uint8_t *dec_atom(ErtsDistExternal *, uint8_t *, Eterm *);
static uint8_t *dec_pid(ErtsDistExternal *, Eterm **, uint8_t *, ErlOffHeap *, Eterm *);
static ssize_t decoded_size(uint8_t *ep, uint8_t *endp, int internal_tags, struct B2TContext_t *);
static BIF_RETTYPE term_to_binary_trap_1(BIF_ALIST_1);

static Eterm erts_term_to_binary_int(Process *p, Eterm Term, int level, size_t flags,
                                     Binary *context_b);

static size_t encode_size_struct2(ErtsAtomCacheMap *, Eterm, unsigned);
struct TTBSizeContext_;
static int encode_size_struct_int(struct TTBSizeContext_ *, ErtsAtomCacheMap *acmp, Eterm obj,
                                  unsigned dflags, ssize_t *reds, size_t *res);
...
insert_acache_map(ErtsAtomCacheMap *acmp, Eterm atom, uint32_t dflags)
{
   ...
   * and this_ atom cannot be represented in latin1,
   * we are not allowed to cache it.
   *
   * In this_ case all atoms are assumed to have
...
erts_finalize_atom_cache_map(ErtsAtomCacheMap *acmp, uint32_t dflags)
{
  if (acmp) {
    ...
    for (i = 0; i < acmp->sz; i++) {
      ...
      /* Enough for a new_ atom cache value */
...
uint8_t *erts_encode_ext_dist_header_setup(uint8_t *ctl_ext, ErtsAtomCacheMap *acmp)
{
  ...
  if (!acmp) {
    ...
    int i;
    uint8_t *ep = ctl_ext;
    uint8_t dist_hdr_flags = acmp->long_atoms ? ERTS_DIST_HDR_LONG_ATOMS_FLG : 0;
    ASSERT(acmp->hdr_sz >= 0);

    /*
     * Write cache update instructions. Note that this_ is a purely
...
uint8_t *erts_encode_ext_dist_header_finalize(uint8_t *ext, ErtsAtomCache *cache, uint32_t dflags)
{
  ...
  if (ci > 0) {
    ...
    while (iix >= 0) {
      ...
      atom = make_atom((size_t) get_int32(&ip[2]));
    ...
    while (1) {
      ...
      if (flgs_bytes > 4) {
        *--ep = (uint8_t)((flgs >> 24) & 0xff);
        *--ep = (uint8_t)((flgs >> 16) & 0xff);
        *--ep = (uint8_t)((flgs >> 8) & 0xff);
        *--ep = (uint8_t)(flgs & 0xff);
        flgs_buf_ix++;
        flgs_bytes -= 4;
      } else {
        switch (flgs_bytes) {
...
size_t erts_encode_dist_ext_size(Eterm term, uint32_t flags, ErtsAtomCacheMap *acmp)
{
  size_t sz = 0;
...
size_t erts_encode_ext_size(Eterm term)
{
  return encode_size_struct2(nullptr, term, TERM_TO_BINARY_DFLAGS)
         + 1 /* VERSION_MAGIC */;
}

size_t erts_encode_ext_size_2(Eterm term, unsigned dflags)
{
  return encode_size_struct2(nullptr, term, dflags)
         + 1 /* VERSION_MAGIC */;
}

size_t erts_encode_ext_size_ets(Eterm term)
{
  return encode_size_struct2(nullptr, term, TERM_TO_BINARY_DFLAGS | DFLAG_INTERNAL_TAGS);
}
...
void erts_encode_ext(Eterm term, uint8_t **ext)
{
  uint8_t *ep = *ext;
  *ep++ = VERSION_MAGIC;
...
uint8_t *erts_encode_ext_ets(Eterm term, uint8_t *ep, struct erl_off_heap_header **off_heap)
{
  return enc_term(nullptr, term, ep, TERM_TO_BINARY_DFLAGS | DFLAG_INTERNAL_TAGS,
                  off_heap);
}

ErtsDistExternal *
erts_make_dist_ext_copy(ErtsDistExternal *edep, size_t xsize)
{
  size_t align_sz;
  size_t dist_ext_sz;
  size_t ext_sz;
...
#define CHKSIZE(SZ) \
  do { if ((SZ) > edep->ext_endp - ep) ERTS_EXT_HDR_FAIL; } while(0)
    ...
    if (no_atoms) {
      ...
      if (flgsp[byte_ix] & (((uint8_t) ERTS_DIST_HDR_LONG_ATOMS_FLG) << bit_ix)) {
        long_atoms = 1;
      }
...
erts_decode_dist_ext_size(ErtsDistExternal *edep)
{
  ...
  res = decoded_size(ep, edep->ext_endp, 0, nullptr);
...
ssize_t erts_decode_ext_size(uint8_t *ext, size_t size)
{
  if (size == 0 || *ext != VERSION_MAGIC) {
    return -1;
  }

  return decoded_size(ext + 1, ext + size, 0, nullptr);
}

ssize_t erts_decode_ext_size_ets(uint8_t *ext, size_t size)
{
  ssize_t sz = decoded_size(ext, ext + size, 1, nullptr);
  ASSERT(sz >= 0);
  return sz;
}
                     ...
                     ErtsDistExternal *edep)
{
  ...
  ep = dec_term(edep, hpp, ep, off_heap, &obj, nullptr);
...
Eterm erts_decode_ext(Eterm **hpp, ErlOffHeap *off_heap, uint8_t **ext)
{
  Eterm obj;
  uint8_t *ep = *ext;
  ...
  ep = dec_term(nullptr, hpp, ep, off_heap, &obj, nullptr);

  if (!ep) {
#ifdef DEBUG
    bin_write(ERTS_PRINT_STDERR, nullptr, *ext, 500);
#endif
    return THE_NON_VALUE;
  }
...
Eterm erts_decode_ext_ets(Eterm **hpp, ErlOffHeap *off_heap, uint8_t *ext)
{
  Eterm obj;
  ext = dec_term(nullptr, hpp, ext, off_heap, &obj, nullptr);
  ASSERT(ext);
  return obj;
}
...
{
  Eterm res = erts_term_to_binary_int(BIF_P, BIF_ARG_1, 0, TERM_TO_BINARY_DFLAGS, nullptr);
...
BIF_RETTYPE term_to_binary_2(BIF_ALIST_2)
{
  ...
  int level = 0;
  size_t flags = TERM_TO_BINARY_DFLAGS;
  Eterm res;
...
error:
  ...
  res = erts_term_to_binary_int(p, Term, level, flags, nullptr);
...
static uLongf binary2term_uncomp_size(uint8_t *data, ssize_t size)
{
  z_stream stream;
  int err;
...
static ERTS_INLINE int
binary2term_prepare(ErtsBinary2TermState *state, uint8_t *data, ssize_t data_size,
                    B2TContext *ctx)
{
  uint8_t *bytes = data;
  ...
  if (size < 5 || *bytes != COMPRESSED) {
    ...
    if (dest_len > 32 * 1024 * 1024
        || (state->extp = (uint8_t *)erts_alloc_fnf(ERTS_ALC_T_EXT_TERM_DATA, dest_len)) == nullptr) {
      /*
      ...
      state->extp = (uint8_t *)erts_alloc(ERTS_ALC_T_EXT_TERM_DATA, dest_len);
      ctx->reds -= dest_len;
    }
...
binary2term_create(ErtsDistExternal *edep, ErtsBinary2TermState *state, Eterm **hpp,
                   ErlOffHeap *ohp)
{
  Eterm res;

  if (!dec_term(edep, hpp, state->extp, ohp, &res, nullptr)) {
    res = THE_NON_VALUE;
  }
...
ssize_t
erts_binary2term_prepare(ErtsBinary2TermState *state, uint8_t *data, ssize_t data_size)
{
  ssize_t res;

  if (binary2term_prepare(state, data, data_size, nullptr) < 0 ||
      (res = decoded_size(state->extp, state->extp + state->extsize, 0, nullptr)) < 0) {
...
{
  return binary2term_create(nullptr, state, hpp, ohp);
}
...
static BIF_RETTYPE binary_to_term_trap_1(BIF_ALIST_1)
{
  ...
  return binary_to_term_int(BIF_P, 0, THE_NON_VALUE, context_bin, nullptr,
                                      ...
                                      Export *bif_init, Eterm arg0, Eterm arg1)
{
...
#else
  SWord initial_reds = (size_t)(ERTS_BIF_REDS_LEFT(p) * B2T_BYTES_PER_REDUCTION);
#endif
  B2TContext c_buff;
  B2TContext *ctx;
  int is_first_call;

  if (context_b == nullptr) {
    /* Setup enough to get started */
    is_first_call = 1;
    ctx = &c_buff;
    ctx->state = B2TPrepare;
    ctx->aligned_alloc = nullptr;
    ctx->flags = flags;
    ctx->bif = bif_init;
    ctx->arg[0] = arg0;
    ctx->arg[1] = arg1;
  ...
  do {
    ...
    case B2TDecodeBinary: {
      ...
      dec_term(&fakedep, nullptr, nullptr, &MSO(p), nullptr, ctx);
      break;
    }
...
{
  return binary_to_term_int(BIF_P, 0, BIF_ARG_1, nullptr, bif_export[BIF_binary_to_term_1],
...
BIF_RETTYPE binary_to_term_2(BIF_ALIST_2)
{
  ...
  return binary_to_term_int(BIF_P, flags, BIF_ARG_1, nullptr, bif_export[BIF_binary_to_term_2],
                            BIF_ARG_1, BIF_ARG_2);
...
external_size_1(BIF_ALIST_1)
{
  ...
  size_t size = erts_encode_ext_size(Term);
...
static Eterm
erts_term_to_binary_simple(Process *p, Eterm Term, size_t size, int level, size_t flags)
{
  Eterm bin;
  size_t real_size;
  uint8_t *endp;
  ...
  if (level != 0) {
    ...
    if (sizeof(buf) < size) {
      bytes = (uint8_t *)erts_alloc(ERTS_ALC_T_TMP, size);
    }
    ...
    bin = new_binary(p, nullptr, real_size + 1);
    ...
    bin = new_binary(p, (uint8_t *)nullptr, size);
...
Eterm
erts_term_to_binary(Process *p, Eterm Term, int level, size_t flags)
{
  size_t size;
  size = encode_size_struct2(nullptr, Term, flags) + 1 /* VERSION_MAGIC */;
...
static void ttb_context_destructor(Binary *context_bin)
{
  ...
  if (context->alive) {
    ...
    case TTBEncode:
      ...
      if (context->s.ec.result_bin != nullptr) { /* Set to nullptr if ever made alive! */
    ...
    case TTBCompress:
      ...
      if (context->s.cc.destination_bin != nullptr) { /* Set to nullptr if ever made alive! */
      ...
      if (context->s.cc.result_bin != nullptr) { /* Set to nullptr if ever made alive! */
                                     ...
                                     Binary *context_b)
{
  ...
  Eterm c_term;
#ifndef EXTREME_TTB_TRAPPING
  ssize_t reds = (ssize_t)(ERTS_BIF_REDS_LEFT(p) * TERM_TO_BINARY_LOOP_FACTOR);
#else
  ssize_t reds = 20; /* For testing */
#endif
  ssize_t initial_reds = reds;
  TTBContext c_buff;
...
#define RETURN_STATE()              \
  ...
  if (context_b == nullptr) {
    ...
    context->alive = 1;
    context->s.sc.estack.start = nullptr;
  ...
  for (;;) {
    ...
    case TTBSize: {
      ...
      /* Try for fast path */
      if (encode_size_struct_int(&context->s.sc, nullptr, Term,
      ...
      /* Next state immediately, no need to export_ context */
      context->state = TTBEncode;
      context->s.ec.flags = flags;
      context->s.ec.level = level;
      context->s.ec.wstack.wstart = nullptr;
    ...
    case TTBEncode: {
      uint8_t *endp;
      uint8_t *bytes = (uint8_t *) context->s.ec.result_bin->orig_bytes;
      size_t real_size;
      Binary *result_bin;

      flags = context->s.ec.flags;

      if (enc_term_int(&context->s.ec, nullptr, Term, bytes + 1, flags, nullptr, &reds, &endp) < 0) {
        EXPORT_CONTEXT();
        RETURN_STATE();
      }
...
return_normal:
        ...
        pb->val = result_bin;
        pb->bytes = (uint8_t *) result_bin->orig_bytes;
        pb->flags = 0;
      ...
      EXPORT_CONTEXT();
      bytes = (uint8_t *) result_bin->orig_bytes; /* result_bin is reallocated */
      ...
      context->s.cc.dbytes = (uint8_t *) result_bin->orig_bytes + 6;
      break;
    }
    ...
    case TTBCompress: {
      ...
      case Z_STREAM_END: {
        uint8_t *dbytes = (uint8_t *) context->s.cc.destination_bin->orig_bytes + 1;
...
static uint8_t *
dec_atom(ErtsDistExternal *edep, uint8_t *ep, Eterm *objp)
{
  size_t len;
  int n;
...
static ERTS_INLINE ErlNode *dec_get_node(Eterm sysname, size_t creation)
{
  switch (creation) {
...
static uint8_t *
dec_pid(ErtsDistExternal *edep, Eterm **hpp, uint8_t *ep, ErlOffHeap *off_heap, Eterm *objp)
{
  Eterm sysname;
  size_t data;
  size_t num;
  ...
  *objp = NIL;    /* In case we fail, don't leave a hole in the heap */
  ...
  /* eat first atom */
  if ((ep = dec_atom(edep, ep, &sysname)) == nullptr) {
    return nullptr;
  }
...
{
  uint8_t *res;
  (void) enc_term_int(nullptr, acmp, obj, ep, dflags, off_heap, nullptr, &res);
  return res;
}
...
#if HALFWORD_HEAP
      uint8_t *size_p = (uint8_t *) wobj;
#else
      uint8_t *size_p = (uint8_t *) obj;
#endif
...
L_jump_start:
    ...
    case SMALL_DEF: {
      ...
      ssize_t val = signed_val(obj);

      if ((size_t)val < 256) {
        *ep++ = SMALL_INTEGER_EXT;
        put_int8(val, ep);
        ep++;
      } else if (sizeof(ssize_t) == 4 || IS_SSMALL32(val)) {
        *ep++ = INTEGER_EXT;
        put_int32(val, ep);
        ep += 4;
      } else {
    ...
    case BIG_DEF: {
      ...
      if (sizeof(ssize_t) == 4 && n <= 4) {
        size_t dig = big_digit(obj, 0);
        ssize_t val = sign ? -dig : dig;
...
fun_env:
  ...
  if (ctx) {
    ASSERT(ctx->wstack.wstart == nullptr);
    *reds = r;
  }
...
is_external_string(Eterm list, int *p_is_string)
{
   ...
   * not exceeded the maximum length of a string, this_
   * is a string.
   */
...
{
  const size_t area_sz = (end - start) * sizeof(Eterm);
  struct erl_off_heap_header *hdr;
  struct erl_off_heap_header **hdr_nextp = nullptr;

  for (hdr = off_heap->first; ; hdr = hdr->next) {
    if (!in_area(hdr, start, area_sz)) {
      if (hdr_nextp != nullptr) {
        *hdr_nextp = nullptr;
  ...
  for (hdr = off_heap->first; hdr != nullptr; hdr = hdr->next) {
...
** On failure return nullptr and (R13B04) *hpp will be unchanged.
*/
static uint8_t *
dec_term(ErtsDistExternal *edep, Eterm **hpp, uint8_t *ep, ErlOffHeap *off_heap,
...
ref_ext_common:
    ...
    case BINARY_EXT: {
      ...
      if ((unsigned)n <= ERL_ONHEAP_BIN_LIMIT) {
        ...
        *objp = make_binary(hb);
        ...
        pb->val = dbin;
        pb->bytes = (uint8_t *) dbin->orig_bytes;
        pb->flags = 0;
        *objp = make_binary(pb);
    ...
    case EXPORT_EXT: {
      ...
      if ((ep = dec_atom(edep, ep, &mod)) == nullptr) {
        goto error;
      }

      if ((ep = dec_atom(edep, ep, &name)) == nullptr) {
        goto error;
      }

      *hpp = hp;
      ep = dec_term(edep, hpp, ep, off_heap, &temp, nullptr);
      hp = *hpp;
    ...
    case NEW_FUN_EXT: {
      ...
      *objp = make_fun(funp);
      ...
      /* Module */
      if ((ep = dec_atom(edep, ep, &module)) == nullptr) {
        goto error;
      }
      ...
      *hpp = hp;
      ...
      /* Index */
      if ((ep = dec_term(edep, hpp, ep, off_heap, &temp, nullptr)) == nullptr) {
        goto error;
      }
      ...
      if (funp->fe->native_address == nullptr) {
...
/* returns the number of bytes needed to encode an object
   ...
   N.B. That this_ must agree with to_external2() above!!!
   (except for cached atoms) */
static size_t encode_size_struct2(ErtsAtomCacheMap *acmp, Eterm obj, unsigned dflags)
{
  size_t res;
  (void) encode_size_struct_int(nullptr, acmp, obj, dflags, nullptr, &res);
  return res;
}
...
L_jump_start:
    ...
    case BIG_DEF:
      ...
      if (sizeof(ssize_t) == 4 && i <= 4 && (big_digit(obj, 0) - big_sign(obj)) < (1 << 31)) {
        result += 1 + 4;  /* INTEGER_EXT */
      } else if (i < 256) {
  ...
  if (ctx) {
    ASSERT(ctx->estack.start == nullptr);
    *reds = r;
  }
...
static ssize_t
decoded_size(uint8_t *ep, uint8_t *endp, int internal_tags, B2TContext *ctx)
{
  int heap_size;
  int terms;
  int atom_extra_skip;
