...
static Eterm
build_compile_result(Process *p, Eterm error_tag, pcre *result, int errcode, const char *errstr,
                     int errofset, int unicode, int with_ok, Eterm extra_err_tag)
{
  Eterm *hp;
  Eterm ret;
  size_t pattern_size;
  ...
  if (!result) {
    ...
    /* XXX: Optimize - keep in offheap binary to allow this_ to
 ...
 * watever is saved. The code is pointed out by this_ and cannot be
    ...
    if (ri->type == RetNone) {
      ...
      if (ri->num_spec <= 0) {
        ...
        for (i = -(ri->num_spec) ; i < rc; ++i) {
          ...
          tmp_vect[i * 2 + 1] = make_signed_integer(restartp->ovector[i * 2 + 1] - restartp->ovector[i * 2],
                                p);
        }
        ...
        for (i = 0; i < ri->num_spec; ++i) {
          ...
          if (x < rc && x >= 0) {
            ...
            tmp_vect[n * 2 + 1] = make_signed_integer(restartp->ovector[x * 2 + 1] - restartp->ovector[x * 2],
                                  p);
          } else {
        ...
        for (i = -(ri->num_spec) ; i < rc; ++i) { /* XXX: Unicode */
          ...
          if (ri->type == RetBin) {
            if (restartp->flags & RESTART_FLAG_SUBJECT_IN_BINARY) {
              ...
              tmp_vect[i] = new_binary(p, (uint8_t *) cp, len);
            }
          } else {
            Eterm *hp2;
            hp2 = HAlloc(p, (2 * len));
          ...
          if (x < rc && x >= 0) {
            ...
            if (ri->type == RetBin) {
              if (restartp->flags & RESTART_FLAG_SUBJECT_IN_BINARY) {
                ...
                tmp_vect[n] = new_binary(p, (uint8_t *) cp, len);
              }
            } else {
              Eterm *hp2;
              hp2 = HAlloc(p, (2 * len));
            ...
            if (ri->type == RetBin) {
              tmp_vect[n] = new_binary(p, (uint8_t *) "", 0);
            } else {
              tmp_vect[n] = NIL;
            }
          }
...
static void build_one_capture(const pcre *code, ReturnInfo **ri, int *sallocated, int has_dupnames,
                              char *name)
{
  ReturnInfo *r = (*ri);
...
code and subject */
{
  ...
  restartp = ERTS_MAGIC_BIN_DATA<RestartContext *>(mbp);
...
re_inspect_2(BIF_ALIST_2)
{
  ...
  for (i = 0; i < top; ++i) {
    if (last == nullptr || !has_dupnames || strcmp((char *) last + 2,
        (char *) name + 2)) {
      tmp_vec[j++] = new_binary(BIF_P, (uint8_t *) name + 2, strlen((char *) name + 2));
    }
