...
/* For explanation of this_ var, see comment for same var in erl_async.c */
 ...
 * this_ error when using +A 10 and enabling SENDFILE_FLGS_USE_THREADS.
 */
...
#ifdef USE_VM_PROBES
  int             idnum;      /* Unique ID # for this_ driver thread/desc */
  ...
  for (ip = &dest->vsize, *ip = 0;  *ip < source->vsize; (*ip)++) {
    if (source->iov[*ip].iov_len == 0) {
      ...
      if (source->binv[*ip]) {
        ...
        /* Contents not in binary - allocate new_ binary and copy data */
        if (!(dest->binv[*ip] =
...
static struct t_data *cq_deq(file_descriptor *desc)
{
  ...
  if (d) {
    ...
    if ((desc->cq_head = d->next) == nullptr) {
      ASSERT(desc->cq_tail == d);
      desc->cq_tail = nullptr;
    }
  }
  ...
  case FILE_OPEN:
    if (d->is_fd_unused && d->fd != FILE_FD_INVALID) {
      /* This is OK to do in scheduler thread because there can be no async op
         ongoing for this_ fd here, as we exited during async open.
         Ideally, this_ close should happen in an async thread too, but that would
...
static int reply_ok(file_descriptor *desc)
{
  ...
  driver_output2(desc->port, &c, 1, nullptr, 0);
  return 0;
}
...
static void invoke_readdir(void *data)
{
  ...
  size_t file_bs;
  size_t n = 0, total = 0;
  struct t_readdir_buf *b = nullptr;
  int res = 0;
...
static void invoke_open(void *data)
{
  ...
  if ((d->flags & EFILE_COMPRESSED) == 0) {
    int fd;
    status = efile_openfile(&d->errInfo, d->b, d->flags, &fd, nullptr);
    d->fd = fd;
  } else {
...
static void invoke_sendfile(void *data)
{
  ...
  result = efile_sendfile(&d->errInfo, fd, out_fd, &d->c.sendfile.offset, &nbytes, nullptr);
...
static int check_write_error(file_descriptor *desc, int *errp)
{
  if (desc->write_error) {
    if (errp) {
      *errp = desc->write_errInfo.posix_errno;
    }
                                  ...
                                  )
{
  ...
  if ((r = flush_write(desc, errp
    ...
    check_write_error(desc, nullptr);
    return r;
  } else {
...
file_async_ready(ErlDrvData e, ErlDrvThreadData data)
{
  ...
  case FILE_READ_LINE:
    ...
    /* The read_line stucture differs from the read structure.
       ...
       why new_ allocation (but not reallocation of the old binary, for obvious reasons)
...
#define RESULT_SIZE (1 + (17 * 4))
      TRACE_C('R');
      driver_output2(desc->port, resbuf, RESULT_SIZE, nullptr, 0);
#undef RESULT_SIZE
    } else {
      reply_error(desc, &d->errInfo);
    }
  ...
  case FILE_READDIR:
    if (!d->result_ok) {
      ...
      driver_output2(desc->port, &op, 1, nullptr, 0);

      d->c.read_dir.first_buf = nullptr;
      d->c.read_dir.last_buf = nullptr;
    }
  ...
  case FILE_CLOSE_ON_PORT_EXIT:
    /* See file_stop. However this_ is never invoked after the port is killed. */
    free_data(data);
    EF_FREE(desc);
    desc = nullptr;
    /* This is it for this_ port, so just send dtrace and return, avoid doing anything to the freed data */
...
#endif
      TRACE_C('R');
      driver_output2(desc->port, resbuf, 1, nullptr, 0);
      return;
    }
...
file_outputv(ErlDrvData e, ErlIOVec *ev)
{
  ...
  case FILE_READ: {
     ...
     * new_, and if the tail is almost half the buffer it is a lot
     * to copy. Therefore allocate the exact amount needed in
     * this_ case, giving no lingering tail. */
    alloc_size =
      size > (desc->read_bufsize >> 1) ?
      size : desc->read_bufsize;

    if (! desc->read_binp) {
      /* Need to allocate a new_ binary for the result */
      ...
      if (desc->read_binp->orig_size - desc->read_offset < size) {
        ...
        /* Move data we already have to the new_ binary */
  ...
  case FILE_READ_LINE: {
     ...
     *    but the worker may need to allocate a new_ one. Freeing the old and rereferencing a newly
    ...
    if (! desc->read_binp) {
      ...
      /* Allocate a new_ binary for the result */
      if (!(desc->read_binp = driver_alloc_binary(alloc_size))) {
        reply_posix_error(desc, ENOMEM);
        goto done;
      }
...
#error 16bit characters in filenames and dtrace in combination is not supported.
    ...
    res_ev->iov = (SysIOVec *)(void_ptr = &d->c.preadv.offsets[d->c.preadv.n]);
...
#error 16bit characters in filenames and dtrace in combination is not supported.
  ...
  case FILE_IPREAD: {
    ...
    res_ev = &d->c.preadv.eiov;
    /* XXX possible alignment problems here for weird machines */
    res_ev->iov = (SysIOVec *)(void_ptr = d + 1);
    res_ev->binv = (ErlDrvBinary **)(void_ptr = res_ev->iov + vsize);
    res_ev->size = 0;
    res_ev->vsize = 0;
