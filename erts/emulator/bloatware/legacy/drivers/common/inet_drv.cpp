...
#define sock_sendv(s, vec, size, np, flag) \
            WSASend((s),(WSABUF*)(vec),(size),(np),(flag),nullptr,nullptr)
 ...
 * later when the function returns the new_ desired state is compared
 ...
 *  * tcp_inet_ose_dispatch_signal : checks state changes and sends new_ signals
...
static typeof(sctp_bindx) *p_sctp_bindx = nullptr;
#else
typedef int (*p_sctp_bindx_t)(int sd, struct sockaddr *addrs, int addrcnt, int flags);
static p_sctp_bindx_t p_sctp_bindx = nullptr;
#endif

#if defined(__GNUC__) && defined(HAVE_SCTP_PEELOFF)
static typeof(sctp_peeloff) *p_sctp_peeloff = nullptr;
#else
typedef int (*p_sctp_peeloff_t)(int sd, sctp_assoc_t assoc_id);
static p_sctp_peeloff_t p_sctp_peeloff = nullptr;
#endif

#if defined(__GNUC__) && defined(HAVE_SCTP_GETLADDRS)
static typeof(sctp_getladdrs) *p_sctp_getladdrs = nullptr;
#else
typedef int (*p_sctp_getladdrs_t)
(int sd, sctp_assoc_t assoc_id, struct sockaddr **ss);
static p_sctp_getladdrs_t p_sctp_getladdrs = nullptr;
#endif

#if defined(__GNUC__) && defined(HAVE_SCTP_FREELADDRS)
static typeof(sctp_freeladdrs) *p_sctp_freeladdrs = nullptr;
#else
typedef void (*p_sctp_freeladdrs_t)(struct sockaddr *addrs);
static p_sctp_freeladdrs_t p_sctp_freeladdrs = nullptr;
#endif

#if defined(__GNUC__) && defined(HAVE_SCTP_GETPADDRS)
static typeof(sctp_getpaddrs) *p_sctp_getpaddrs = nullptr;
#else
typedef int (*p_sctp_getpaddrs_t)
(int sd, sctp_assoc_t assoc_id, struct sockaddr **ss);
static p_sctp_getpaddrs_t p_sctp_getpaddrs = nullptr;
#endif

#if defined(__GNUC__) && defined(HAVE_SCTP_FREEPADDRS)
static typeof(sctp_freepaddrs) *p_sctp_freepaddrs = nullptr;
#else
typedef void (*p_sctp_freepaddrs_t)(struct sockaddr *addrs);
p_sctp_freepaddrs_t p_sctp_freepaddrs = nullptr;
#endif
...
typedef struct {
  ...
  inet_async_op *oph;          /* queue head or nullptr */
  inet_async_op *opt;          /* queue tail or nullptr */
  ...
  inet_address *peer_ptr;    /* fake peername or nullptr */
  inet_address *name_ptr;    /* fake sockname or nullptr */
...
static char *signo_to_string(SIGSELECT signo)
{
  ...
  case SO_EVENT_EOF_REQUEST: {
    return eof_req;
  }

  case SO_EVENT_EOF_REPLY: {
    return eof_rep;
  }

  case SO_EVENT_ACCEPT_REQUEST: {
    return accept_req;
  }
...
static struct erl_drv_entry tcp_inet_driver_entry = {
  tcp_inet_init,  /* inet_init will add this_ driver !! */
  tcp_inet_start,
  tcp_inet_stop,
  tcp_inet_command,
#ifdef __WIN32__
...
static struct erl_drv_entry udp_inet_driver_entry = {
  packet_inet_init,  /* inet_init will add this_ driver !! */
  udp_inet_start,
  packet_inet_stop,
  packet_inet_command,
#ifdef __WIN32__
...
typedef struct {
  ...
  inet_async_multi_op *multi_first;/* nullptr == no multi-accept-queue, op is in ordinary queue */
...
inline void *ALLOC(ErlDrvSizeT X)
{
  return alloc_wrapper(X);
}
...
inline void *REALLOC(void *X, ErlDrvSizeT Y)
{
  return realloc_wrapper(X, Y);
}
 ...
 struct sockaddr_storage *addr)
{
  ...
  */
  ...
  /* NB: the following functions are safe to use, as they create tuples
     ...
     a ptr to "abuf" would be dangling upon exiting this_ function:   */
...
{
#ifdef __GNUC__
#warning CHECK_DOUBLE_RELEASE is enabled, this_ is a custom build emulator
#endif
  int i;
 ...
 * a ref to this_ buffer then call driver_free_binary else
                           ...
                           ErlDrvMonitor *monitorp)
{
  ...
  for (opp = desc->multi_first, slap = nullptr;
       opp != nullptr && opp->op.caller != caller;
...
/* setup a new_ async id + caller (format async_id into buf) */
                           ...
                           ErlDrvMonitor *monitorp)
{
  ...
  if ((opp = desc->oph) == nullptr) {          /* queue empty */
...
{
  return enq_async_w_tmo(desc, buf, req, INET_INFINITY, nullptr);
}
                           ...
                           int *rp, unsigned *tp, ErlDrvMonitor *monitorp)
{
  ...
  if ((opp = desc->opt) == nullptr) {  /* queue empty */
...
{
  return deq_async_w_tmo(desc, ap, cp, rp, nullptr, nullptr);
}
/* send message:
...
static int inet_port_data(inet_descriptor *desc, const char *buf, int len)
{
  ...
  if ((desc->mode == INET_MODE_LIST) || (hsz > len)) {
    return driver_output2(desc->port, (char *)buf, len, nullptr, 0);
  } else if (hsz > 0) {
...
inet_port_binary_data(inet_descriptor *desc, ErlDrvBinary *bin, int offs, int len)
{
  ...
  if ((desc->mode == INET_MODE_LIST) || (hsz > len)) {
    return driver_output2(desc->port, bin->orig_bytes + offs, len, nullptr, 0);
  } else
                    ...
                    const char *buf, int len, const char *prefix, int plen)
{
  ...
  if ((bin = driver_alloc_binary(plen + len)) == nullptr) {
...
(ErlDrvTermData *spec, int i, struct sctp_sndrcvinfo *sri)
{
  ...
  if (sri->sinfo_flags & SCTP_UNORDERED) {
    i = LOAD_ATOM(spec, i, am_unordered);
    n++;
  }

  if (sri->sinfo_flags & SCTP_ADDR_OVER) {
    i = LOAD_ATOM(spec, i, am_addr_over);
    n++;
  }

  if (sri->sinfo_flags & SCTP_ABORT) {
    i = LOAD_ATOM(spec, i, am_abort);
    n++;
  }

  if (sri->sinfo_flags & SCTP_EOF) {
    i = LOAD_ATOM(spec, i, am_eof);
    n++;
  }
...
(ErlDrvTermData *spec, int i, struct msghdr *mptr)
{
  ...
  for (cmsg = frst_msg; cmsg != nullptr; cmsg = CMSG_NXTHDR(mptr, cmsg)) {
    ...
    if (((char *)cmsg + cmsg->cmsg_len) - (char *)frst_msg >
        mptr->msg_controllen)
      /* MUST check this_ in Linux --  the returned "cmsg" may actually
         go too far! */
    {
      break;
    }
    ...
    /* The ONLY kind of ancillary SCTP data which can occur on receiving
       ...
       by the user). So parse this_ type:
    */
...
(ErlDrvTermData *spec, int i, char *chunk, int chlen)
{
  ...
  */
  ...
  while (coff < len) {
    ...
    /* Install the corresp atom for this_ "ccode": */
  ...
  case SCTP_ASSOC_CHANGE: {
    ...
    */
    ...
    /* The ABORT chunk may or may not be present at the end, depending
       ...
       implementation, this_ chunk is not delivered anyway, so we leave
...
#     endif
  ...
  case SCTP_PARTIAL_DELIVERY_EVENT: {
    /* It is not clear  whether this_ event  is sent  to the sender
    (when the receiver gets only a part of a message),   or to
    the receiver itself.  In any case, we do not support partial
    delivery of msgs in this_ implementation, so this_ is an error
    condition:
  ...
  /* XXX: No more supported SCTP Event types. The standard also provides
     ...
     request delivery of such events in this_ implementation, so they
     cannot occur:
  */
  default:
    ASSERT(0);
...
tcp_reply_binary_data(tcp_descriptor *desc, ErlDrvBinary *bin, int offs, int len)
{
  ...
  if (desc->inet.deliver == INET_DELIVER_PORT) {
    ...
    if (desc->inet.active == INET_PASSIVE) {
      return inet_async_binary_data(INETP(desc), 0, bin, offs, bodylen, nullptr);
    } else {
...
#ifdef HAVE_SCTP
  /* Check the size of SCTP AssocID -- currently both this_ driver and the
...
#     else
  p_sctp_getladdrs = nullptr;
  p_sctp_freeladdrs = nullptr;
#     endif
...
#     else
  p_sctp_getpaddrs = nullptr;
  p_sctp_freepaddrs = nullptr;
#     endif
  inet_init_sctp();
...
#       endif
  {
    static void *h_libsctp = nullptr;

    if (erts_sys_ddll_open_noext(STRINGIFY(LIBSCTP), &h_libsctp, nullptr)
        == 0) {
      void *ptr;

      if (erts_sys_ddll_sym(h_libsctp, "sctp_bindx", &ptr) == 0) {
        p_sctp_bindx = (p_sctp_bindx_t)ptr;

        if (erts_sys_ddll_sym(h_libsctp, "sctp_peeloff", &ptr) == 0) {
          p_sctp_peeloff = (p_sctp_peeloff_t)ptr;
        } else {
          p_sctp_peeloff = nullptr;
        }

        if (erts_sys_ddll_sym(h_libsctp, "sctp_getladdrs", &ptr) == 0) {
          p_sctp_getladdrs = (p_sctp_getladdrs_t)ptr;
        } else {
          p_sctp_getladdrs = nullptr;
        }

        if (erts_sys_ddll_sym(h_libsctp, "sctp_freeladdrs", &ptr) == 0) {
          p_sctp_freeladdrs = (p_sctp_freeladdrs_t)ptr;
        } else {
          p_sctp_freeladdrs = nullptr;
          p_sctp_getladdrs = nullptr;
        }
        ...
        if (erts_sys_ddll_sym(h_libsctp, "sctp_getpaddrs", &ptr) == 0) {
          p_sctp_getpaddrs = (p_sctp_getpaddrs_t)ptr;
        } else {
          p_sctp_getpaddrs = nullptr;
        }

        if (erts_sys_ddll_sym(h_libsctp, "sctp_freepaddrs", &ptr) == 0) {
          p_sctp_freepaddrs = (p_sctp_freepaddrs_t)ptr;
        } else {
          p_sctp_freepaddrs = nullptr;
          p_sctp_getpaddrs = nullptr;
        }
...
** return nullptr on error and ptr after port address on success
*/
                              ...
                              char *src, ErlDrvSizeT *len)
{
  ...
  if ((family == AF_INET) && (*len >= 2 + 4)) {
    ...
    *len = sizeof(struct sockaddr_in);
    ...
    dst->sai6.sin6_flowinfo = 0;   /* XXX this_ may be set as well ?? */
...
static int inet_address_to_erlang(char *dst, inet_address **src)
{
  ...
  case AF_INET6:
    if (dst) {
      ...
      port = sock_ntohs((*src)->sai6.sin6_port);
      put_int16(port, dst + 1);
      VALGRIND_MAKE_MEM_DEFINED(&(*src)->sai6.sin6_addr,
                                16); /* false undefs from syscall sctp_get[lp]addrs */
...
(int n, inet_address *addrs, char **rbuf, ErlDrvSizeT rsize)
{
  ...
  if (n == 0) {
    return ctl_reply(INET_REP_OK, nullptr, 0, rbuf, rsize);
  }
  ...
  for (i = 0;  i < n;  i++) {
    s = inet_address_to_erlang(nullptr, &ia);
...
static void desc_close(inet_descriptor *desc)
{
  if (desc->s != INVALID_SOCKET) {
    ...
    if (!desc->is_ignored)
      ...
      inet_stop_select((ErlDrvEvent)(long)desc->event, nullptr);
    }
                                  ...
                                  char **rbuf, ErlDrvSizeT rsize)
{
  ...
  if (desc->netns != nullptr) {
    /* Temporarily change network namespace for this_ thread
  ...
  if (desc->netns != nullptr) {
    ...
    if (setns(current_ns, CLONE_NEWNET) != 0) {
       ...
       * Can this_ even happen? Should the emulator be aborted?
       */
      if (desc->s != INVALID_SOCKET) {
        save_errno = sock_errno();
      }
...
static int parse_addr(char *ptr, int n, long *x)
{
  ...
  while (n--) {
    ...
    case '.':
      if ((dots > 2) || (digs == 0) || (digs > 3) || (v > 0xff)) {
        return -1;
      }
  ...
  if ((dots != 3) || (digs == 0) || (digs > 3) || (v > 0xff)) {
    return -1;
  }
                                       ...
                                       char **rbuf, ErlDrvSizeT rsize)
{
  ...
  len = 0;
  err = WSAIoctl(desc->s, SIO_GET_INTERFACE_LIST, nullptr, 0,
  ...
  while (len--) {
    ...
    case INET_IFOPT_NETMASK:
      ...
      *sptr++ = INET_IFOPT_NETMASK;
      ...
      if ((sptr = sockaddr_to_buf((struct sockaddr *)
                                  &ifp->iiNetmask, sptr, s_end)) == nullptr) {
        goto error;
      }
...
static int get_ifconf(SOCKET s, struct ifconf *ifcp)
{
  ...
  for (;;) {
    ...
    if (ioctl(s, SIOCGIFCONF, (char *)ifcp) < 0) {
      ...
      if (ifcp->ifc_len == ifc_len) {
        break;  /* buf large enough */
      }
  ...
  while (len--) {
    ...
    case INET_IFOPT_ADDR:
      ...
      *sptr++ = INET_IFOPT_ADDR;
      ...
      if ((sptr = sockaddr_to_buf(&ifreq.ifr_addr, sptr, s_end)) == nullptr) {
        goto error;
      }
    ...
    case INET_IFOPT_HWADDR: {
      ...
      *sptr++ = INET_IFOPT_HWADDR;
      put_int16(sizeof(ifreq.ifr_enaddr), sptr);
      sptr += 2;
      /* raw memcpy (fix include autoconf later) */
    ...
    case INET_IFOPT_BROADADDR:
      ...
      *sptr++ = INET_IFOPT_BROADADDR;
      ...
      if ((sptr = sockaddr_to_buf(&ifreq.ifr_broadaddr, sptr, s_end)) == nullptr) {
        goto error;
      }
    ...
    case INET_IFOPT_DSTADDR:
      ...
      *sptr++ = INET_IFOPT_DSTADDR;
      ...
      if ((sptr = sockaddr_to_buf(&ifreq.ifr_dstaddr, sptr, s_end)) == nullptr) {
        goto error;
      }
                                   ...
                                   char **rbuf, ErlDrvSizeT rsize)
{
  ...
  while (buf < b_end) {
    switch (*buf++) {
    case INET_IFOPT_ADDR:
      if ((buf = buf_to_sockaddr(buf, b_end, &ifreq.ifr_addr)) == nullptr) {
        goto error;
      }
    ...
    case INET_IFOPT_BROADADDR:
      ...
      if ((buf = buf_to_sockaddr(buf, b_end, &ifreq.ifr_broadaddr)) == nullptr) {
        goto error;
      }
    ...
    case INET_IFOPT_DSTADDR:
      ...
      if ((buf = buf_to_sockaddr(buf, b_end, &ifreq.ifr_dstaddr)) == nullptr) {
        goto error;
      }
    ...
    case INET_IFOPT_FLAGS: {
      ...
      /* update flags */
      if (eflags & INET_IFF_UP) {
        flags |= IFF_UP;
      }

      if (eflags & INET_IFF_DOWN) {
        flags &= ~IFF_UP;
      }

      if (eflags & INET_IFF_BROADCAST) {
        flags |= IFF_BROADCAST;
      }

      if (eflags & INET_IFF_NBROADCAST) {
        flags &= ~IFF_BROADCAST;
      }

      if (eflags & INET_IFF_POINTTOPOINT) {
        flags |= IFF_POINTOPOINT;
      }

      if (eflags & INET_IFF_NPOINTTOPOINT) {
        flags &= ~IFF_POINTOPOINT;
      }
...
static void set_netmask_bytes(char *c, int len, int pref_len)
{
  ...
  for (i = 0, m = pref_len >> 3;  i < m && i < len;  i++) {
    c[i] = '\xFF';
  }

  if (i < len) {
    c[i++] = 0xFF << (8 - (pref_len & 7));
  }
...
#   define SOCKADDR_TO_BUF(opt, sa)         \
    ...
    if (fpGetAdaptersAddresses) {
      ...
      for (i = 17;  i;  i--) {
        ret = fpGetAdaptersAddresses(
                family, flags, nullptr, ip_adaddrs_p, &ip_adaddrs_size);
  ...
  if (! ip_adaddrs_p) {
    ...
    if (! i) {
      ...
      FREE(ip_addrs_p);
      return ctl_reply(INET_REP_OK, nullptr, 0, rbuf_pp, rsize);
    }
  } else {
    ip_addrs_p = nullptr;
  }
  ...
  *buf_p++ = INET_REP_OK;
  ...
  for (ia_p = nullptr, ip_addrs_p ? ((void *)(i = 0)) : (ia_p = ip_adaddrs_p);
       ip_addrs_p ? (i < ip_addrs_p->dwNumEntries) : (ia_p != nullptr);
       ip_addrs_p ? ((void *)(i++)) : (ia_p = ia_p->Next)) {
    MIB_IPADDRROW *ipaddrrow_p = nullptr;
    DWORD flags = INET_IFF_MULTICAST;
    DWORD index = 0;
    WCHAR *wname_p = nullptr;
    MIB_IFROW ifrow;
...
index:
    ...
    if (wname_p) {
      ...
      len =
        WideCharToMultiByte(
          CP_UTF8, 0, wname_p, -1, nullptr, 0, nullptr, nullptr);
      ...
      BUF_ENSURE(len);
      WideCharToMultiByte(
        CP_UTF8, 0, wname_p, -1, buf_p, len, nullptr, nullptr);
      buf_p += len;
    } else {
      /* Found no name -
    ...
    *buf_p++ = INET_IFOPT_FLAGS;
    ...
    if (ipaddrrow_p) {
      ...
      for (p = ia_p->FirstUnicastAddress;
        ...
        for (q = ia_p->FirstPrefix;
          ...
          if (sa_p->sa_family != sp_p->sa_family) {
            continue;
          }
    ...
    if (ifrow.dwPhysAddrLen) {
      ...
      *buf_p++ = INET_IFOPT_HWADDR;
      put_int16(ifrow.dwPhysAddrLen, buf_p);
      buf_p += 2;
    ...
    /* That is all for this_ interface */
    BUF_ENSURE(1);
    *buf_p++ = '\0';
      ...
      if (res == 0 && res_prio == 0) {
        ...
        if (propagate) {
          /* Some kernels set a SO_PRIORITY by default that you are not permitted to reset,
             silently ignore this_ error condition */
...
static int inet_set_opts(inet_descriptor *desc, char *ptr, int len)
{
  ...
  int propagate = 0; /* Set to 1 if failure to set this_ option
  ...
  while (len >= 5) {
    ...
    case INET_LOPT_ACTIVE:
      ...
      if ((desc->stype == SOCK_STREAM) && (desc->active != INET_PASSIVE) &&
        ...
        if (desc->exitf) {
          ...
          return 0; /* Give up on this_ socket, descriptor lost */
        } else {
    ...
    case INET_LOPT_MSGQ_HIWTRMRK: {
      ...
      erl_drv_busy_msgq_limits(desc->port, nullptr, &high);
      continue;
    }
    ...
    case INET_LOPT_MSGQ_LOWTRMRK: {
      ...
      erl_drv_busy_msgq_limits(desc->port, &low, nullptr);
      continue;
    }
...
#ifdef SO_PRIORITY
      type = SO_PRIORITY;
      propagate = 1; /* We do want to know if this_ fails */
...
{
  ini->sinit_num_ostreams   = get_int16(curr);
  curr += 2;
  ini->sinit_max_instreams  = get_int16(curr);
  curr += 2;
  ini->sinit_max_attempts   = get_int16(curr);
  curr += 2;
  ini->sinit_max_init_timeo = get_int16(curr);
  curr += 2;
  return curr;
}
...
static char *sctp_get_sendparams(struct sctp_sndrcvinfo *sri, char *curr)
{
  ...
  sri->sinfo_stream       = get_int16(curr);
  curr += 2;
  sri->sinfo_ssn      = 0;
  ...
  */
  eflags        = get_int16(curr);
  curr += 2;
  cflags        = 0;

  if (eflags & SCTP_FLAG_UNORDERED) {
    cflags |= SCTP_UNORDERED;
  }

  if (eflags & SCTP_FLAG_ADDR_OVER) {
    cflags |= SCTP_ADDR_OVER;
  }

  if (eflags & SCTP_FLAG_ABORT) {
    cflags |= SCTP_ABORT;
  }

  if (eflags & SCTP_FLAG_EOF) {
    cflags |= SCTP_EOF;
  }
  ...
  curr += 4;
  sri->sinfo_context      = get_int32(curr);
  curr += 4;
  sri->sinfo_timetolive   = get_int32(curr);
  curr += 4;
  sri->sinfo_tsn      = 0;
  sri->sinfo_cumtsn     = 0;
  sri->sinfo_assoc_id     = GET_ASSOC_ID(curr);
  curr += ASSOC_ID_LEN;
...
# endif
  ...
  while (curr < ptr + len) {
    ...
    */
    ...
    case INET_LOPT_MODE:
      desc->mode   = get_int32(curr);
      curr += 4;
      res = 0;
      continue;

    case INET_LOPT_ACTIVE:
      desc->active = get_int32(curr);
      curr += 4;

      if (desc->active == INET_MULTI) {
        long ac = desc->active_count;
        int16_t nval = get_int16(curr);
        curr += 2;
        ac += nval;
    ...
    case SCTP_OPT_RTOINFO: {
      ...
      arg.rtoi.srto_assoc_id = GET_ASSOC_ID(curr);
      curr += ASSOC_ID_LEN;
      arg.rtoi.srto_initial  = get_int32(curr);
      curr += 4;
      arg.rtoi.srto_max      = get_int32(curr);
      curr += 4;
      arg.rtoi.srto_min      = get_int32(curr);
      curr += 4;
    ...
    case SCTP_OPT_ASSOCINFO: {
      ...
      arg.ap.sasoc_assoc_id    = GET_ASSOC_ID(curr);
      curr += ASSOC_ID_LEN;
      arg.ap.sasoc_asocmaxrxt  = get_int16(curr);
      curr += 2;
      arg.ap.sasoc_number_peer_destinations =
        get_int16(curr);
      curr += 2;
      arg.ap.sasoc_peer_rwnd   = get_int32(curr);
      curr += 4;
      arg.ap.sasoc_local_rwnd  = get_int32(curr);
      curr += 4;
      arg.ap.sasoc_cookie_life = get_int32(curr);
      curr += 4;
    ...
    case INET_OPT_LINGER: {
      CHKLEN(curr, 2 * 4);
      arg.lin.l_onoff  = get_int32(curr);
      curr += 4;
      arg.lin.l_linger = get_int32(curr);
      curr += 4;
...
#       endif
    ...
    case SCTP_OPT_SET_PEER_PRIMARY_ADDR: {
      ...
      */
      arg.prim.sspp_assoc_id = GET_ASSOC_ID(curr);
      curr += ASSOC_ID_LEN;
    ...
    case SCTP_OPT_ADAPTATION_LAYER: {
      ...
      arg.ad.ssb_adaptation_ind = sock_htonl(get_int32(curr));
      curr += 4;
...
#           endif
      ...
      CHKLEN(curr, ASSOC_ID_LEN);
      arg.pap.spp_assoc_id = GET_ASSOC_ID(curr);
      curr += ASSOC_ID_LEN;
      ...
      arg.pap.spp_hbinterval = get_int32(curr);
      curr += 4;
      arg.pap.spp_pathmaxrxt = get_int16(curr);
      curr += 2;
...
#     ifdef HAVE_STRUCT_SCTP_PADDRPARAMS_SPP_FLAGS
      ...
      if (hb_enable) {
        cflags |= SPP_HB_ENABLE;
      }

      if (hb_disable) {
        cflags |= SPP_HB_DISABLE;
      }

      if (eflags & SCTP_FLAG_HB_DEMAND) {
        cflags |= SPP_HB_DEMAND;
      }
      ...
      if (pmtud_enable) {
        cflags |= SPP_PMTUD_ENABLE;
      }

      if (pmtud_disable) {
        cflags |= SPP_PMTUD_DISABLE;
      }
...
#     ifdef HAVE_STRUCT_SCTP_PADDRPARAMS_SPP_SACKDELAY
      ...
      if (sackdelay_enable) {
        cflags |= SPP_SACKDELAY_ENABLE;
      }

      if (sackdelay_disable) {
        cflags |= SPP_SACKDELAY_DISABLE;
      }
...
#     endif
    ...
    case SCTP_OPT_EVENTS: {
      ...
      */
      arg.es.sctp_data_io_event          = get_int8(curr);
      curr++;
      arg.es.sctp_association_event      = get_int8(curr);
      curr++;
      arg.es.sctp_address_event        = get_int8(curr);
      curr++;
      arg.es.sctp_send_failure_event     = get_int8(curr);
      curr++;
      arg.es.sctp_peer_error_event       = get_int8(curr);
      curr++;
      arg.es.sctp_shutdown_event         = get_int8(curr);
      curr++;
      arg.es.sctp_partial_delivery_event = get_int8(curr);
      curr++;
      arg.es.sctp_adaptation_layer_event = get_int8(curr);
      curr++;
...
# ifdef SCTP_DELAYED_ACK_TIME
    ...
    case SCTP_OPT_DELAYED_ACK_TIME: {
      ...
      arg.av.assoc_id    = GET_ASSOC_ID(curr);
      curr += ASSOC_ID_LEN;
      arg.av.assoc_value = get_int32(curr);
      curr += 4;
...
#define TRUNCATE_TO(Size,Ptr)       \
  ...
  *ptr = INET_REP_OK;
  ...
  while (len--) {
    opt = *buf++;
    proto = SOL_SOCKET;
    ival = 0; /* Windows Vista needs this_ (only writes part of it) */
    arg_sz = sizeof(ival);
    arg_ptr = (char *) &ival;
  ...
  while (buflen > 0) {
    ...
    */
    ...
    case INET_LOPT_MODE: {
      ...
      switch (desc->mode) {
      case INET_MODE_LIST  : {
        i = LOAD_ATOM(spec, i, am_list);
        break;
      }

      case INET_MODE_BINARY: {
        i = LOAD_ATOM(spec, i, am_binary);
        break;
      }
    ...
    case INET_LOPT_ACTIVE: {
      ...
      case INET_ONCE   : {
        i = LOAD_ATOM(spec, i, am_once);
        break;
      }

      case INET_MULTI  : {
        i = LOAD_INT(spec, i, desc->active_count);
        break;
      }
    ...
    /* The following option returns a tuple {bool_, int}:   */
    case INET_OPT_LINGER: {
      struct linger lg;
...
#           endif
      ...
      if (sock_getopt(desc->s, proto, type, &res, &sz) < 0) {
        continue;
      }
...
#     ifdef HAVE_STRUCT_SCTP_PADDRPARAMS_SPP_FLAGS
      ...
      if (ap.spp_flags & SPP_HB_ENABLE) {
        i = LOAD_ATOM(spec, i, am_hb_enable);
        n++;
      }

      if (ap.spp_flags & SPP_HB_DISABLE) {
        i = LOAD_ATOM(spec, i, am_hb_disable);
        n++;
      }

      if (ap.spp_flags & SPP_HB_DEMAND) {
        i = LOAD_ATOM(spec, i, am_hb_demand);
        n++;
      }

      if (ap.spp_flags & SPP_PMTUD_ENABLE) {
        i = LOAD_ATOM(spec, i, am_pmtud_enable);
        n++;
      }

      if (ap.spp_flags & SPP_PMTUD_DISABLE) {
        i = LOAD_ATOM(spec, i, am_pmtud_disable);
        n++;
      }
...
#     ifdef HAVE_STRUCT_SCTP_PADDRPARAMS_SPP_SACKDELAY
      ...
      if (ap.spp_flags & SPP_SACKDELAY_ENABLE) {
        i = LOAD_ATOM(spec, i, am_sackdelay_enable);
        n++;
      }

      if (ap.spp_flags & SPP_SACKDELAY_DISABLE) {
        i = LOAD_ATOM(spec, i, am_sackdelay_disable);
        n++;
      }
...
#           endif
  ...
  if (buflen != 0) {
    RETURN_ERROR(spec, -EINVAL);  /* Optparam mismatch */
  }
...
static ErlDrvData inet_start(ErlDrvPort port, int size, int protocol)
{
  ...
  if ((desc = (inet_descriptor *) ALLOC(size)) == nullptr) {
    return nullptr;
  }
  ...
  desc->peer_ptr = nullptr;
  desc->name_ptr = nullptr;
  ...
  desc->empty_out_q_subs.next = nullptr;
...
#ifdef __OSE__
  desc->select_state = 0;
  desc->events[0] = nullptr;
  desc->events[1] = nullptr;
  desc->events[2] = nullptr;
  desc->events[3] = nullptr;
  desc->events[4] = nullptr;
  desc->events[5] = nullptr;
#endif
                             ...
                             ErlDrvSizeT len, char **rbuf, ErlDrvSizeT rsize)
{
  ...
  case INET_REQ_GETSTAT: {
    ...
    if (dstlen > rsize) {
      if ((dst = (char *) ALLOC(dstlen)) == nullptr) {
        return 0;
      }

      *rbuf = dst;  /* call will free this_ buffer */
    } else {
  ...
  case INET_REQ_BIND:  {      /* bind socket */
    ...
    if (inet_set_faddress(desc->sfamily, &local, buf, &len) == nullptr) {
      return ctl_error(EINVAL, rbuf, rsize);
    }
  ...
  case INET_REQ_GETSERVBYNAME: { /* L1 Name-String L2 Proto-String */
    ...
    if ((srv = sock_getservbyname(namebuf, protobuf)) == nullptr) {
      return ctl_error(EINVAL, rbuf, rsize);
    }
  ...
  case INET_REQ_GETSERVBYPORT: { /* P1 P0 L1 Proto-String */
    ...
    if ((srv = sock_getservbyport(port, protobuf)) == nullptr) {
      return ctl_error(EINVAL, rbuf, rsize);
    }
...
static void inet_output_count(inet_descriptor *desc, ErlDrvSizeT len)
{
  ...
  if (n == 0) { /* WRAP, use old avg as input to a new_ sequence */
    n = 1;
  }
...
/*
** Set new_ size on buffer, used when packet size is determined
...
static int tcp_expand_buffer(tcp_descriptor *desc, int len)
{
  ...
  if ((bin = driver_realloc_binary(desc->i_buf, ulen)) == nullptr) {
    return -1;
  }
...
static ErlDrvData prep_tcp_inet_start(ErlDrvPort port, char *args)
{
  ...
  desc->http_state = 0;
  desc->mtd = nullptr;
  desc->multi_first = desc->multi_last = nullptr;
  DEBUGF(("tcp_inet_start(%ld) }\r\n", (long)port));
  return (ErlDrvData) desc;
}
...
/* Copy a descriptor, by creating a new_ port with same settings
 * as the descriptor desc.
 * return nullptr on error (SYSTEM_LIMIT no ports avail)
 */
                                     ...
                                     ErlDrvTermData owner, int *err)
{
  ...
  copy_desc = (tcp_descriptor *) prep_tcp_inet_start(port, nullptr);
  ...
  /* Some flags must be inherited at this_ point */
  ...
  /* The new_ port will be linked and connected to the original caller */
...
static void tcp_close_check(tcp_descriptor *desc)
{
  ...
  if (desc->inet.state == INET_STATE_ACCEPTING) {
    ...
    while (deq_multi_op(desc, &id, &req, &caller, nullptr, &monitor) == 0) {
...
static void tcp_inet_stop(ErlDrvData e)
{
  ...
  desc->i_buf = nullptr; /* net_mess2 may call this_ function recursively when
                                     ...
                                     char **rbuf, ErlDrvSizeT rsize)
{
  ...
  tcp_inet_ose_dispatch_signals((tcp_descriptor *)e, prev_select_state, nullptr);
                                 ...
                                 char **rbuf, ErlDrvSizeT rsize)
{
  ...
  case INET_REQ_ACCEPT: {  /* do async accept */
    ...
    if (desc->inet.state == INET_STATE_ACCEPTING) {
      ...
      MultiTimerData *mtd = nullptr, *omtd = nullptr;
      ...
      if (s == INVALID_SOCKET) {
        ...
        if ((accept_desc = tcp_inet_copy(desc, s, caller, &err)) == nullptr) {
          sock_close(s);
...
static void tcp_inet_multi_timeout(ErlDrvData e, ErlDrvTermData caller)
{
  ...
  if (remove_multi_op(desc, &id, &req, caller, nullptr, &monitor) != 0) {
    return;
  }
...
static void tcp_inet_process_exit(ErlDrvData e, ErlDrvMonitor *monitorp)
{
  ...
  if ((state & INET_STATE_MULTI_ACCEPTING) == INET_STATE_MULTI_ACCEPTING) {
    ...
    if (remove_multi_op(desc, &id, &req, who, &timeout, nullptr) != 0) {
      return;
    }
...
static void inet_stop_select(ErlDrvEvent event, void *_)
{
  ...
  ErlDrvOseEventId id;
  union SIGNAL *sig;
  erl_drv_ose_event_fetch(event, nullptr, &id, nullptr);
...
static int tcp_deliver(tcp_descriptor *desc, int len)
{
  ...
  if (len == 0) {
    ...
    if ((desc->i_buf == nullptr) || (desc->i_remain > 0)) {
      return count;
    }
  ...
  while (len > 0) {
    ...
    if (len * 4 >= desc->i_buf->orig_size * 3) { /* >=75% */
      ...
      if (desc->i_ptr_start + len == desc->i_ptr) { /* no */
        tcp_clear_input(desc);
      } else { /* move trail to beginning of a new_ buffer */
      ...
      /* Windows needs workaround for this_ in tcp_inet_event...  */
      if (code < 0) {
        return code;
      }
...
static int tcp_recv(tcp_descriptor *desc, int request_len)
{
  ...
  if (desc->i_buf == nullptr) {  /* allocte a read buffer */
    int sz = (request_len > 0) ? request_len : desc->inet.bufsz;

    if ((desc->i_buf = alloc_buffer(sz)) == nullptr) {
      return -1;
    }
...
static int winsock_event_select(inet_descriptor *desc, int flags, int on)
{
  ...
  /* The RIGHT WAY (TM) to do this_ is to make sure:
     A) The cancelling of all network events is done with
        nullptr as the event parameter (bug in NT's winsock),
     ...
     events again. */
  if (WSAEventSelect(desc->s, nullptr, 0) != 0) {
    DEBUGF(("port %d: winsock_event_select: "
    ...
    if (netEv.lNetworkEvents & FD_CLOSE) {
      ...
      for (;;) {
        ...
        /* XXX The buffer will be thrown away on error (empty que).
           Possible SMP FIXME. */
        if (!desc->inet.active && (desc->inet.opt) == nullptr) {
          goto error;
        }
...
tryagain:
    ...
    if (IS_SOCKET_ERROR(n)) {
       ...
       * send out then try to send at least max_sz (this_ comes with
...
static int tcp_inet_output(tcp_descriptor *desc, HANDLE event)
{
  ...
  if (desc->inet.state == INET_STATE_CONNECTING) {
    ...
    for (;;) {
      ...
      if ((iov = driver_peekq(ix, &vsize)) == nullptr) {
...
write_error:
        ...
        /* Workaround for redhat/CentOS 6.3 returning
...
{
  /* SMP: FIXME this_ is probably not SMP safe but may be ok anyway? */
...
/* Copy a descriptor, by creating a new_ port with same settings
 * as the descriptor desc.
 * return nullptr on error (ENFILE no ports avail)
 */
...
static udp_descriptor *sctp_inet_copy(udp_descriptor *desc, SOCKET s, int *err)
{
  ...
  copy_desc = (udp_descriptor *) packet_inet_start(port, nullptr, IPPROTO_SCTP);
  ...
  /* The new_ port will be linked and connected to the caller */
                                    ...
                                    ErlDrvSizeT len, char **rbuf, ErlDrvSizeT rsize)
{
  ...
  case INET_REQ_OPEN:   /* open socket and return internal index */
    ...
    switch (buf[1]) {
    case INET_TYPE_STREAM:
      type = SOCK_STREAM;
      break;

    case INET_TYPE_DGRAM:
      type = SOCK_DGRAM;
      break;
#ifdef HAVE_SCTP

    case INET_TYPE_SEQPACKET:
      type = SOCK_SEQPACKET;
      break;
#endif
    ...
    if ((*rbuf)[0] != INET_REP_ERROR) {
       ...
       * existing machine doesn't close the socket. (Linux behaves this_
       * way)
       */
  ...
  case INET_REQ_CONNECT:  {
    ...
    */
    ...
    if (IS_SCTP(desc)) {
      ...
      if (inet_set_address(desc->sfamily, &remote, buf, &len) == nullptr) {
        return ctl_error(EINVAL, rbuf, rsize);
      }
    ...
    /* UDP */
    if (len == 0) {
      /* What does it mean???  nullptr sockaddr??? */
      sock_connect(desc->s, (struct sockaddr *) nullptr, 0);
  ...
  case SCTP_REQ_BINDX: {
    ...
    /* Add additional addresses by calling sctp_bindx with one address
       at a time, since this_ is what some OSes promise will work.
  ...
  case SCTP_REQ_PEELOFF: {
    ...
    if ((new_udesc = sctp_inet_copy(udesc, new_socket, &err)) == nullptr) {
...
static void packet_inet_command(ErlDrvData e, char *buf, ErlDrvSizeT len)
{
  ...
  if (IS_SCTP(desc)) {
     ...
     */
    mhdr.msg_name           = nullptr;          /* Already connected  */
    mhdr.msg_namelen        = 0;
...
static int packet_inet_input(udp_descriptor *udesc, HANDLE event)
{
  ...
  while (packet_count--) {
    ...
    if (udesc->i_buf == nullptr) {
      ...
      if ((udesc->i_buf = alloc_buffer(udesc->i_bufsz)) == nullptr) {
      ...
      if ((tmp = realloc_buffer(udesc->i_buf, bufsz)) == nullptr) {
        release_buffer(udesc->i_buf);
        udesc->i_buf = nullptr;
        return packet_error(udesc, ENOMEM);
      } else {
      ...
      */
      ...
      if ((desc->mode == INET_MODE_BINARY)
        ...
        if ((tmp = realloc_buffer(udesc->i_buf, bufsz)) != nullptr) {
          udesc->i_buf = tmp;
          udesc->i_bufsz = bufsz;
          udesc->i_ptr = nullptr;  /* not used from here */
        }
      }
...
static void find_dynamic_functions(void)
{
  ...
  fpSetHandleInformation = (module != nullptr) ?
 ...
 *
 * XXX It is not clear whether this_ works/is necessary in Windows 95.
...
{
  if (s != INVALID_SOCKET) {
    if (fpSetHandleInformation != nullptr) {
                                           ...
                                           ErlDrvTermData caller))
{
  ...
  mtd->caller = caller;
  mtd->next = mtd->prev = nullptr;

  for (p = *first, s = nullptr; p != nullptr; s = p, p = p->next) {
