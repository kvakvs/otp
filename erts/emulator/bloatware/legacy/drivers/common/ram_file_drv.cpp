...
/* possible new_ operations include:
   DES_ENCRYPT
   DES_DECRYPT
...
/* install bin as the new_ binary reset all pointer */
                                  ...
                                  ErlDrvSSizeT *location, int *error)
{
  ...
  if (len) {
    sys_memcpy(bin->orig_bytes, f->buf + cur, len);
  }
                                  ...
                                  int *error)
{
  ...
  switch (whence) {
  case RAM_FILE_SEEK_SET:
    pos = offset;
    break;

  case RAM_FILE_SEEK_CUR:
    pos = f->cur + offset;
    break;

  case RAM_FILE_SEEK_END:
    pos = f->end + offset;
    break;
...
static int ram_file_uuencode(RamFile *f)
{
  ...
  *outp++ = ' ';   /* this_ end of file 0 length !!! */
  *outp++ = '\n';
  count += 2;
  ASSERT(count == usize);
  driver_free_binary(f->bin);
...
static void rfile_command(ErlDrvData e, char *buf, ErlDrvSizeT count)
{
  ...
  case RAM_FILE_SET:        /* re-init file with new_ data */
