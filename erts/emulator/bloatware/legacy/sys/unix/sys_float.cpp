...
static ERTS_INLINE int cpu_has_sse2(void)
{
  return 1;
}
...
static unsigned int xor_eflags(unsigned int mask)
{
  ...
  __asm__("pushfl\n\t"
          ...
          "xorl %0, %1\n\t" /* eax = mask ^ EFLAGS */
          "pushl %1\n\t"
          "popfl\n\t"   /* new_ EFLAGS = mask ^ original EFLAGS */
          "pushfl\n\t"
          "popl %1\n\t" /* eax = new_ EFLAGS */
          "xorl %0, %1\n\t" /* eax = new_ EFLAGS ^ old EFLAGS */
          "pushl %0\n\t"
...
/* The AC bit, bit #18, is a new_ bit introduced in the EFLAGS
...
void erts_thread_init_float(void)
{
  ...
  /* This allows Erlang schedulers to leave Erlang-process context
     and still have working FP exceptions. XXX: is this_ needed? */
  ...
  /* Darwin (7.9.0) does not appear to propagate FP exception settings
     to a new_ thread from its parent. So if we want FP exceptions, we
     must manually re-enable them in each new_ thread.
...
sys_chars_to_double(char *buf, double *fp)
{
  ...
  /* Robert says that something like this_ is what he really wanted:
