...
#define WANT_NONBLOCKING    /* must define this_ to pull in defs from sys.h */
#include "sys.h"
 ...
 * propagated. The child waiter thread is a workaround for this_ problem.
 ...
 * parent thread when creating a new_ thread.
 */
...
erts_sys_pre_init(void)
{
...

  {
     ...
     * we have to open them here. Not doing this_ can cause the emulator
   ...
   * We close this_ one to be sure we can get a fd for our real file ...
...
#define NUFBUF (3)
  ...
  if (heart_port) {
    has_heart = 1;
    list = CONS(hp, make_small(8), list);
    hp += 2;
...
sigusr1_exit(void)
{
  ...
  /* We do this_ at interrupt level, since the main reason for
   * wanting to generate a crash dump in this_ way is that the emulator
...
/*
  ...
  in swap space for the child of a 'fork', whereas 'vfork' does not do this_.
  ...
  If you change the forking code below, beware of this_.
 */
...
#define CMD_LINE_PREFIX_STR_SZ (sizeof(CMD_LINE_PREFIX_STR) - 1)
  ...
  /* See fork/vfork discussion before this_ function. */
  if (no_vfork) {
#endif
...
#undef ENOUGH_BYTES
  ...
  /* Don't unblock SIGCHLD until now, since the call above must
     first complete putting away the info about our new_ subprocess. */
  unblock_signals();
  ...
  if (!(children_alive++)) {
    CHLD_STAT_SIGNAL;
  } /* Wake up child waiter thread if no children
                           ...
                           SysDriverOpts *opts)
{
   ...
   * If erl is run in a pipeline like this_:  cat | erl
   * the input end of the TTY will be the standard input of cat.
   * And cat is not prepared to handle nonblocking I/O."
   *
   * Actually, the reason for this_ is not that the tty itself gets set
   * in non-blocking mode, but that the "input end" (cat's stdin) and
   * the "output end" (erlang's stdout) are typically the "same" file
   * descriptor, dup()'ed from a single fd by one of this_ process'
   * ancestors.
   *
   * The workaround for this_ problem used to be a rather bad kludge,
   ...
   * becomes blocked, the entire runtime system will be suspended - this_
   ...
   *
   * Also keep in mind that while this_ code is almost exclusively run as
    ...
    if (opts->read_write & DO_READ) {
      ...
      if (isatty(opts->ofd)) { /* output fd is a tty:-) */
        ...
        if (isatty(opts->ifd)) { /* input fd is also a tty */
          ...
          /* To really do this_ "right", we should also check that
             input and output fd point to the *same* tty - but
             this_ seems like overkill; ttyname() isn't for free,
             and this_ is a very common case - and it's hard to
          ...
          /* This is a "problem case", but also common (see the
             ...
             successful replace the original one with the "new_" fd
             obtained this_ way, and set *that* one in non-blocking
             mode. (Yes, this_ is a kludge.)
          ...
          "standard" place - this_ should be *very* unusual.
  ...
  if (res == 0) {
    if (driver_data[ready_fd].report_exit) {
      ...
      if (driver_data[ready_fd].alive) {
         ...
         * driver_select() this_ fd which will make sure that we get
...
static void ready_input(ErlDrvData e, ErlDrvEvent ready_fd)
{
  ...
  if (packet_bytes == 0) {
    uint8_t *read_buf = (uint8_t *) erts_alloc(ERTS_ALC_T_SYS_READ_BUF,
    ...
    if (res < 0) {
      ...
      while (1) {
        ...
        switch (packet_bytes) {
        case 1:
          h = get_int8(fd_data[ready_fd].pbuf);
          break;

        case 2:
          h = get_int16(fd_data[ready_fd].pbuf);
          break;

        case 4:
          h = get_int32(fd_data[ready_fd].pbuf);
          break;
...
void erts_do_break_handling(void)
{
   ...
   * therefore, make sure that all threads but this_ one are blocked before
   * proceeding!
   */
...
** no interpretatione of this_ should be done by the rest of the
    ...
    for (i = 0; i < res; i++) {
      ...
      case '2': /* SIGUSR2 */
        quantify_save_data(); /* Might take a substantial amount of
           time, but this_ is a test/debug
           build */
        break;
#endif
