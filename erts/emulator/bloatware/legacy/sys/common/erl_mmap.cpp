...
/* Initialize 'desc' and insert as new_ free segment [start->end].
 * The new_ segment must not contain or be adjacent to any free segment in 'map'.
 */
 ...
 * The new_ segment location must overlap the old location and
...
Eterm erts_mmap_info_options(const char *prefix,
                             int *print_to_p,
                             void *print_to_arg,
                             size_t **hpp,
                             size_t *szp)
...
Eterm erts_mmap_debug_info(Process *p)
{
  if (mmap_state.supercarrier) {
    ...
    sa_list = TUPLE2(hp, am_atom_put("sa_free_segs", 12), sa_list);
    hp += 3;
    sua_list = TUPLE2(hp, am_atom_put("sua_free_segs", 13), sua_list);
    hp += 3;
    list = CONS(hp, sua_list, list);
    hp += 2;
    list = CONS(hp, sa_list, list);
    hp += 2;
...
void test_it(void)
{
...

  {
    ...
    HARD_CHECK_TREE(&map.atree, 0);
    HARD_CHECK_TREE(&map.stree, 0);
