 ...
 *              are used if possible instead of creating new_ segments.
 *
...
static ERTS_INLINE int cache_bless_segment(MemKind *mk, void *seg, UWord size, size_t flags)
{
  ...
  ASSERT(!MSEG_FLG_IS_2POW(flags) || (MSEG_FLG_IS_2POW(flags) && MAP_IS_ALIGNED(seg)
                                      && IS_2POW(size)));
  ...
  if (!erts_circleq_is_empty(&(mk->cache_free))) {
     ...
     * Note: Though this_ is the wanted policy I don't think it is used significantly.
 ...
 * - Check if we can empty some cached segments in this_
 *   MemKind.
 */
 ...
 *   on this_ mseg-allocator and memkind.
             ...
             UWord old_size, UWord *new_size_p, size_t flags, const ErtsMsegOpt_t *opt)
{
  ...
  /* Just allocate a new_ segment if we didn't have one before */
  if (!seg || !old_size) {
  ...
  /* Dealloc old segment if new_ segment is of size 0 */
  if (!(*new_size_p)) {
...
erts_mseg_init(ErtsMsegInit_t *init)
{
  ...
  if (sizeof(void *) != 8) {
    erl::exit(-1, "Halfword emulator cannot be run in 32bit mode");
  }
  ...
  if (!IS_2POW(GET_PAGE_SIZE)) {
    erl::exit(erts::ABORT_EXIT, "erts_mseg: Unexpected page_size %beu\n", GET_PAGE_SIZE);
  }
