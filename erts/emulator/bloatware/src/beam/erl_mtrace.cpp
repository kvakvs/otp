...
#define WRITE_UI8(P, V) (*(P) = (uint8_t) ((V) & 0xff))
...
do {									\
    ...
    switch ((M)) {							\
    case 7: *((P)++) = (uint8_t) ((v__ >> 56) & 0xff);			\
    case 6: *((P)++) = (uint8_t) ((v__ >> 48) & 0xff);			\
    case 5: *((P)++) = (uint8_t) ((v__ >> 40) & 0xff);			\
    case 4: *((P)++) = (uint8_t) ((v__ >> 32) & 0xff);			\
    case 3: *((P)++) = (uint8_t) ((v__ >> 24) & 0xff);			\
    case 2: *((P)++) = (uint8_t) ((v__ >> 16) & 0xff);			\
    case 1: *((P)++) = (uint8_t) ((v__ >>  8) & 0xff);			\
    case 0: *((P)++) = (uint8_t) ( v__        & 0xff);			\
...
static uint8_t trace_buffer[TRACE_BUF_SZ];
static uint8_t *tracep;
static uint8_t *endp;
...
write_trace_header(const char *nodename, const char *pid, const char *hostname)
{
    ...
    PUT_UI8(tracep, (uint8_t) n_len);
    memcpy((void *) tracep, (void *) nodename, n_len);
    tracep += n_len;

    PUT_UI8(tracep, (uint8_t) h_len);
    memcpy((void *) tracep, (void *) hostname, h_len);
    tracep += h_len;

    PUT_UI8(tracep, (uint8_t) p_len);
     ...
     * as a new_ tagged entry. The reader should skip stuff at the
...
static ERTS_INLINE void
write_realloc_entry(uint8_t tag,
		    void *res,
		    ErtsAlcType_t x,
		    ErtsAlcType_t y,
		    void *ptr,
