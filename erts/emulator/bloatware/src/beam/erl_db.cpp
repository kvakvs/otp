...
typedef union {
  ...
  uint8_t cache_line_align__[ERTS_ALC_CACHE_LINE_ALIGN_SIZE(
...
static meta_main_tab_t *meta_main_tab;

/* A slot in meta_main_tab can have three states:
 * FREE : Free to use for new_ table. Part of linked free-list.
...
union {
  ...
  uint8_t _cache_line_alignment[64];
} meta_name_tab_rwlocks[META_NAME_TAB_LOCK_CNT];
#endif
typedef struct meta_name_tab_entry_t_ {
  union {
    Eterm name_atom;
  ...
  union {
    DbTable *tb;
    meta_name_tab_entry_t_ *mvec;
  } pu;
} meta_name_tab_entry_t;
static meta_name_tab_entry_t *meta_name_tab;

static unsigned meta_name_tab_mask;

static ERTS_INLINE
meta_name_tab_entry_t *meta_name_tab_bucket(Eterm name,
    erts_smp_rwmtx_t **lockp)
{
  unsigned bix = atom_val(name) & meta_name_tab_mask;
  meta_name_tab_entry_t *bucket = &meta_name_tab[bix];
#ifdef ERTS_SMP
...
free_dbtable(void *vtb)
{
  ...
  print_table(ERTS_PRINT_STDOUT, nullptr, 1, meta_pid_to_tab);
  ...
  print_table(ERTS_PRINT_STDOUT, nullptr, 1, meta_pid_to_fixed_tab);
#endif
#ifdef ERTS_SMP
                          ...
                          int meta_already_locked)
{
  ...
  if (is_small(id)) {
    ...
    meta_name_tab_entry_t *bucket = meta_name_tab_bucket(id, &mtl);
...
static int insert_named_tab(Eterm name_atom, DbTable *tb, int have_lock)
{
  ...
  meta_name_tab_entry_t *new_entry;
  meta_name_tab_entry_t *bucket = meta_name_tab_bucket(name_atom,
                                  &rwlock);
  ...
  if (bucket->pu.tb == nullptr) { /* empty */
    ...
    if (is_atom(bucket->u.name_atom)) { /* single */
      ...
      cnt = 2;
      size = sizeof(meta_name_tab_entry_t) * cnt;
      entries = (meta_name_tab_entry_t *)erts_db_alloc_nt(ERTS_ALC_T_DB_NTAB_ENT, size);
      ...
      old_size = sizeof(meta_name_tab_entry_t) * cnt;
      size = sizeof(meta_name_tab_entry_t) * (cnt + 1);
      entries = (meta_name_tab_entry_t *)erts_db_realloc_nt(ERTS_ALC_T_DB_NTAB_ENT,
                bucket->pu.mvec,
                old_size,
                size);
...
static int remove_named_tab(DbTable *tb, int have_lock)
{
  ...
  if (bucket->pu.tb == nullptr) {
    ...
    if (cnt == 2) { /* multi -> single */
      size_t size;
      meta_name_tab_entry_t *entries = bucket->pu.mvec;
      *bucket = entries[1 - i];
      size = sizeof(meta_name_tab_entry_t) * cnt;
      ...
      old_size = sizeof(meta_name_tab_entry_t) * (cnt + 1);
      size = sizeof(meta_name_tab_entry_t) * cnt;
      bucket->pu.mvec = (meta_name_tab_entry_t *)erts_db_realloc_nt(ERTS_ALC_T_DB_NTAB_ENT,
                        bucket->pu.mvec,
                        old_size,
                        size);
      ERTS_ETS_MISC_MEM_ADD(size - old_size);
...
BIF_RETTYPE ets_safe_fixtable_2(BIF_ALIST_2)
{
  ...
  if ((tb = db_get_table(BIF_P, BIF_ARG_1, DB_READ, kind)) == nullptr) {
...
BIF_RETTYPE ets_update_element_3(BIF_ALIST_3)
{
  ...
  if ((tb = db_get_table(BIF_P, BIF_ARG_1, DB_WRITE, LCK_WRITE_REC)) == nullptr) {
    BIF_ERROR(BIF_P, BADARG);
  }
...
** Returns new_ value(s) (integer or [integer])
*/
BIF_RETTYPE ets_update_counter_3(BIF_ALIST_3)
{
  ...
  Eterm *ret_list_currp = nullptr;
  Eterm *ret_list_prevp = nullptr;
  Eterm iter;
...
BIF_RETTYPE ets_insert_2(BIF_ALIST_2)
{
  ...
  if ((tb = db_get_table(BIF_P, BIF_ARG_1, DB_WRITE, kind)) == nullptr) {
    BIF_ERROR(BIF_P, BADARG);
  }
...
#ifdef HARDDEBUG
  erts_fprintf(stderr,
               "ets:new_(%T,%T)=%T; Process: %T, initial: %T:%T/%bpu\n",
               BIF_ARG_1, BIF_ARG_2, ret, BIF_P->common.id,
               BIF_P->initial[0], BIF_P->initial[1], BIF_P->initial[2]);
  erts_fprintf(stderr, "ets: new_: meta_pid_to_tab common.memory_size = %ld\n",
               erts_smp_atomic_read_nob(&meta_pid_to_tab->common.memory_size));
  erts_fprintf(stderr, "ets: new_: meta_pid_to_fixed_tab common.memory_size = %ld\n",
               erts_smp_atomic_read_nob(&meta_pid_to_fixed_tab->common.memory_size));
#endif
...
BIF_RETTYPE ets_lookup_2(BIF_ALIST_2)
{
  ...
  if ((tb = db_get_table(BIF_P, BIF_ARG_1, DB_READ, LCK_READ)) == nullptr) {
    BIF_ERROR(BIF_P, BADARG);
  }
...
BIF_RETTYPE ets_delete_1(BIF_ALIST_1)
{
  ...
  if ((tb = db_get_table(BIF_P, BIF_ARG_1, DB_WRITE, LCK_WRITE)) == nullptr) {
    BIF_ERROR(BIF_P, BADARG);
  }
  ...
  if (erts_smp_rwmtx_tryrwlock(mmtl) == EBUSY) {
    /*
     * We keep our increased refc over this_ op in order to
...
badarg:
  ...
  if (to_proc != nullptr && to_proc != BIF_P) {
    erts_smp_proc_unlock(to_proc, to_locks);
  }
...
BIF_RETTYPE ets_setopts_2(BIF_ALIST_2)
{
  ...
  for (tail = is_tuple(BIF_ARG_2) ? CONS(fakelist, BIF_ARG_2, NIL) : BIF_ARG_2;
    ...
    case am_protection:
      if (arityval(tp[0]) != 2 || protection != 0) {
        goto badarg;
      }

      switch (tp[2]) {
      case am_private:
        protection = DB_PRIVATE;
        break;

      case am_protected:
        protection = DB_PROTECTED;
        break;

      case am_public:
        protection = DB_PUBLIC;
        break;
  ...
  if (tail != NIL
      || (tb = db_get_table(BIF_P, BIF_ARG_1, DB_WRITE, LCK_WRITE)) == nullptr
      || tb->common.owner != BIF_P->common.id) {
    goto badarg;
  }
...
static BIF_RETTYPE ets_select_delete_1(BIF_ALIST_1)
{
  ...
  if ((tb = db_get_table(p, tptr[1], DB_WRITE, kind)) == nullptr) {
    BIF_ERROR(p, BADARG);
  }
...
/*
** Return a list of tables on this_ node
*/
...
ets_select3(Process *p, Eterm arg1, Eterm arg2, Eterm arg3)
{
  ...
  if ((tb = db_get_table(p, arg1, DB_READ, LCK_READ)) == nullptr) {
    BIF_ERROR(p, BADARG);
  }
...
static BIF_RETTYPE ets_select_trap_1(BIF_ALIST_1)
{
  ...
  if ((tb = db_get_table(p, tptr[1], DB_READ, kind)) == nullptr) {
    BIF_ERROR(p, BADARG);
  }
...
static BIF_RETTYPE ets_select1(Process *p, Eterm arg1)
{
  ...
  if (arityval(*tptr) < 1 ||
      (tb = db_get_table(p, tptr[1], DB_READ, LCK_READ)) == nullptr) {
    BIF_ERROR(p, BADARG);
  }
...
BIF_RETTYPE ets_info_1(BIF_ALIST_1)
{
  ...
  */
  ...
  if ((tb = db_get_table(BIF_P, BIF_ARG_1, DB_INFO, LCK_READ)) == nullptr) {
...
error:
  ...
  for (lst = BIF_ARG_1; is_list(lst); lst = CDR(list_val(lst))) {
    ...
    res = db_prog_match(BIF_P, mp, CAR(list_val(lst)), nullptr, nullptr, 0,
      ...
      while (state->slots.ix < state->slots.size) {
        ...
        if (tb) {
          ...
          if (!(tb->common.status & DB_DELETE)) {
            ...
            for (pp = &tb->common.fixations; *pp != nullptr;
                 pp = &(*pp)->next) {
              if ((*pp)->pid == pid) {
                DbFixation *fix = *pp;
                               ...
                               db_lock_kind_t *kind_p)
{
  ...
  for (pp = &tb->common.fixations; *pp != nullptr; pp = &(*pp)->next) {
...
unlocked:
  ...
  if (!IS_FIXED(tb) && IS_HASH_TABLE(tb->common.status)
      && erts_smp_atomic_read_nob(&tb->hash.fixdel) != (erts_aint_t)nullptr) {
#ifdef ERTS_SMP
...
static Eterm table_info(Process *p, DbTable *tb, Eterm What)
{
   ...
   */
    ...
    print_table(ERTS_PRINT_STDOUT, nullptr, 1, tb);
    ret = am_true;
 ...
 *
 * Given a name atom (as returned from ets:new_/2), return a list of 'cnt'
  ...
  while (cnt) {
    if (index != atom_val(name)) {
      while (index >= atom_table_size()) {
        ...
        erts_atom_put((uint8_t *) tmp, strlen(tmp), ERTS_ATOM_ENC_LATIN1, 1);
      }
