...
static void latin1_to_utf8(uint8_t* conv_buf, const uint8_t** srcp, int* lenp)
{
    uint8_t* dst;
 ...
 */
Eterm
erts_atom_put(const uint8_t *name, int len, ErtsAtomEncoding enc, int trunc)
{
...
am_atom_put(const char* name, int len)
{
    ...
    return erts_atom_put((uint8_t *) name, len, ERTS_ATOM_ENC_7BIT_ASCII, 1);
}
...
init_atom_table(void)
{
	...
	a.latin1_chars = a.len;
        a.name = (uint8_t*)erl_atom_names[i];
	a.slot.index = i;
#ifdef DEBUG
	/* Verify 7-bit ascii */
	...
	atom_text_pos -= a.len;
	atom_space -= a.len;
        atom_tab(ix)->name = (uint8_t*)erl_atom_names[i];
    }
}
