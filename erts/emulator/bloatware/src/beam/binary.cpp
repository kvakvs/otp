...
erts_init_binary(void)
{
	...
	/* I assume that any compiler should be able to optimize this_
	   away. If not, this_ test is not very expensive... */
...
/*
 * Create a brand new_ binary from scratch.
 */
...
Eterm erts_new_mso_binary(Process *p, uint8_t *buf, int len)
{
    ProcBin* pb;
    Binary* bptr;
...
/*
 * Create a brand new_ binary from scratch on the heap.
 */
...
erts_realloc_binary(Eterm bin, size_t size)
{
	...
	pb->val = newbin;
	pb->size = size;
	pb->bytes = (uint8_t*) newbin->orig_bytes;
	pb->flags = 0;
	bin = make_binary(pb);
    }
    return bin;
...
uint8_t*
erts_get_aligned_binary_bytes_extra(Eterm bin, uint8_t** base_ptr, ErtsAlcType_t allocator, unsigned extra)
{
    uint8_t* bytes;
    Eterm* real_bin;
    Uint byte_size;
    ...
    if (bit_offs) {
	uint8_t* buf = (uint8_t *) erts_alloc(allocator, byte_size + extra);
	*base_ptr = buf;
	buf += extra;
...
{   
	...
	bytes = (uint8_t*) erts_alloc(ERTS_ALC_T_TMP, sizeof(uint8_t)*size);
...
binary_to_list(Process *c_p, Eterm *hp, Eterm tail, uint8_t *bytes, Uint size, Uint bitoffs)
{
	...
	Binary *mbp = erts_create_magic_binary(sizeof(ErtsB2LState),
        ...
        auto sp = ERTS_MAGIC_BIN_DATA<ErtsB2LState *>(mbp);
	Eterm mb;
...
static BIF_RETTYPE binary_to_list_continue(BIF_ALIST_1)
{
    ...
    return binary_to_list_chunk(BIF_P,
				BIF_ARG_1,
                                ERTS_MAGIC_BIN_DATA<ErtsB2LState *>(mbp),
...
{
    auto sp = ERTS_MAGIC_BIN_DATA<ErtsL2BState *>(mbp);
		     ...
		     int gc_disabled)
{
	...
	if (!gc_disabled) {
	    ...
	    Binary *mbp = erts_create_magic_binary(sizeof(ErtsL2BState),
            ...
            auto new_sp = ERTS_MAGIC_BIN_DATA<ErtsL2BState *>(mbp);
...
static BIF_RETTYPE list_to_binary_continue(BIF_ALIST_1)
{
    ...
    return list_to_binary_chunk(BIF_ARG_1,
                                ERTS_MAGIC_BIN_DATA<ErtsL2BState *>(mbp),
				ERTS_BIF_REDS_LEFT(BIF_P),
				1);
}
...
BIF_RETTYPE erts_list_to_binary_bif(Process *c_p, Eterm arg, Export *bif)
{
    ...
    if (is_nil(arg))
	ERTS_BIF_PREP_RET(ret, new_binary(c_p, (uint8_t *) "", 0));
	...
	if (is_binary(h)
		...
		if (size == 0) {
                    ERTS_BIF_PREP_RET(ret, new_binary(c_p, (uint8_t *) nullptr, 0));
		    break; /* done */
		}
...
BIF_RETTYPE list_to_bitstring_1(BIF_ALIST_1)
{
    ...
    if (is_nil(BIF_ARG_1))
	ERTS_BIF_PREP_RET(ret, new_binary(BIF_P, (uint8_t *) "", 0));
	...
	if (is_binary(h) && is_nil(t)) {
		...
		ErlDrvSizeT size = state.buf.iolist.size;
                ...
                state.bin = new_binary(BIF_P, (uint8_t *) nullptr, size);
