...
**
** Fine grained locking is supported by this_ code to allow concurrent updates
...
**     that this_ does not mean that (pointers to) table objects are guaranteed
...
#else /* ERTS_SMP */
# define RLOCK_HASH(tb,hval) nullptr
# define WLOCK_HASH(tb,hval) nullptr
...
/* How the table segments relate to each other:
   ...
   | bucket[255]     |   |   |        |       [511]    |     |  |       [767] |
   |-----------------|   |   |        |----------------|     |  #=============#
   | prev_segtab=nullptr|   |   |   +--<---prev_segtab    |     |
   | nsegs = 2       |   |   |   |    | nsegs = 256    |     |
+->| segtab[0] -->-------+---|---|--<---segtab[0]      |<-+  |
|  | segtab[1] -->-----------+---|--<---segtab[1]      |  |  |
...
#define EQ_REL(x,y,y_base) \
    (is_same(x,nullptr,y,y_base) || (is_not_both_immed((x),(y)) && eq_rel((x),nullptr,(y),y_base)))

/* Is this_ a live object (not pseodo-deleted) with the specified key?
*/
...
/* Has this_ object the specified key? Can be pseudo-deleted.
*/
...
static ERTS_INLINE HashDbTerm *new_dbterm(DbTableHash *tb, Eterm obj)
{
  ...
  if (tb->common.compress) {
    p = (HashDbTerm *)db_store_term_comp(&tb->common, nullptr, offsetof(HashDbTerm, dbterm), obj);
  } else {
    p = (HashDbTerm *)db_store_term(&tb->common, nullptr, offsetof(HashDbTerm, dbterm), obj);
  }
    ...
    Eterm obj)
{
  ...
  if (tb->common.compress) {
    ret = (HashDbTerm *)db_store_term_comp(&tb->common, &(old->dbterm), offsetof(HashDbTerm, dbterm),
                                           obj);
  } else {
...
static void restore_fixdel(DbTableHash *tb, FixedDeletion *fixdel)
{
  ...
  if (erts_smp_atomic_cmpxchg_relb(&tb->fixdel,
                                   (erts_aint_t) fixdel,
                                   (erts_aint_t) nullptr) != (erts_aint_t) nullptr) {
...
Uint db_kept_items_hash(DbTableHash *tb)
{
  ...
  do {
    for (b = BUCKET(tb, ix); b != nullptr; b = b->next) {
      if (b->hvalue == INVALID_HASH) {
        ++kept_items;
      }
    }
...
int db_create_hash(Process *p, DbTable *tbl)
{
  ...
  erts_smp_atomic_init_nob(&tb->fixdel, (erts_aint_t)nullptr);
  erts_smp_atomic_init_nob(&tb->segtab, (erts_aint_t)nullptr);
  SET_SEGTAB(tb, alloc_ext_seg(tb, 0, nullptr)->segtab);
  tb->nsegs = NSEG_1;
  tb->nslots = SEGSZ;
  ...
  if (tb->common.status & DB_SET) {
    ...
    *bp = q;
    ...
    do {
      ...
      q = q->next;
    } while (q != nullptr && has_key(tb, q, key, hval));
...
int db_get_hash(Process *p, DbTable *tbl, Eterm key, Eterm *ret)
{
  ...
  while (b1 != 0) {
    if (has_live_key(tb, b1, key, hval)) {
      ...
      if (tb->common.status & (DB_BAG | DB_DUPLICATE_BAG)) {
        while (b2 != nullptr && has_key(tb, b2, key, hval)) {
          b2 = b2->next;
        }
      }
  ...
  while (b1 != 0) {
    if (has_live_key(tb, b1, key, hval)) {
      if (tb->common.status & (DB_BAG | DB_DUPLICATE_BAG)) {
        ...
        while (b2 != nullptr && has_live_key(tb, b2, key, hval)) {
...
/*
** NB, this_ is for the db_erase/2 bif.
*/
...
/*
 * Continue collecting select matches, this_ may happen either due to a trap
...
#define RET_TO_BIF(Term, State) do { *ret = (Term); return State; } while(0);
  ...
  while ((current = BUCKET(tb, slot_ix)) == nullptr) {
...
#define RET_TO_BIF(Term,RetVal) do {    \
  ...
  *ret = (Term);        \
  ...
  if (!mpi.key_given) {
    /* Run this_ code if pattern is variable or GETKEY(pattern)  */
    /* is a variable                                            */
    slot_ix = 0;
    lck = RLOCK_HASH(tb, slot_ix);
    ...
    for (;;) {
      ...
      if ((current = BUCKET(tb, slot_ix)) != nullptr) {
        break;
      }
  ...
  if (chunk_size) {
    ...
    if (rest != NIL || slot_ix >= 0) { /* Need more calls */
      ...
      continuation = TUPLE6(hp, tb->common.id, make_small(slot_ix),
                            ...
                            mpb, rest,
                            make_small(rest_size));
      mpi.mp = nullptr; /*otherwise the return macro will destroy it */
      hp += 7;
      RET_TO_BIF(TUPLE2(hp, match_list, continuation), DB_ERROR_NONE);
    ...
    if (current != nullptr) {
      if (current->hvalue != INVALID_HASH) {
        if (db_match_dbterm(&tb->common, p, mpi.mp, 0,
                            &current->dbterm, nullptr, 0) == am_true) {
          ++got;
        }
...
#define RET_TO_BIF(Term,RetVal) do {    \
  *ret = (Term);        \
  ...
  if (slot_ix >= NACTIVE(tb)) { /* Is this_ posible? */
    RUNLOCK_HASH(lck);
    goto done;
  }
...
static int db_free_table_continue_hash(DbTable *tbl)
{
  ...
  erts_smp_atomic_set_relb(&tb->fixdel, (erts_aint_t)nullptr);
                           ...
                           struct mp_info *mpi)
{
  ...
  if (num_heads > 10) {
    ...
    mpi->lists = (mp_prefound *)erts_alloc(ERTS_ALC_T_DB_SEL_LIST,
    ...
    if (tpl == am_Underscore || db_is_variable(tpl) != -1) {
      ...
      if (is_value(key)) {
        if (!db_has_variable(key)) {   /* Bound key */
          ...
          if (lck == nullptr) {
            search_slot = search_list(tb, key, hval, *bp) != nullptr;
          } else {
...
/* Extend table with one new_ segment
*/
static int alloc_seg(DbTableHash *tb)
{
  ...
  if (seg_ix + 1 == tb->nsegs) { /* New segtab needed (extended segment) */
    ...
    segtab[seg_ix] = &seg->s;
    /* We don't use the new_ segtab until next call (see "shrink race") */
  } else { /* Just a new_ plain segment */
    struct segment **segtab;

    if (seg_ix == tb->nsegs) { /* Time to start use segtab from last call */
      struct ext_segment *eseg;
      eseg = (struct ext_segment *) SEGTAB(tb)[seg_ix - 1];
      MY_ASSERT(eseg != nullptr && eseg->s.is_ext_segment);
...
static int free_seg(DbTableHash *tb, int free_records)
{
   ...
   * For this_ to work, we must of course allocate a new_ segtab one call
   * earlier in alloc_seg() as well. And this_ is also the reason why
  ...
  if (seg_ix > 0) {
    if (seg_ix < tb->nsegs) {
      SEGTAB(tb)[seg_ix] = nullptr;
    }
  } else {
    SET_SEGTAB(tb, nullptr);
  }
...
/* Grow table with one new_ bucket.
** Allocate new_ segment if needed.
*/
static void grow(DbTableHash *tb, int nactive)
{
  HashDbTerm **pnext;
                        ...
                        HashDbTerm *list)
{
  ...
  for (list = list->next; list != nullptr; list = list->next) {
    if (list->hvalue != INVALID_HASH) {
      return list;
    }
  }
