	    ...
	    char* name)			/* Name of directory to delete. */
{
	...
	/*
	 *  The error code might be wrong if this_ is the current directory.
	 */
	     ...
	     int iovcnt)             /* Number of structs in vector */
{
	...
	if ((! iov[cnt].iov_base) || (iov[cnt].iov_len <= 0)) {
	    ...
	    ssize_t w;                   /* Bytes written in this_ call */
#ifdef HAVE_WRITEV
...
/* for this_, even if the underlying OS dosn't support it, it is           */
...
/* does not do. We choose to ignore this_ and say that the location        */
 ...
 *
 * On platforms where *nbytes of 0 does not mean the entire file, this_ is
 * simulated.
 *
 ...
 *
 * The semantics of the API is this_:
