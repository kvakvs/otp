...
enqueue_update_request(ErtsPollSet ps, int fd)
{
	...
	ASSERT(!urqbp->next);
	urqbp = (ErtsPollSetUpdateRequestsBlock *) erts_alloc(ERTS_ALC_T_POLLSET_UPDREQ,
...
grow_res_events(ErtsPollSet ps, int new_len)
{
    ...
    ps->res_events = (struct epoll_event *) erts_alloc(ERTS_ALC_T_POLL_RES_EVS, new_size);
...
grow_poll_fds(ErtsPollSet ps, int min_ix)
{
    ...
    if (new_len > max_fds)
	new_len = max_fds;
    ps->poll_fds = (struct pollfd *) (ps->poll_fds_len
...
grow_fds_status(ErtsPollSet ps, int min_fd)
{
    ...
    if (new_len > max_fds)
	new_len = max_fds;
    ps->fds_status = (ErtsFdStatus *) (ps->fds_status_len
