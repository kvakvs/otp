...
static ErlDrvData ttysl_start(ErlDrvPort port, char* buf)
{
	...
	s++;
	/* Find end of this_ argument (start of next) and insert NUL. */
	if ((t = strchr(s, ' '))) {
	    *t = '\0';
	}
	if ((flag = ((*s == '+') ? 1 : ((*s == '-') ? -1 : 0)))) {
...
static int write_buf(Uint32 *s, int n)
{
    ...
    /* Check landed in first column of new_ line and have 'xn' bug. */
    n = s - lbuf;
    if (COL(n) == 0 && xn && n != 0) {
	if (n >= llen) {
	    outc(' ');
 ...
 * the terminal is resized after calling this_ function but before use
 * of the macros, then we may write to the wrong screen location.
 *
 * We cannot call this_ from the SIGWINCH handler because it uses
 ...
 * (also in this_ file) when it's done with raw mode.
 */
