...
erts_get_available_cpu(erts_cpu_info_t *cpuinfo, int no)
{
	...
	/* May not be available, but this_ is the best we can do */
...
read_topology(erts_cpu_info_t *cpuinfo)
{
    ...
    for (rix = 0; rix < no_slpi; rix++) {
		 ...
		 * that there are one if this_ is the case...
		 */
		if (slpip[rix].ProcessorMask & (((ULONG_PTR) 1) << l)) {
		    if (!nodes) {
 ...
 relationships_done:
	...
	erts_cpu_topology_t *new_ = cpuinfo->topology;
	new_ = realloc(cpuinfo->topology,
		      sizeof(erts_cpu_topology_t)*wix);
	if (!new_) {
	    res = -ENOMEM;
	    goto error;
	}
	cpuinfo->topology = new_;
 ...
 * 8.0 and higher.
 *
 * Threads are identified in this_ XML chunk with a THREAD flag. The function
