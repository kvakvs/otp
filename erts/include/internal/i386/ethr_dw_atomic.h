				 ...
				 ethr_sint_t *xchg)
{
	...
	: "m"(*p), "1"(xchg[1]), "2"(xchg[0]), "c"(new_[1]), "r"(new_[0])
	: "cc", "memory");
	...
	"setz %3\n\t"
	: "=m"(*p), "=d"(xchg[1]), "=a"(xchg[0]), "=q"(xchgd)
	: "m"(*p), "1"(xchg[1]), "2"(xchg[0]), "c"(new_[1]), "b"(new_[0])
	: "cc", "memory");
...
ethr_native_su_dw_atomic_read(ethr_native_dw_atomic_t *var)
{
	...
	new_[0] = new_[1] = xchg.sint[0] = xchg.sint[1] = 0x83838383;
	(void) ethr_native_dw_atomic_cmpxchg_mb(var, new_, xchg.sint);
	return xchg.sint64;
    }
}
			     ...
			     ethr_sint64_t val)
{
	...
	ethr_dw_atomic_no_sse2_convert_t new_;
	new_.sint64 = val;
	while (!ethr_native_dw_atomic_cmpxchg_mb(var, new_.sint, xchg));
    }
}
