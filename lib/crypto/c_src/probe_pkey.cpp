...
#endif
#ifdef HAVE_SLH_DSA
    pkey_probe_t::from_slhdsa_params("slh_dsa_shake_128s", "SLH-DSA-SHAKE-128s",
                                     EVP_PKEY_SLH_DSA_SHAKE_128S),
    pkey_probe_t::from_slhdsa_params("slh_dsa_shake_128f", "SLH-DSA-SHAKE-128f",
                                     EVP_PKEY_SLH_DSA_SHAKE_128F),
    pkey_probe_t::from_slhdsa_params("slh_dsa_sha2_128s", "SLH-DSA-SHA2-128s",
                                     EVP_PKEY_SLH_DSA_SHA2_128S),
    pkey_probe_t::from_slhdsa_params("slh_dsa_sha2_128f", "SLH-DSA-SHA2-128f",
                                     EVP_PKEY_SLH_DSA_SHA2_128F),
    pkey_probe_t::from_slhdsa_params("slh_dsa_shake_192s", "SLH-DSA-SHAKE-192s",
                                     EVP_PKEY_SLH_DSA_SHAKE_192S),
    pkey_probe_t::from_slhdsa_params("slh_dsa_shake_192f", "SLH-DSA-SHAKE-192f",
                                     EVP_PKEY_SLH_DSA_SHAKE_192F),
    pkey_probe_t::from_slhdsa_params("slh_dsa_sha2_192s", "SLH-DSA-SHA2-192s",
                                     EVP_PKEY_SLH_DSA_SHA2_192S),
    pkey_probe_t::from_slhdsa_params("slh_dsa_sha2_192f", "SLH-DSA-SHA2-192f",
                                     EVP_PKEY_SLH_DSA_SHA2_192F),
    pkey_probe_t::from_slhdsa_params("slh_dsa_shake_256s", "SLH-DSA-SHAKE-256s",
                                     EVP_PKEY_SLH_DSA_SHAKE_256S),
    pkey_probe_t::from_slhdsa_params("slh_dsa_shake_256f", "SLH-DSA-SHAKE-256f",
                                     EVP_PKEY_SLH_DSA_SHAKE_256F),
    pkey_probe_t::from_slhdsa_params("slh_dsa_sha2_256s", "SLH-DSA-SHA2-256s",
                                     EVP_PKEY_SLH_DSA_SHA2_256S),
    pkey_probe_t::from_slhdsa_params("slh_dsa_sha2_256f", "SLH-DSA-SHA2-256f",
                                     EVP_PKEY_SLH_DSA_SHA2_256F),
#endif
};

pkey_collection_t pkey_collection("crypto.pkey_collection", pkey_probes,
                                  sizeof(pkey_probes) / sizeof(pkey_probes[0]));
...
extern "C" ERL_NIF_TERM pkey_algorithms_as_list(ErlNifEnv *env,
                                                const bool fips_enabled) {
  return pkey_collection.to_list(env, fips_enabled);
}

ERL_NIF_TERM pkey_type_t::get_atom() const { return this->init->atom; }
...
void pkey_type_t::check_usage_types(const bool fips_enabled) {
  ...
  auto_pkey_ctx_t ctx(EVP_PKEY_CTX_new_from_name(
      nullptr, this->init->get_v3_name(), fips_filter));
  ...
  ctx.reset(EVP_PKEY_CTX_new_from_name(nullptr, this->init->get_v3_name(),
                                       fips_filter));
...
// allow the algorithm.
void pkey_probe_t::probe(ErlNifEnv *env, const bool fips_enabled,
                         std::vector<pkey_type_t> &output) {
  ...
  if (this->evp_pkey_id) {
    output.back().alg.reset(
        EVP_SIGNATURE_fetch(nullptr, this->get_v3_name(), nullptr));
  }
...
extern "C" pkey_type_C *get_pkey_type(ErlNifEnv *env, ERL_NIF_TERM atom) {
  ...
  for (auto p = pkey_collection.begin(env, fips_enabled);
       p != pkey_collection.end(fips_enabled); ++p) {
    if (p->get_atom() == atom) {
      return &*p;
    }
  }
