...
curve_collection_t curve_collection("crypto.curve_collection", curve_probes,
                                    sizeof(curve_probes) /
                                        sizeof(curve_probes[0]));

//
// Implementation of Curve Algorithm storage API
//

extern "C" ERL_NIF_TERM curve_algorithms_as_list(ErlNifEnv *env,
                                                 const bool fips_enabled) {
  return curve_collection.to_list(env, fips_enabled);
}

// Check if the curve in nid is supported by the current cryptolib and current
// FIPS state.
...
void curve_type_t::check_fips_availability(const bool fips_mode) {
#if defined(FIPS_SUPPORT) && defined(HAS_3_0_API)
  // This checking code only runs under FIPS and OpenSSL 3+, other cases
  ...
  const auto_pkey_ctx_t pctx(
      EVP_PKEY_CTX_new_from_name(nullptr, "EC", get_fips_filter(fips_mode)));
  if (!pctx) {
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
      OSSL_PKEY_PARAM_GROUP_NAME, const_cast<char *>(this->init->sn), 0);
...
void curve_probe_t::probe(ErlNifEnv *env, const bool fips_mode,
                          std::vector<curve_type_t> &output) {
  this->atom = create_or_existing_atom(env, this->sn, this->atom);
  this->resolve_nid();
#ifdef HAVE_EC
  // Some curves can be pre-checked by their NID. Passing NID=0 will skip this
  // check
