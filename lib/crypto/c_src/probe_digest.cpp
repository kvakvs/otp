...
#endif
#ifdef HAVE_SHAKE128
    digest_probe_t("shake128", "SHAKE-128", &EVP_shake128)
        .set_xof_default_length(16),
#endif
#ifdef HAVE_SHAKE256
    digest_probe_t("shake256", "SHAKE-256", &EVP_shake256)
        .set_xof_default_length(32),
#endif
#ifdef HAVE_SM3
...
digest_collection_t
    digest_collection("crypto.digest.digest_collection", digest_probes,
...
extern "C" ERL_NIF_TERM digest_types_as_list(ErlNifEnv *env,
                                             const bool fips_forbidden) {
...
void digest_type_t::create_md_resource(const bool fips_enabled) {
#if defined(FIPS_SUPPORT) && defined(HAS_3_0_API)
  auto_md_t fetched_md(EVP_MD_fetch(nullptr, this->init->get_v3_name(),
  ...
  // Record failed algorithm instantiation for FIPS enabled & OpenSSL API 3.0
  // only
  if (fips_enabled && !check_valid_in_fips(fetched_md.pointer)) {
    ...
    this->resource =
        std::move(fetched_md); // pass ownership and move data to this->md
  }
#else
  // construct from the old API, each probe has a constructor function
  this->resource.reset(
      const_cast<evp_md_pointer_type_t>(this->init->v1_ctor()));
#endif // HAS_3_0_API && FIPS_SUPPORT
}

digest_type_t::digest_type_t(const digest_probe_t *init_)
    : init(init_), flags(init_->flags),
      xof_default_length(init_->xof_default_length) {}

void digest_probe_t::probe(ErlNifEnv *env, const bool fips_mode,
                           std::vector<digest_type_t> &output) {
  output.emplace_back(this);
  // Unavailable are skipped. Available are added. Forbidden are added, but with
...
extern "C" const EVP_MD *get_digest_type_resource(const digest_type_C *p) {
  return p ? p->resource.pointer : nullptr;
}
