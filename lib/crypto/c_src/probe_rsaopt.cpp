...
rsaopt_collection_t rsaopt_collection("crypto.rsaopt_collection", rsaopt_probes,
                                      sizeof(rsaopt_probes) /
                                          sizeof(rsaopt_probes[0]));
...
extern "C" ERL_NIF_TERM rsaopts_as_list(ErlNifEnv *env,
...
// allow the algorithm.
void rsaopt_probe_t::probe(ErlNifEnv *env, const bool fips_enabled,
                           std::vector<rsaopt_type_t> &output) {
