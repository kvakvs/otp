...
mac_probe_t mac_probes[] = {make_poly1305_probe(), make_hmac_probe(),
                            make_cmac_probe()};

mac_collection_t mac_collection("crypto.mac_collection", mac_probes,
                                sizeof(mac_probes) / sizeof(mac_probes[0]));
...
extern "C" ERL_NIF_TERM mac_algorithms_as_list(ErlNifEnv *env,
...
#endif
  return !fips_forbidden &&
         !this->flags.algorithm_init_failed; // && this->init->type != NO_mac
}

#if defined(HAS_3_0_API) && defined(FIPS_SUPPORT)
// This must be run after this->evp_mac is already initialized in
// fetch_algorithm()
void mac_type_t::update_flags_check_fips_availability(const bool fips_enabled) {
  // Initialize an algorithm to check that all its dependencies are valid in
  // FIPS
  if (this->evp_mac) {
    ...
    // Try to initialize the digest algorithm for use, this will check the
    // dependencies
...
void mac_type_t::fetch_algorithm(const bool fips_enabled) {
  ...
  this->evp_mac.reset(EVP_MAC_fetch(nullptr, this->init->str_v3,
                                    get_fips_filter(fips_enabled)));
...
// allow the algorithm.
void mac_probe_t::probe(ErlNifEnv *env, const bool fips_enabled,
                        std::vector<mac_type_t> &output) {
...
extern "C" mac_type_C *find_mac_type_by_name_keylen(ErlNifEnv *env,
                                                    ERL_NIF_TERM type,
                                                    const size_t key_len) {
  const bool fips_enabled = FIPS_MODE();
  for (auto p = mac_collection.begin(env, fips_enabled);
       p != mac_collection.end(fips_enabled); ++p) {
    if (p->get_atom() == type &&
        (p->init->key_len == 0 || p->init->key_len == key_len)) {
      return &*p;
    }
  }
  return nullptr;
...
extern "C" mac_type_C *find_mac_type_by_name(ErlNifEnv *env,
                                             ERL_NIF_TERM type) {
  const bool fips_enabled = FIPS_MODE();
  for (auto p = mac_collection.begin(env, fips_enabled);
       p != mac_collection.end(fips_enabled); ++p) {
