...
kem_collection_t kem_collection("crypto.kem_collection", kem_probes,
                                sizeof(kem_probes) / sizeof(kem_probes[0]));
#else
...
extern "C" ERL_NIF_TERM kem_algorithms_as_list(ErlNifEnv *env,
...
bool kem_type_t::check_kem_algorithm(bool fips_enabled) {
  ...
  const auto_pkey_ctx_t ctx(EVP_PKEY_CTX_new_from_name(
      nullptr, this->init->str_v3, get_fips_filter(fips_enabled)));
...
// allow the algorithm.
void kem_probe_t::probe(ErlNifEnv *env, const bool fips_enabled,
                        std::vector<kem_type_t> &output) {
  // Nothing will happen if HAVE_ML_KEM is not defined, the output will remain
  // empty
#ifdef HAVE_ML_KEM
